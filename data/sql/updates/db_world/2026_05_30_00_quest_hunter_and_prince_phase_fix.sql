...
WHERE `entryorguid` = 32497 AND `source_type` = 0;
...
--    These fire from the quest script (source_type 5), entryorguid = questId.
--    target_type 7 = SMART_TARGET_ACTION_INVOKER (the player who failed/abandoned).
-- ============================================================
DELETE FROM `smart_scripts`
WHERE `entryorguid` IN (13400, 13361) AND `source_type` = 5;
