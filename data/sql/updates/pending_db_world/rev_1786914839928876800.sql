...
-- Drop the personal-achievement gate on the gossip event - CONDITION_ACHIEVEMENT on 4530 / 4597,
-- "The Lich King (10 / 25 player)". The plaque is hidden outright until the realm kill, and past
-- that the cinematic is open to anyone who can see it.
DELETE FROM `conditions` WHERE `SourceTypeOrReferenceId` = 22 AND `SourceGroup` = 1 AND `SourceEntry` = 202443 AND `SourceId` = 1;

-- Event 2 above is what turns the fountain (202616, guid 151164) into the statue. Stock AC gated it
-- on CONDITION_REALM_ACHIEVEMENT "Realm First! Light of Dawn" (4576), tying a permanent city feature
-- to one guild winning a realm-first race: on any realm where nobody ever took it, the statue never
-- appears at all. Gate it on the same worldstate the plaque uses instead. SmartAI re-polls a failed
-- condition every 5s (SmartScript::ProcessTimedAction), so the reveal needs no timer of its own.
--
-- ConditionTarget = 1 is load-bearing: SMART_EVENT_UPDATE_OOC dispatches with unit == nullptr, and
-- Condition::Meets rejects a null mConditionTargets[ConditionTarget] before it reads ConditionType.
DELETE FROM `conditions` WHERE `SourceTypeOrReferenceId` = 22 AND `SourceGroup` = 3 AND `SourceEntry` = 202443 AND `SourceId` = 1;
INSERT INTO `conditions` (`SourceTypeOrReferenceId`, `SourceGroup`, `SourceEntry`, `SourceId`, `ElseGroup`, `ConditionTypeOrReference`, `ConditionTarget`, `ConditionValue1`, `ConditionValue2`, `ConditionValue3`, `NegativeCondition`, `ErrorType`, `ErrorTextId`, `ScriptName`, `Comment`) VALUES
(22, 3, 202443, 1, 0, 11, 1, 20009, 1, 0, 0, 0, 0, '', 'Dedication of Honor - reveal the statue once the realm has defeated the Lich King');
