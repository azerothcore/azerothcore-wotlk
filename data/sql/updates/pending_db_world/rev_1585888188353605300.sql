INSERT INTO `version_db_world` (`sql_rev`) VALUES ('1585888188353605300');

-- fix creatures come together and don't move randomly
UPDATE `creature` SET `position_x`=5608.7485, `position_y`=5587.217, `position_z`=-88.64437, `spawndist`=8, `MovementType`=1 WHERE `guid`=104560;
UPDATE `creature` SET `position_x`=5659.1381, `position_y`=5556.411, `position_z`=-85.08512, `spawndist`=8, `MovementType`=1 WHERE `guid`=104561;
UPDATE `creature` SET `position_x`=5618.4741, `position_y`=5537.622, `position_z`=-88.30284, `spawndist`=8, `MovementType`=1 WHERE `guid`=105391;
UPDATE `creature` SET `position_x`=5638.1625, `position_y`=5495.338, `position_z`=-84.40144, `spawndist`=8, `MovementType`=1 WHERE `guid`=105390;
UPDATE `creature` SET `position_x`=5814.5957, `position_y`=5253.0585, `position_z`=-94.8729 WHERE `guid`=132681;
UPDATE `creature` SET `position_x`=5815.2612, `position_y`=5248.3652, `position_z`=-81.3897 WHERE `guid`=132860;
