INSERT INTO `version_db_world` (`sql_rev`) VALUES ('1625739882460062000');
...
-- ==========================================
UPDATE `creature` SET `wander_distance`=3, `MovementType`=1 WHERE `guid` IN (79910,79908,79905,79909);
UPDATE `creature` SET `wander_distance`=8, `MovementType`=1 WHERE `guid` IN (79904,79906,79902,79903,79913,79911,79916,79914,79915);


-- ====================================================
-- Add some Random Movement For 4 KIds At Eastvale Camp
-- ====================================================
UPDATE `creature` SET `wander_distance` =8, `MovementType`=1 WHERE `id` IN (798,800,802,801);

-- ===================================
-- Remove Random Movement For Some NPC
-- ===================================
UPDATE `creature` SET `wander_distance` =0, `MovementType`=0 WHERE `guid` IN (80534,80535,80536,80518,80519,80520,80521,80512,80513,80514,80499,80500,80501,80506,80507,80508,80523,80525,80526,80518,80519,80520,80521,80747,80748,81151,81152,81153,81122,81131,81133,81141,81142,81143,81144,81117,81118,81119,81120,80393,80394);
UPDATE `creature` SET `wander_distance` =0, `MovementType`=0 WHERE `guid` IN (79890,79888,79889,81106);

-- =================================
-- Generic NPC Movement Improvements
-- =================================
UPDATE `creature` SET `wander_distance` =15, `MovementType`=1 WHERE `id` IN (30,113,118,299,524,525,822,1922,883,1933,2442,721,1412) AND `MovementType` IN (0,1);


-- ==============================
-- Individual NPC Movement Tweaks
-- ==============================
UPDATE `creature` SET `wander_distance` =3, `MovementType`=1 WHERE `guid` IN (79918,79919);
UPDATE `creature` SET `wander_distance` =8, `MovementType`=1 WHERE `guid` IN (86159);
