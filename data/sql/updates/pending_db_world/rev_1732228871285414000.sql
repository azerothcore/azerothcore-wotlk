--
-- Currently in AC there's no `waypoints` data for `Cork Gizelton`. But there's `script_waypoint`. But in the case of:
-- Deletes all entries for `Cork Gizelton` between `waypoint id` 1 to 282 in `waypoints`.
DELETE FROM `waypoints` WHERE `entry` = 11625 AND`pointid` BETWEEN 1 AND 282;

-- Copies the waypoint data from `script_waypoint` to `waypoints` for `Cork Gizelton`
INSERT IGNORE INTO `waypoints` (`entry`, `pointid`, `position_x`, `position_y`, `position_z`, `delay`, `point_comment`) SELECT `entry`, `pointid`, `location_x`, `location_y`, `location_z`, `waittime`, `point_comment` FROM `script_waypoint` WHERE `entry` = 11625;

-- Deletes all entries for `Cork Gizelton` between `waypoint id` 1 to 282 in `script_waypoints`.
DELETE FROM `script_waypoint` WHERE `entry` = 11625 AND`pointid` BETWEEN 1 AND 282;
