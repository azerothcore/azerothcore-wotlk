UPDATE `creature_template` SET `speed_walk` = , `speed_run` = , `BaseAttackTime` = , `RangeAttackTime` =  WHERE `entry` = ;
UPDATE `creature_model_info` SET `BoundingRadius` = , `CombatReach` =  WHERE `DisplayID` = ;
