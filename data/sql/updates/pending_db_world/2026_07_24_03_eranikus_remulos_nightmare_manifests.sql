...
-- facing-reset onto his last dialogue line directly, so this isn't gated on the broken escort
-- either way - see below for why the escort itself is fixed too, restoring the actual walk.
...
-- All four escort paths above (11832, 15491, 1183200, 1183201) are empty in `waypoint_data` -
-- the table AC's engine actually reads (confirmed against WaypointMgr.cpp) - but this DB also
-- carries a legacy `waypoints` table (entry/pointid columns) with the real coordinates for all
-- of them, never migrated and never read by the current engine. Copies that data across so the
-- original escort-driven movement (Eranikus landing, Remulos's whole route) actually runs -
-- purely additive, doesn't touch any of the fixes above, which stay as the reliable trigger for
-- the actual unlock/flag logic regardless of whether the movement itself keeps working.
