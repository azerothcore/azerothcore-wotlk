...
public:
    ...
    // Attack state helpers for testing ValidateAttackersAndOwnTarget
    // Both sides must be TestCreature so we can access protected members
    void TestSetAttacking(TestCreature* victim);
    void TestClearAttacking();

    // Clear all combat state for cleanup
    void CleanupCombatState();

    // Aura effect injection for testing (pushes into m_modAuras)
    void TestPushModAura(uint32 type, AuraEffect* effect);
    void TestClearModAuras(uint32 type);
