...
void TestCreature::TestSetAttacking(TestCreature* victim)
{
    m_attacking = victim;
    if (victim)
        victim->m_attackers.insert(this);
}

void TestCreature::TestClearAttacking()
{
    if (m_attacking)
    {
        // m_attacking might not be a TestCreature, but in tests it always is
        static_cast<TestCreature*>(m_attacking)->m_attackers.erase(this);
        m_attacking = nullptr;
    }
}

void TestCreature::TestPushModAura(uint32 type, AuraEffect* effect)
{
    m_modAuras[type].push_back(effect);
}

void TestCreature::TestClearModAuras(uint32 type)
{
    m_modAuras[type].clear();
}
