...
protected:
    void SetUp() override
    {
        ...
        ON_CALL(*_worldMock, getIntConfig(CONFIG_NPC_EVADE_IF_NOT_REACHABLE)).WillByDefault(Return(10));
...
TEST_F(CombatManagerIntegrationTest, EvadeTimer_FullCountdown_Expires)
{
    ...
    // Advance the full 10 seconds (config default) — timer should expire
...
TEST_F(CombatManagerIntegrationTest, EvadeTimer_CountdownToRegenPhase)
{
    ...
    // With config default 10s, timer starts above EVADE_REGEN_DELAY (5000)
    EXPECT_TRUE(cm.IsInEvadeMode());
    EXPECT_FALSE(cm.IsEvadeRegen()); // timer 10000 > EVADE_REGEN_DELAY

    // Advance 6 seconds — timer at 4000, now in regen zone
    cm.Update(6000);
    EXPECT_TRUE(cm.IsEvadeRegen());
}
...
// ============================================================================
// Config-driven evade timer tests
// Verify StartEvadeTimer reads from CONFIG_NPC_EVADE_IF_NOT_REACHABLE
// ============================================================================

TEST_F(CombatManagerIntegrationTest,
       StartEvadeTimer_UsesConfigValue_TimerMatchesConfig)
{
    auto& cm = _creatureA->TestGetCombatMgr();

    // Config mock returns 10 for CONFIG_NPC_EVADE_IF_NOT_REACHABLE
    // So timer should be 10 * 1000 = 10000ms
    cm.StartEvadeTimer();
    EXPECT_TRUE(cm.IsInEvadeMode());

    // After 9999ms, still active
    cm.Update(9999);
    EXPECT_TRUE(cm.IsInEvadeMode());

    // After 1 more ms (total 10000), expires
    cm.Update(1);
    EXPECT_FALSE(cm.IsInEvadeMode());
}

TEST_F(CombatManagerIntegrationTest,
       StartEvadeTimer_DifferentConfigValue_ChangesTimer)
{
    // Override mock to return 3 seconds
    ON_CALL(*_worldMock, getIntConfig(CONFIG_NPC_EVADE_IF_NOT_REACHABLE))
    ...
    // After 2999ms, still active
    cm.Update(2999);
    EXPECT_TRUE(cm.IsInEvadeMode());

    // After 1 more ms (total 3000), expires
    cm.Update(1);
    EXPECT_FALSE(cm.IsInEvadeMode());

    // Restore mock to default 10s for other tests
    ON_CALL(*_worldMock, getIntConfig(CONFIG_NPC_EVADE_IF_NOT_REACHABLE))
        .WillByDefault(Return(10));
}

// ============================================================================
// EngageWithTarget integration tests
// Verify combat + threat creation through the public API
// ============================================================================

TEST_F(CombatManagerIntegrationTest,
       EngageWithTarget_CreatesThreatAndCombat)
{
    // Neither creature should be in combat initially
    EXPECT_FALSE(_creatureA->GetCombatManager().HasCombat());
    EXPECT_FALSE(_creatureB->GetCombatManager().HasCombat());
    EXPECT_TRUE(_creatureA->GetThreatMgr().IsThreatListEmpty());

    // A engages B
    _creatureA->EngageWithTarget(_creatureB);

    // Both should be in combat (bidirectional via CombatManager)
    EXPECT_TRUE(_creatureA->GetCombatManager().HasCombat());
    EXPECT_TRUE(_creatureB->GetCombatManager().HasCombat());

    // A should have B on its threat list
    EXPECT_TRUE(_creatureA->GetThreatMgr().IsThreatenedBy(_creatureB));

    // B should be on A's threat list (A threatens B via _threatenedByMe)
    EXPECT_FALSE(_creatureB->GetThreatMgr().IsThreatListEmpty()
                 && _creatureA->GetThreatMgr().IsThreatListEmpty());
}

TEST_F(CombatManagerIntegrationTest,
       EngageWithTarget_Null_DoesNotCrash)
{
    _creatureA->EngageWithTarget(nullptr);
    EXPECT_FALSE(_creatureA->GetCombatManager().HasCombat());
}

TEST_F(CombatManagerIntegrationTest,
       EngageWithTarget_AlreadyInCombat_AddsThreat)
{
    TestCreature* creatureC = new TestCreature();
    creatureC->SetupForCombatTest(_map, 3, 12347);
    creatureC->SetFaction(90002);
    ...
    // Now A also engages C
    _creatureA->EngageWithTarget(creatureC);
    EXPECT_TRUE(_creatureA->GetThreatMgr().IsThreatenedBy(creatureC));

    // B is still on the threat list
    EXPECT_TRUE(_creatureA->GetThreatMgr().IsThreatenedBy(_creatureB));
...
// ============================================================================
// CombatStop integration tests
// Verify combat and threat cleanup through the public API
// ============================================================================

TEST_F(CombatManagerIntegrationTest,
       CombatStop_ClearsCombatAndThreat)
{
    // Put A in combat with B
    _creatureA->EngageWithTarget(_creatureB);
    EXPECT_TRUE(_creatureA->GetCombatManager().HasCombat());
    EXPECT_TRUE(_creatureA->GetThreatMgr().IsThreatenedBy(_creatureB));

    // CombatStop should clear everything
    _creatureA->CombatStop(true);
    EXPECT_FALSE(_creatureA->GetCombatManager().HasCombat());
    EXPECT_TRUE(_creatureA->GetThreatMgr().IsThreatListEmpty());
}

TEST_F(CombatManagerIntegrationTest,
       CombatStop_BidirectionalCleanup)
{
    _creatureA->EngageWithTarget(_creatureB);
    EXPECT_TRUE(_creatureB->GetCombatManager().HasCombat());

    // A stops combat - B should also leave combat (no other refs)
    _creatureA->CombatStop(true);
    EXPECT_FALSE(_creatureA->GetCombatManager().HasCombat());
    EXPECT_FALSE(_creatureB->GetCombatManager().HasCombat());
}
...
TEST_F(CombatManagerIntegrationTest,
       CombatStop_MultipleTargets_ClearsAll)
{
    TestCreature* creatureC = new TestCreature();
    creatureC->SetupForCombatTest(_map, 3, 12347);
    creatureC->SetFaction(90002);
...
// ============================================================================
// SetImmuneToAll Tests
// ============================================================================

TEST_F(CombatManagerIntegrationTest, SetImmuneToAll_EndsAllCombat)
{
    TestCreature* creatureC = new TestCreature();
    creatureC->SetupForCombatTest(_map, 3, 12347);
    creatureC->SetFaction(90002);
    ...
    _creatureA->SetImmuneToAll(true, false);

    EXPECT_FALSE(_creatureA->TestGetCombatMgr().HasCombat());
    EXPECT_TRUE(_creatureA->IsImmuneToPC());
    EXPECT_TRUE(_creatureA->IsImmuneToNPC());

    _creatureA->SetImmuneToAll(false, false);

    creatureC->CleanupCombatState();
    delete creatureC;
}

TEST_F(CombatManagerIntegrationTest, SetImmuneToAll_KeepCombat_PreservesRefs)
{
    _creatureA->TestGetCombatMgr().SetInCombatWith(_creatureB);
    EXPECT_TRUE(_creatureA->TestGetCombatMgr().HasCombat());

    _creatureA->SetImmuneToAll(true, true);
...
TEST_F(CombatManagerIntegrationTest, SetImmuneToAll_RemoveRestoresFlags)
{
    _creatureA->SetImmuneToAll(true, false);
    EXPECT_TRUE(_creatureA->IsImmuneToPC());
    EXPECT_TRUE(_creatureA->IsImmuneToNPC());
    ...
    _creatureA->SetImmuneToAll(false, false);
    EXPECT_FALSE(_creatureA->IsImmuneToPC());
    EXPECT_FALSE(_creatureA->IsImmuneToNPC());
}

// ============================================================================
// SetImmuneToPC Tests (selective EndCombat by player-controlled flag)
// ============================================================================

TEST_F(CombatManagerIntegrationTest,
       SetImmuneToPC_EndsCombatWithPlayerControlled)
{
    // Mark B as player-controlled
    _creatureB->SetUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED);

    _creatureA->TestGetCombatMgr().SetInCombatWith(_creatureB);
    EXPECT_TRUE(_creatureA->TestGetCombatMgr().HasCombat());

    _creatureA->SetImmuneToPC(true, false);

    // Combat with player-controlled unit should end
    EXPECT_FALSE(_creatureA->TestGetCombatMgr().IsInCombatWith(_creatureB));
    EXPECT_FALSE(_creatureA->TestGetCombatMgr().HasCombat());

    _creatureB->RemoveUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED);
    _creatureA->SetImmuneToPC(false, false);
}

TEST_F(CombatManagerIntegrationTest,
       SetImmuneToPC_KeepsCombatWithNPCs)
{
    // B is NOT player-controlled (default for test creatures)
    _creatureA->TestGetCombatMgr().SetInCombatWith(_creatureB);
    EXPECT_TRUE(_creatureA->TestGetCombatMgr().HasCombat());

    _creatureA->SetImmuneToPC(true, false);

    // Combat with NPC should NOT end
    EXPECT_TRUE(_creatureA->TestGetCombatMgr().IsInCombatWith(_creatureB));
...
TEST_F(CombatManagerIntegrationTest,
       SetImmuneToPC_SelectiveEndCombat)
{
    TestCreature* creatureC = new TestCreature();
    creatureC->SetupForCombatTest(_map, 3, 12347);
    creatureC->SetFaction(90002);
    // C is player-controlled, B is not
    creatureC->SetUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED);
    ...
    // Only player-controlled ref should be ended
    EXPECT_TRUE(_creatureA->TestGetCombatMgr().IsInCombatWith(_creatureB));
    EXPECT_FALSE(_creatureA->TestGetCombatMgr().IsInCombatWith(creatureC));
...
TEST_F(CombatManagerIntegrationTest,
       SetImmuneToPC_KeepCombat_PreservesRefs)
{
    _creatureB->SetUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED);
    _creatureA->TestGetCombatMgr().SetInCombatWith(_creatureB);
    EXPECT_TRUE(_creatureA->TestGetCombatMgr().HasCombat());

    _creatureA->SetImmuneToPC(true, true);
...
// ============================================================================
// ValidateAttackersAndOwnTarget Tests
// ============================================================================

TEST_F(CombatManagerIntegrationTest,
       ValidateAttackersAndOwnTarget_StopsInvalidAttackers)
{
    // B attacks A (set up attack state directly)
    _creatureB->TestSetAttacking(_creatureA);
    EXPECT_EQ(_creatureA->getAttackers().size(), 1u);
    EXPECT_EQ(_creatureA->getAttackers().count(_creatureB), 1u);

    // Make A immune to NPC - B can no longer validly attack A
    _creatureA->SetUnitFlag(UNIT_FLAG_IMMUNE_TO_NPC);
    _creatureA->ValidateAttackersAndOwnTarget();

    // B should have stopped attacking
    EXPECT_EQ(_creatureB->GetVictim(), nullptr);
    EXPECT_EQ(_creatureA->getAttackers().size(), 0u);

    _creatureA->RemoveUnitFlag(UNIT_FLAG_IMMUNE_TO_NPC);
}

TEST_F(CombatManagerIntegrationTest,
       ValidateAttackersAndOwnTarget_StopsOwnInvalidTarget)
{
    // A attacks B
    _creatureA->TestSetAttacking(_creatureB);
    EXPECT_EQ(_creatureA->GetVictim(), _creatureB);

    // Make B immune to NPC - A can no longer validly attack B
    _creatureB->SetUnitFlag(UNIT_FLAG_IMMUNE_TO_NPC);
    _creatureA->ValidateAttackersAndOwnTarget();

    // A should have stopped attacking
    EXPECT_EQ(_creatureA->GetVictim(), nullptr);

    _creatureB->RemoveUnitFlag(UNIT_FLAG_IMMUNE_TO_NPC);
}

TEST_F(CombatManagerIntegrationTest,
       ValidateAttackersAndOwnTarget_KeepsValidAttackers)
{
    // B attacks A
    _creatureB->TestSetAttacking(_creatureA);
    EXPECT_EQ(_creatureA->getAttackers().size(), 1u);

    // No immune flags - attackers are valid
    _creatureA->ValidateAttackersAndOwnTarget();

    // B should still be attacking
    EXPECT_EQ(_creatureB->GetVictim(), _creatureA);
    EXPECT_EQ(_creatureA->getAttackers().size(), 1u);

    _creatureB->TestClearAttacking();
}

// ============================================================================
// SetImmuneToNPC with mixed player-controlled units
// ============================================================================

TEST_F(CombatManagerIntegrationTest,
       SetImmuneToNPC_KeepsCombatWithPlayerControlled)
{
    ...
    // Combat with player-controlled unit should NOT end
    EXPECT_TRUE(_creatureA->TestGetCombatMgr().IsInCombatWith(_creatureB));
...
TEST_F(CombatManagerIntegrationTest,
       SetImmuneToNPC_SelectiveEndCombat)
{
    TestCreature* creatureC = new TestCreature();
    creatureC->SetupForCombatTest(_map, 3, 12347);
    creatureC->SetFaction(90002);
    // B is player-controlled, C is not
    _creatureB->SetUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED);
    ...
    // Only NPC ref should be ended, player-controlled kept
    EXPECT_TRUE(_creatureA->TestGetCombatMgr().IsInCombatWith(_creatureB));
    EXPECT_FALSE(_creatureA->TestGetCombatMgr().IsInCombatWith(creatureC));
