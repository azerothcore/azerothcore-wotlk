...
enum NPCArtorius
{
  ARTORIUS_EVENT_ENCOUNTER_START            = 1,
  ARTORIUS_EVENT_REVEAL                     = 2,
  ARTORIUS_EVENT_RANGE_CHECK                = 3,
  ARTORIUS_EVENT_UNFAIR_FIGHT               = 4,
...
struct NPCStaveQAI
{
    ...
    void InitMembers(Creature* creature)
    {
        threatList = creature->getThreatManager().getThreatList();
        StorePlayerGUID();
    }
    ...
    bool IsFairFight(Creature* creature)
    {
        for (ThreatContainer::StorageType::const_iterator itr = threatList.begin(); itr != threatList.end(); ++itr)
        {
            Unit* unit = ObjectAccessor::GetUnit(*creature, (*itr)->getUnitGuid());
            bool allowedEntry = IsAllowedEntry(unit->GetEntry());

            if (!(*itr)->getThreat())
            {
                // if target threat is 0 its fair, this prevents despawn in the case when
                // there is a bystander since UpdateVictim adds nearby enemys to the threatlist
                continue;
            }
    ...
    bool ValidThreatlist(Creature* creature)
    {
        ...
        bool isFair = IsFairFight(creature);

        return isFair;
    }

    void SetHomePosition(Creature* creature)
    {
        Position homePosition;
        creature->GetPosition(&homePosition);

        if (homePosition.IsPositionValid())
        {
            creature->SetHomePosition(homePosition);
            creature->SetWalk(false); // maybe we dont need this
        }
    }
};
...
public:
    ...
    struct npc_artoriusAI : public ScriptedAI, public NPCStaveQAI
    {
        npc_artoriusAI(Creature *creature) : ScriptedAI(creature) { }
        ...
        void AttackStart(Unit* target) override
        {
            ...
            if (me->GetEntry() == ARTORIUS_EVIL_ENTRY)
            {
                ...
                events.ScheduleEvent(ARTORIUS_EVENT_RANGE_CHECK, 1000);
                events.ScheduleEvent(ARTORIUS_EVENT_UNFAIR_FIGHT, 1000);
            }
        ...
        void RevealForm()
        {
            if (encounterStarted && me->GetEntry() == ARTORIUS_NORMAL_ENTRY)
            {
                me->UpdateEntry(ARTORIUS_EVIL_ENTRY);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (eventId)
            {
                case ARTORIUS_EVENT_ENCOUNTER_START:
                    me->MonsterSay(ARTORIUS_SAY, LANG_UNIVERSAL, 0);
                    me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                    events.ScheduleEvent(ARTORIUS_EVENT_REVEAL, 8000);
                    break;
                case ARTORIUS_EVENT_REVEAL:
                    RevealForm();
                    break;
            }
            ...
            if (UpdateVictim())
            {
                // This should prevent hunters from staying in combat when feign death is used and their pet has 0 threat
                if (!pGUID.IsEmpty()
                  && me->GetVictim()->GetOwnerGUID() == pGUID
                  && ObjectAccessor::GetPlayer(*me, pGUID)->HasAura(5384)
                  && !IsAllowedEntry(me->GetVictim()->GetEntry())
                )
                {
                    pGUID.Clear();
                    EnterEvadeMode();
            ...
            switch (eventId)
            {
                case ARTORIUS_EVENT_RANGE_CHECK:
                    ...
                    break;
                case ARTORIUS_EVENT_UNFAIR_FIGHT:
                    if (!ValidThreatlist(me))
                    {
                        SetHomePosition(me);
        ...
        {
            if (action == ARTORIUS_EVENT_ENCOUNTER_START)
            {
                ...
                events.ScheduleEvent(ARTORIUS_EVENT_ENCOUNTER_START, 2000);
            }
        }
    };
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 /*action*/) override
    {
        ...
        creature->AI()->DoAction(ARTORIUS_EVENT_ENCOUNTER_START);
