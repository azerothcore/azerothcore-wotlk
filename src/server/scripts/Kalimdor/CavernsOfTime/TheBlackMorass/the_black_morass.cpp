...
struct npc_medivh_bm : public ScriptedAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        switch (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_OUTRO_2:
                ...
                break;
            case EVENT_OUTRO_3:
                SummonOrcs(-2046.158f, -3.0f, 37000, 30s, true);
                events.ScheduleEvent(EVENT_OUTRO_4, 2s);
                break;
            case EVENT_OUTRO_4:
                SummonOrcs(-2055.97f, -2.0f, 33000, 28s, false);
                events.ScheduleEvent(EVENT_OUTRO_5, 2s);
                break;
            case EVENT_OUTRO_5:
                SummonOrcs(-2064.0f, -1.5f, 29000, 26s, false);
                events.ScheduleEvent(EVENT_OUTRO_6, 2s);
                break;
            case EVENT_OUTRO_6:
                SummonOrcs(-2074.35f, -0.1f, 26000, 24s, false);
    ...
    void SummonOrcs(float x, float y, uint32 duration, Milliseconds homeTime, bool first)
    {
        for (uint8 i = 0; i < 6; ++i)
        {
