    ...
    struct npc_medivh_bmAI : public ScriptedAI
    {
        npc_medivh_bmAI(Creature* creature) : ScriptedAI(creature)
        {
            ...
            for (uint8 i = 0; i < 10; ++i)
                groundArray.push_back(G3D::Vector3(creature->GetPositionX() + 8.0f * std::cos(2.0f * M_PI * i / 10.0f), creature->GetPositionY() + 8.0f * std::sin(2.0f * M_PI * i / 10.0f), creature->GetPositionZ()));

            for (uint8 i = 0; i < 40; ++i)
                airArray.push_back(G3D::Vector3(creature->GetPositionX() + i * 0.25f * std::cos(2.0f * M_PI * i / 10.0f), creature->GetPositionY() + i * 0.25f * std::sin(2.0f * M_PI * i / 10.0f), creature->GetPositionZ() + i / 4.0f));
            for (uint8 i = 40; i < 80; ++i)
                airArray.push_back(G3D::Vector3(creature->GetPositionX() + 10.0f * std::cos(2.0f * M_PI * i / 10.0f), creature->GetPositionY() + 10.0f * std::sin(2.0f * M_PI * i / 10.0f), creature->GetPositionZ() + i / 4.0f));
        }
            ...
            if (Creature* summon = me->SummonCreature(entry, pos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 150000))
                if (instance)
                {
                    if (Unit* medivh = ObjectAccessor::GetUnit(*me, instance->GetGuidData(DATA_MEDIVH)))
                    {
                        ...
                        summon->SetHomePosition(medivh->GetPositionX() + 14.0f * std::cos(o), medivh->GetPositionY() + 14.0f * std::sin(o), medivh->GetPositionZ(), summon->GetAngle(medivh));
