...
public:
    ...
    struct npc_natrualist_biteAI : public ScriptedAI
    {
        ...
        {
            uint32 menuId = GetData(1) ? 7540 : 7520;
            player->PrepareGossipMenu(me, menuId, false);
        ...
        bool GossipSelect(Player* player, uint32 /*sender*/, uint32 /*action*/) override
        {
            ...
            if (GetData(1))
            {
                me->CastSpell(player, SPELL_MARK_OF_BITE, true);
            ...
            else
            {
                ...
                Creature* cr;
                if ((cr = me->SummonCreature(17957, -186, -790, 43.8f, 4.2f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000)))
                    cr->AI()->AttackStart(me);
                if ((cr = me->SummonCreature(17960, -188, -783, 43.8f, 4.2f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000)))
                    cr->AI()->AttackStart(player);
                if ((cr = me->SummonCreature(17957, -196, -783, 43.8f, 4.4f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000)))
                    cr->AI()->AttackStart(player);
                GameObject* cage = me->FindNearestGameObject(GO_CAGE_ENTRY, 20.0f);
                if (cage)
                    cage->SetGoState(GO_STATE_ACTIVE);
                me->SetHomePosition(-195.39f, -795.91f, 43.8f, 1.0f);
                Talk(1);
            }
            return true;
        }
