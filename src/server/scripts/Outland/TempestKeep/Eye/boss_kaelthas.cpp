    ...
    class spell_kaelthas_summon_nether_vapor_SpellScript : public SpellScript
    {
        ...
        void HandleScriptEffect(SpellEffIndex effIndex)
        {
            ...
            for (uint32 i = 0; i < 5; ++i)
                GetCaster()->SummonCreature(NPC_NETHER_VAPOR, GetCaster()->GetPositionX() + 6 * std::cos(i / 5.0f * 2 * M_PI), GetCaster()->GetPositionY() + 6 * sin(i / 5.0f * 2 * M_PI), GetCaster()->GetPositionZ() + 7.0f + i, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 30000);
        }
