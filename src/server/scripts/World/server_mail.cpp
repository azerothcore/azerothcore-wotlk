        ...
        for (auto const& servMail : sObjectMgr->GetAllServerMailStore())
        {
            ...
            mySess->GetQueryProcessor().AddCallback(CharacterDatabase.AsyncQuery(stmt)
                .WithPreparedCallback([mySess, servMail](PreparedQueryResult result)
                    {
                        if (!result)
                            sObjectMgr->SendServerMail(mySess->GetPlayer(), servMail.second.id, servMail.second.reqLevel, servMail.second.reqPlayTime, servMail.second.moneyA, servMail.second.moneyH, servMail.second.subject, servMail.second.body, servMail.second.active);
                    }));
        }
    }
};
