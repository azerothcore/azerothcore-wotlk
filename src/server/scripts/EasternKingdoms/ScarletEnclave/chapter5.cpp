            ...
            switch (eventId)
            {
                        ...
                        for (SummonList::const_iterator itr = summons.begin(); itr != summons.end(); ++itr)
                            if (Creature* summon = ObjectAccessor::GetCreature(*me, *itr))
                                if (summon->GetEntry() <= NPC_RIMBLAT_EARTHSHATTER && summon->GetEntry() != NPC_HIGHLORD_TIRION_FORDRING)
                                {
                                    ...
                                    summon->GetMotionMaster()->MovePoint(3, lk->GetPositionX() + 2.0f * std::cos(o), lk->GetPositionY() + 2.0f * sin(o), lk->GetPositionZ());
                ...
                case EVENT_OUTRO_SCENE_47:
                    ...
                    if (Creature* tirion = GetEntryFromSummons(NPC_HIGHLORD_TIRION_FORDRING))
                    {
                        ...
                        tirion->GetMotionMaster()->MovePoint(4, me->GetPositionX() + 2.0f * std::cos(o), me->GetPositionY() + 2.0f * sin(o), me->GetPositionZ(), false);
