        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case EVENT_SUMMON_ORBS:
                    ...
                    {
                        float x = me->GetPositionX() + 18.0f * std::cos((i * 2.0f - 1.0f) * M_PI / 3.0f);
                        ...
                        if (Creature* orb = me->SummonCreature(NPC_SHIELD_ORB, x, y, 40.0f, 0, TEMPSUMMON_CORPSE_DESPAWN))
                        {
                            ...
                            {
                                x = me->GetPositionX() + 18.0f * std::cos(((i * 2.0f - 1.0f) * M_PI / 3.0f) + (j / 20.0f * 2 * M_PI));
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_SCENE_02:
                    ...
                    break;
                case EVENT_SCENE_03:
                    me->SummonCreature(NPC_SHATTRATH_PORTAL_DUMMY, 1727.08f + std::cos(5.14f), 656.82f + std::sin(5.14f), 28.37f + 2.0f, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 10000);
                    me->SummonCreature(NPC_SHATTRATH_PORTAL_DUMMY, 1738.84f + std::cos(2.0f), 627.32f + std::sin(2.0f), 28.26f + 2.0f, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 10000);
                ...
                case EVENT_SCENE_05:
                    if (Creature* first = me->SummonCreature(NPC_SHATTERED_SUN_SOLDIER, 1729.48f, 640.49f, 28.06f, 3.49f))
                    {
                        ...
                        for (uint8 i = 0; i < 9; ++i)
                            if (Creature* follower = me->SummonCreature(NPC_SHATTERED_SUN_SOLDIER, 1729.48f + 5 * std::cos(i * 2.0f * M_PI / 9), 640.49f + 5 * std::sin(i * 2.0f * M_PI / 9), 28.06f, 3.49f))
