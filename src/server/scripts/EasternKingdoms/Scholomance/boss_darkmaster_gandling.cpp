    ...
    struct boss_darkmaster_gandlingAI : public BossAI
    {
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spellinfo) override
        {
            ...
            if (spellinfo && spellinfo->Id == SPELL_SHADOW_PORTAL && target)
            {
                ...
                {
                    me->GetThreatManager().AddThreat(victim, -1000000); // drop current player, add a ton to second. This should guarantee that we don't end up with both 1 and 2 in a cage...
                    if (Unit* newTarget = SelectTarget(SelectTargetMethod::MaxThreat, 1, 200.0f)) // search in whole room
                    {
                        me->GetThreatManager().AddThreat(newTarget, 1000000);
                    }
                }
            }
        }
