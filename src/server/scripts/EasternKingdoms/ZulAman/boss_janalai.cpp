...
struct boss_janalai : public BossAI {
        ...
        if (summon->GetEntry() == NPC_AMANI_HATCHLING) {
            if (summon->GetPositionY() > 1150)
                summon->GetMotionMaster()->MovePoint(
                    0, hatcherway[0][3].GetPositionX() + rand() % 4 - 2,
                    1150.0f + rand() % 4 - 2, hatcherway[0][3].GetPositionY());
            else
                summon->GetMotionMaster()->MovePoint(
                    0, hatcherway[1][3].GetPositionX() + rand() % 4 - 2,
                    1150.0f + rand() % 4 - 2, hatcherway[1][3].GetPositionY());
        }
    ...
    void JustEngagedWith(Unit *who) override {
        ...
        scheduler.Schedule(10s, GROUP_HATCHING, [this](TaskContext context) {
            ...
            if (_sideHatched[0] && !_sideHatched[1]) {
                me->SummonCreature(NPC_AMANI_HATCHER, hatcherway[1][0],
                                   TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000);
                me->SummonCreature(NPC_AMANI_HATCHER, hatcherway[1][0],
                                   TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000);
            } else if (!_sideHatched[0] && _sideHatched[1]) {
                me->SummonCreature(NPC_AMANI_HATCHER, hatcherway[0][0],
                                   TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000);
                me->SummonCreature(NPC_AMANI_HATCHER, hatcherway[0][0],
                                   TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000);
            } else {
        ...
        me->m_Events.AddEventAtOffset([&] { DoCastSelf(SPELL_ENRAGE, true); },
        ...
        for (uint8 i = 0; i < 4; ++i) {
            ...
            for (uint8 j = 0; j < wallNum; j++) {
                Creature *wall =
                    wallNum == 3
                        ? me->SummonCreature(
                              NPC_FIRE_BOMB, fireWallCoords[i].GetPositionX(),
    ...
    void SpawnBombs() {
        ...
        for (int i = 0; i < MAX_BOMB_COUNT; ++i) {
            ...
            DoSpawnCreature(NPC_FIRE_BOMB, dx, dy, 0, 0,
                            TEMPSUMMON_TIMED_DESPAWN, 15000);
        }
    }
    ...
    void ThrowBombs() {
        ...
        summons.DoForAllSummons(
            [this, &bombTimer, throwDuration](WorldObject *summon) {
                if (summon->GetEntry() == NPC_FIRE_BOMB) {
                    if (Creature *bomb = summon->ToCreature()) {
                        // Calculate timing to spread throws over 5 seconds
                        std::chrono::milliseconds throwTime =
                            bombTimer % throwDuration;
    ...
    void MovementInform(uint32, uint32) override {
        if (_waypoint == 5) {
            ...
            scheduler.Schedule(1500ms, [this](TaskContext context) {
                me->CastCustomSpell(SPELL_HATCH_EGG_ALL, SPELLVALUE_MAX_TARGETS,
                                    _repeatCount);
                ...
                if (me->FindNearestCreature(NPC_EGG, 100.0f))
                    ...
                    if (WorldObject *summoner = GetSummoner())
                        if (Creature *janalai = summoner->ToCreature())
                            janalai->AI()->SetData(DATA_ALL_EGGS_HATCHED,
                                                   _side);
...
class spell_summon_all_players_dummy : public SpellScript {
    ...
    void Register() override {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_summon_all_players_dummy::FilterTargets, EFFECT_0,
            TARGET_UNIT_SRC_AREA_ENEMY);
        OnEffectHitTarget +=
            SpellEffectFn(spell_summon_all_players_dummy::OnHit, EFFECT_0,
                          SPELL_EFFECT_DUMMY);
    }
};
