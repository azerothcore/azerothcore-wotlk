...
public:
    ...
    static bool HandleDebugSendOpcodeCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        while (!parsedStream.eof())
        {
            ...
            {
                LOG_ERROR("root", "Sending opcode that has unknown type '%s'", type.c_str());
                break;
            }
        }
    ...
    static bool HandleWPGPSCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        LOG_DEBUG("root", "(@PATH, XX, %.3f, %.3f, %.5f, 0,0, 0,100, 0),", player->GetPositionX(), player->GetPositionY(), player->GetPositionZ());
