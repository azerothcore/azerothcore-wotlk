...
public:
    ...
    static bool HandleGuildInfoCommand(ChatHandler* handler, char const* args)
    {
        ...
        {
            handler->PSendSysMessage(LANG_GUILD_INFO_GUILD_MASTER, guildMasterName, guild->GetLeaderGUID().GetCounter()); // Guild Master
        }

        handler->PSendSysMessage(LANG_GUILD_INFO_CREATION_DATE, Acore::Time::TimeToHumanReadable(Seconds(guild->GetCreatedDate()))); // Creation Date
