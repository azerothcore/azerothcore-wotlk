...
public:
    ...
    static bool HandleReloadCreatureTemplateCommand(ChatHandler* handler, const char* args)
    {
        ...
        for (Tokenizer::const_iterator itr = entries.begin(); itr != entries.end(); ++itr)
        {
            ...
            cInfo->attackpower              = fields[24].GetUInt32();
            cInfo->DamageModifier           = fields[25].GetFloat();
            cInfo->BaseAttackTime           = fields[26].GetUInt32();
            cInfo->RangeAttackTime          = fields[27].GetUInt32();
            ...
            cInfo->rangedattackpower        = fields[37].GetUInt16();
            cInfo->type                     = fields[38].GetUInt8();
            cInfo->type_flags               = fields[39].GetUInt32();
            cInfo->lootid                   = fields[40].GetUInt32();
            cInfo->pickpocketLootId         = fields[41].GetUInt32();
            cInfo->SkinLootId               = fields[42].GetUInt32();

            for (uint8 i = SPELL_SCHOOL_HOLY; i < MAX_SPELL_SCHOOL; ++i)
                cInfo->resistance[i] = fields[43 + i - 1].GetUInt16();

            cInfo->spells[0]                = fields[49].GetUInt32();
            cInfo->spells[1]                = fields[50].GetUInt32();
            cInfo->spells[2]                = fields[51].GetUInt32();
            cInfo->spells[3]                = fields[52].GetUInt32();
            cInfo->spells[4]                = fields[53].GetUInt32();
            cInfo->spells[5]                = fields[54].GetUInt32();
            cInfo->spells[6]                = fields[55].GetUInt32();
            cInfo->spells[7]                = fields[56].GetUInt32();
            cInfo->PetSpellDataId           = fields[57].GetUInt32();
            cInfo->VehicleId                = fields[58].GetUInt32();
            cInfo->mingold                  = fields[59].GetUInt32();
            cInfo->maxgold                  = fields[60].GetUInt32();
            cInfo->AIName                   = fields[61].GetString();
            cInfo->MovementType             = fields[62].GetUInt8();
            cInfo->InhabitType              = fields[63].GetUInt8();
            cInfo->HoverHeight              = fields[64].GetFloat();
            cInfo->ModHealth                = fields[65].GetFloat();
            cInfo->ModMana                  = fields[66].GetFloat();
            cInfo->ModArmor                 = fields[67].GetFloat();
            cInfo->RacialLeader             = fields[68].GetBool();
            cInfo->movementId               = fields[69].GetUInt32();
            cInfo->RegenHealth              = fields[70].GetBool();
            cInfo->MechanicImmuneMask       = fields[71].GetUInt32();
            cInfo->SpellSchoolImmuneMask    = fields[72].GetUInt32();
            cInfo->flags_extra              = fields[73].GetUInt32();
            cInfo->ScriptID                 = sObjectMgr->GetScriptId(fields[74].GetCString());
