...
public:
    ...
    static bool HandleMmapPathCommand(ChatHandler* handler, Optional<std::string> para)
    {
        ...
        handler->PSendSysMessage(Acore::StringFormatFmt("Result: {} - Length: {} - Type: {}", (result ? "true" : "false"), pointPath.size(), path.GetPathType()));
    ...
    static bool HandleMmapTestArea(ChatHandler* handler)
    {
        ...
        {
            handler->PSendSysMessage(Acore::StringFormatFmt("Found {} Creatures.", creatureList.size()));
