    ...
    switch (MenuId) {
        case 0: {
            if (player->GetSession()->GetSessionDbLocaleIndex() == LOCALE_ruRU) {
                ...
                ss << "Пополнить счёт: onegocore.online\n\n";
                ...
                ss << "Top up account onegocore.online\n\n";
                if (player->GetSession()->IsPremium())
...
void sServerMenu::GossipMenuExchangeHonor(Player* player)
{
    ...
    ClearGossipMenuFor(player);
    AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, ConverterHonorRang(player, 75000, 10000, 1), GOSSIP_SENDER_MAIN + 7, 10000);
    AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, ConverterHonorRang(player, 150000, 20000, 2), GOSSIP_SENDER_MAIN + 7, 20000);
    AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, ConverterHonorRang(player, 375000, 25000, 5), GOSSIP_SENDER_MAIN + 7, 25000);
    AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, ConverterHonorRang(player, 750000, 50000, 10), GOSSIP_SENDER_MAIN + 7, 50000);
    AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, ConverterHonorRang(player, 900000, 100000, 25), GOSSIP_SENDER_MAIN + 7, 100000);
...
uint32 sServerMenu::CalculHonorForExp(Player* player, uint32 honor, uint8 count) 
{
    ...
    return uint32(honor - (player->GetRankByExp() * 50 * count));
}
...
void sServerMenu::ConfirmExchangeHonorForExp(Player* player, uint32 honor, uint32 exp, uint32 count)
{
    ...
    uint8 new_count = count == 50 ? 25 : count == 25 ? 10 : count == 20 ? 5 : count == 10 ? 2 : 1;    
