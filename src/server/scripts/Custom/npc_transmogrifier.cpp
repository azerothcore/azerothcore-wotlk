        ...
        bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action) override
        {
            ...
            switch (sender)
            {
                ...
                default: // Transmogrify
                {
                    ...
                    TransmogTrinityStrings res = sTransmogrification->Transmogrify(player, uint64(action), sender);
    ...
    void OnLogout(Player* player) override
    {
        ...
        if (!player->transmogMap.empty())
        {
            ...
            for (std::vector<uint64>::const_iterator it = items.begin(); it != items.end(); ++it)
            {
                ...
                trans->PAppend("REPLACE INTO custom_transmogrification (GUID, FakeEntry, Owner) VALUES (%u, %u, %u)", (uint32)it2->first, it2->second, lowguid);
            }
        }
    ...
    void OnLogin(Player* player) override
    {
        ...
        if (result)
        {
            do
            {
                ...
                uint64 itemGUID = (uint64)field[0].GetUInt32();
...
public:
    ...
    {
        sLog->outDebug(LOG_FILTER_TRANSMOG, "Deleting non-existing transmogrification entries...");
