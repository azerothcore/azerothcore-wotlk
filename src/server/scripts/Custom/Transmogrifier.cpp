...
struct PendingItemsCheck : public BasicEvent
{
    ...
    bool Execute(uint64, uint32) override
    {
        ...
        if (!pending_transmogs.empty()) {
            ...
            {
                if (lastNotSavedAlert < GameTime::GetGameTimeMS().count() && player->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_GOSSIP) == nullptr)
                {
                    lastNotSavedAlert = GameTime::GetGameTimeMS().count() + 2000;
                    ChatHandler(player->GetSession()).SendNotification("You have not saved your transmogrifications!");
                }
                player->m_Events.AddEvent(this, player->m_Events.CalculateTime(500));
                return false;
            }
...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 menu_id, uint32 gossipListId) override
    {
        if (auto* gossipItem = player->PlayerTalkClass->GetGossipMenu().GetItem(gossipListId))
        {
            uint32 sender = player->PlayerTalkClass->GetGossipOptionSender(gossipListId);
            uint32 action = player->PlayerTalkClass->GetGossipOptionAction(gossipListId);
            return OnGossipSelect(player, creature, sender, action, gossipItem->BoxMoney);
        }
        return OnGossipHello(player, creature);
    }

    bool OnGossipSelectCode(Player* player, Creature* creature, uint32 /*menu_id*/, uint32 gossipListId, const char* code) override
    {
        uint32 sender = player->PlayerTalkClass->GetGossipOptionSender(gossipListId);
        uint32 action = player->PlayerTalkClass->GetGossipOptionAction(gossipListId);
        return GossipSelectCode(player, creature, sender, action, code);
    }
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action, uint32 boxMoney)
    {
            ...
            if (it != player->presetMap.end())
            {
                for (auto&& v : it->second.data)
                {
                    ...
                    case TRANSMOG_TYPE_ITEM:
                        ...
                        {
                            ChatHandler(player->GetSession()).SendNotification("{}", CanTransmogrifyResultMessage(res));
                        }
                        break;
            ...
            for (auto&& v : it->second.data)
            {
                ...
                case TRANSMOG_TYPE_ENCHANT:
                {
                    ...
                    if (auto* source = sSpellItemEnchantmentStore.LookupEntry(enchant)) {
                        auto it = Transmogrification::enchant_visual_to_name.find(source->aura_id);
    ...
    bool GossipSelectCode(Player* player, Creature* creature, uint32 sender, uint32 action, const char* code)
    {
        ...
        case SENDER_CODE_PRESET_SAVE:
        {
            ...
            {
                ChatHandler(player->GetSession()).SendNotification("{}", CanTransmogrifyResultMessage(TransmogResult_TooLongSetName));
                OnGossipSelect(player, creature, SENDER_PRESET_LIST, 0);
                return true;
            }
            ...
            {
                // no displays to save
                ChatHandler(player->GetSession()).SendNotification("{}", CanTransmogrifyResultMessage(TransmogResult_NoTransmogrifications));
                OnGossipSelect(player, creature, SENDER_PRESET_LIST, 0);
                return true;
            }
            ...
            if (presetID >= Transmogrification::instance().MaxSets)
            {
                ...
                ChatHandler(player->GetSession()).SendNotification("{}", CanTransmogrifyResultMessage(TransmogResult_AtMaxSets));
                OnGossipSelect(player, creature, SENDER_PRESET_LIST, 0);
                return true;
            }
        ...
        case SENDER_CODE_PRESET_RENAME:
        {
            ...
            if (player->presetMap.find(presetID) == player->presetMap.end())
            {
                ...
                ChatHandler(player->GetSession()).SendNotification("{}", CanTransmogrifyResultMessage(TransmogResult_NonexistantSet));
                OnGossipSelect(player, creature, SENDER_PRESET_LIST, 0);
                return true;
            }
    ...
    void ShowTransmogs(Player* player, Creature* creature, TransmogGossipSender basesender, AppearanceType transmogtype, uint32 slot_or_slotindex, uint32 page)
    {
        ...
        if (!item)
        {
            ChatHandler(player->GetSession()).SendNotification("No item equipped");
            OnGossipHello(player, creature);
            return;
        }
            ...
            static const auto try_add_enchant = [](std::set<uint32>& added_enchant_visuals, std::vector<Action>& actions, Player* player, const ItemTemplate* target, uint32 enchant) {
                if (!Transmogrification::instance().CannotTransmogrifyItemWithEnchant(player, target, enchant, true)) {
                    ...
                    if (source) {
                        auto it = Transmogrification::enchant_visual_to_name.find(source->aura_id);
                        if (it != Transmogrification::enchant_visual_to_name.end()) {
                            auto result = added_enchant_visuals.insert(source->aura_id);
                            if (result.second)
