...
class spell_pal_sacred_shield_base : public AuraScript
{
    ...
    {
        /*
         * Sacred Shield - Flash of Light heal over time proc
         * Should be covered in HandleProc, but method doesn't get called as expected.
         */

        // Check if we have a spell and if that spell is Flash of Light
        if (eventInfo.GetProcSpell() && eventInfo.GetProcSpell()->GetSpellInfo()->SpellFamilyFlags[0] & 0x40000000)
        {
            // Check if we've got Infusion of Light talented
            if (eventInfo.GetActor()->GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL, SPELLFAMILY_PALADIN, 3021, 0))
            {
                bool validSpell = sSpellMgr->CheckSpellValid(eventInfo.GetProcSpell()->GetSpellInfo(), 66922, false);
                // Do we have a caster & a target and is the spell we want to cast a valid spell?
                if (eventInfo.GetActor() && eventInfo.GetActionTarget() && validSpell)
                {
                    int32 basepoints = int32(float(eventInfo.GetHealInfo()->GetHeal()) / 12.0f);

                    // Item - Paladin T9 Holy 4P Bonus (Flash of Light)
                    if (AuraEffect const *aurEffect = eventInfo.GetActor()->GetAuraEffect(67191, EFFECT_0))
                        AddPct(basepoints, aurEffect->GetAmount());

                    // Apply Flash of Light heal over time proc on target
                    eventInfo.GetActor()->CastCustomSpell(eventInfo.GetActionTarget(), 66922, &basepoints, nullptr, nullptr, true, 0, nullptr, eventInfo.GetActor()->GetGUID());
                }
            }
        }
