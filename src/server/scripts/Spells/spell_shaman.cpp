...
class spell_sha_flametongue_weapon : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        if (bot && bot->IsNPCBot())
        {
            ...
            float basePoints = aurEff->GetSpellInfo()->Effects[aurEff->GetEffIndex()].CalcValue();
            ...
            bot->CastCustomSpell(SPELL_SHAMAN_FLAMETONGUE_ATTACK, SPELLVALUE_BASE_POINT0, amount, target, true, item, aurEff);
            return;
        }
        //end npcbot
