    ...
    class spell_warl_improved_demonic_tactics_AuraScript : public AuraScript
    {
        ...
        void CalculateAmount(AuraEffect const* aurEff, int32& amount, bool&  /*canBeRecalculated*/)
        {
            if (aurEff->GetEffIndex() == EFFECT_0)
                amount = CalculatePct<int32, float>(GetUnitOwner()->ToPlayer()->GetFloatValue(PLAYER_SPELL_CRIT_PERCENTAGE1 + static_cast<uint8>(SpellSchool::Frost)), GetSpellInfo()->Effects[EFFECT_0].CalcValue());
            else
    ...
    class spell_warl_life_tap_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            if (Unit* target = GetHitUnit())
            {
                ...
                int32 mana = int32(damage2Mana + (caster->GetInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_POS + static_cast<uint8>(SpellSchool::Shadow)) * 0.5f));
