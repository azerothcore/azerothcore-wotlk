            ...
            if (findTimer)
            {
                ...
                if (findTimer >= 1000)
                {
                    if (Creature* ghoul = me->FindNearestCreature(NPC_DECAYING_GHOUL, 30.0f, true))
                    {
                        ...
                        ghoul->GetMotionMaster()->MovePoint(1, me->GetPositionX() + 2 * std::cos(o), me->GetPositionY() + 2 * sin(o), me->GetPositionZ());
                        checkTimer = 1;
                        findTimer = 0;
                    }
                    else
    ...
    struct npc_overlord_drakuru_betrayalAI : public ScriptedAI
    {
        ...
        void JustSummoned(Creature* cr) override
        {
            ...
            else
            {
                ...
                cr->GetMotionMaster()->MovePoint(0, me->GetPositionX() + std::cos(o) * 6.0f, me->GetPositionY() + sin(o) * 6.0f, me->GetPositionZ());
            }
        }
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_RECRUIT_2:
                        me->SetWalk(true);
                        me->GetMotionMaster()->MovePoint(0, me->GetPositionX() + (std::cos(_heading) * 10), me->GetPositionY() + (sin(_heading) * 10), me->GetPositionZ());
