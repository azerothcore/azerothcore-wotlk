            ...
            switch (events.ExecuteEvent())
            {
                case EVENT_START:
                    ...
                    if (Creature* cr = me->FindNearestCreature(NPC_LANDGREN, 100.0f, false))
                    {
                        ...
                        me->GetMotionMaster()->MovePoint(1, cr->GetPositionX() + std::cos(o) * 3, cr->GetPositionY() + sin(o) * 3, cr->GetPositionZ());
                        ...
                        for (SummonList::iterator itr = summons.begin(); itr != summons.end(); ++itr, ++i)
                            if (Creature* summon = ObjectAccessor::GetCreature(*me, *itr))
                                if (summon->GetEntry() == NPC_DISGUISED_CRUSADER)
                                {
                                    ...
                                    summon->GetMotionMaster()->MovePoint(2, 6132.38f + 4 * std::cos(2 * M_PI * (i / 3.0)), 2760.76f + 4 * sin(2 * M_PI * (i / 3.0)), me->GetPositionZ());
                                }
                        break;
                    }
