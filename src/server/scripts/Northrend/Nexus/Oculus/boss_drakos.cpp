                ...
                case EVENT_SUMMON:
                    {
                        for( uint8 i = 0; i < 2; ++i )
                        {
                            ...
                            me->SummonCreature(NPC_UNSTABLE_SPHERE, me->GetPositionX() + 5.0f * std::cos(angle), me->GetPositionY() + 5.0f * std::sin(angle), me->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN, 18000);
                        }
...
public:
    ...
    struct npc_oculus_unstable_sphereAI : public ScriptedAI
    {
        ...
        void PickNewLocation()
        {
            ...
            float angle = rand_norm() * 2 * M_PI;
            me->GetMotionMaster()->MovePoint(1, 961.29f + dist * std::cos(angle), 1049.0f + dist * std::sin(angle), 360.0f);
        }
