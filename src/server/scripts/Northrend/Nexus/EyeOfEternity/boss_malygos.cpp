                    ...
                    case MI_POINT_INTRO_SIDE_3:
                        {
                            ...
                            float dist = 75.0f;
                            if (Creature* c = me->SummonCreature(NPC_PORTAL, me->GetPositionX() + std::cos(angle) * dist, me->GetPositionY() + sin(angle) * dist, me->GetPositionZ(), FourSidesPos[id].GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 13000))
            ...
            switch(events.ExecuteEvent())
            {
                ...
                case EVENT_START_PHASE_2_MOVE_TO_SIDE:
                    ...
                    for (int i = 0; i < MAX_NEXUS_LORDS; i++)
                    {
                        ...
                        if (Creature* disk = me->SummonCreature(NPC_HOVER_DISK, CenterPos.GetPositionX() + std::cos(angle) * dist, CenterPos.GetPositionY() + sin(angle) * dist, CenterPos.GetPositionZ() + 30.0f, 0.0f, TEMPSUMMON_MANUAL_DESPAWN, 0))
                ...
                case EVENT_RESUME_FLYING_CIRCLES_PH_2:
                    {
                        ...
                        me->GetMotionMaster()->MovePoint(MI_POINT_CIRCLE_OUTSIDE_PH_2, CenterPos.GetPositionX() + std::cos(newangle)*dist, CenterPos.GetPositionY() + sin(newangle)*dist, Phase2NorthPos.GetPositionZ());
                    }
                    break;
                case EVENT_MOVE_TO_SURGE_OF_POWER:
                    {
                        ...
                        me->GetMotionMaster()->MoveIdle();
                        me->StopMoving();
                        me->GetMotionMaster()->MovePoint(MI_POINT_SURGE_OF_POWER_CENTER, CenterPos.GetPositionX() + std::cos(angle) * 10.0f, CenterPos.GetPositionY() + sin(angle) * 10.0f, Phase2NorthPos.GetPositionZ());
                ...
                if (me->GetVehicleKit()) if (Unit* pass = me->GetVehicleKit()->GetPassenger(0)) if (Player* plr = pass->ToPlayer())
                        {
                            ...
                            plr->SendMonsterMove(me->GetPositionX() + dist * std::cos(arcangle), me->GetPositionY() + dist * sin(arcangle), me->GetPositionZ(), VORTEX_DEFAULT_DIFF * 2, SPLINEFLAG_FLYING);
                                ...
                                case NPC_NEXUS_LORD:
                                    {
                                        ...
                                        me->GetMotionMaster()->MovePoint(MI_POINT_NEXUS_LORD, CenterPos.GetPositionX() + std::cos(newangle) * 22.0f, CenterPos.GetPositionY() + sin(newangle) * 22.0f, newz);
                                    }
                                    break;
                                case NPC_SCION_OF_ETERNITY:
                                    {
                                        ...
                                        me->GetMotionMaster()->MovePoint(MI_POINT_SCION, CenterPos.GetPositionX() + std::cos(newangle) * 30.0f, CenterPos.GetPositionY() + sin(newangle) * 30.0f, newz);
                                    }
                                    break;
                            }
                    break;
