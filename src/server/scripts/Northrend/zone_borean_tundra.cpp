            ...
            if (uint32 eventId = _events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_ADD_ARCANE_CHAINS:
                        ...
                        {
                            summoner->CastSpell(summoner, SPELL_ARCANE_CHAINS_CHANNEL_II, TriggerCastFlags(TRIGGERED_IGNORE_AURA_INTERRUPT_FLAGS && TRIGGERED_IGNORE_CAST_ITEM && TRIGGERED_IGNORE_POWER_AND_REAGENT_COST && TRIGGERED_IGNORE_GCD));
