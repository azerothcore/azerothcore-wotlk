...
public:
    ...
    struct boss_yoggsaron_saraAI : public ScriptedAI
    {
        ...
        void SpawnTentacle(uint32 entry)
        {
            ...
            if (Creature* cr = me->SummonCreature(entry, me->GetPositionX() + dist * std::cos(o), me->GetPositionY() + dist * std::sin(o), 327.2 + Zplus, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000))
            {
        ...
        void SummonDeathOrbs()
        {
            for (uint8 i = 0; i < 4; ++i)
            {
                ...
                float o = rand_norm() * M_PI * 2;
                float Zplus = (dist - 38) / 6.5f;
                me->SummonCreature(NPC_DEATH_ORB, me->GetPositionX() + dist * std::cos(o), me->GetPositionY() + dist * std::sin(o), 327.2 + Zplus, 0, TEMPSUMMON_TIMED_DESPAWN, 20000);
            }
        }
        ...
        void InitWaypoint()
        {
            ...
            if (me->GetPositionX() > Middle.GetPositionX())
            {
                for (uint8 i = 0; i <= dist; ++i)
                {
                    ...
                    AddWaypoint(i, Middle.GetPositionX() + dist * std::cos(angle), Middle.GetPositionY() + dist * std::sin(angle), me->GetPositionZ(), 0);
                }
            }
            else
            {
...
public:
    ...
    struct boss_yoggsaronAI : public ScriptedAI
    {
        ...
        void SummonImmortalGuardian()
        {
            ...
            float o = rand_norm() * M_PI * 2;
            float Zplus = (dist - 38) / 6.5f;
            me->SummonCreature(NPC_IMMORTAL_GUARDIAN, me->GetPositionX() + dist * std::cos(o), me->GetPositionY() + dist * std::sin(o), 327.2 + Zplus, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
        }
