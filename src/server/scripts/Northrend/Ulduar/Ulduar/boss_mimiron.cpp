                ...
                case EVENT_SPAWN_FLAMES_INITIAL:
                    {
                        ...
                        for( uint8 i = 0; i < 3; ++i )
                            if( !pg.empty() )
                            {
                                ...
                                float z = 364.35f;
                                if (!player->IsWithinLOS(player->GetPositionX() + std::cos(angle) * 5.0f, player->GetPositionY() + std::sin(angle) * 5.0f, z))
                                {
                                    angle = player->GetAngle(2744.65f, 2569.46f);
                                }
                                me->CastSpell(player->GetPositionX() + std::cos(angle) * 5.0f, player->GetPositionY() + std::sin(angle) * 5.0f, z, SPELL_SUMMON_FLAMES_INITIAL, true);
                ...
                case EVENT_FINISH:
                    {
                        ...
                        float v_x = me->GetPositionX() + std::cos(angle) * 10.0f;
                        ...
                        for( int i = 0; i < (Phase / 2); ++i )
                        {
                            ...
                            if( Unit* r = vk->GetPassenger(5 + index) )
                                if (Player* temp = SelectTargetFromPlayerList(100.0f))
                                {
                                    ...
                                    exitPos.m_positionX += std::cos(me->GetOrientation()) * 2.35f;
            ...
            if( Phase == 3 && !immobilized )
                if( Unit* victim = me->GetVictim() )
                    if( me->GetExactDist2d(victim) > 25.0f )
                    {
                        ...
                        float x = victim->GetPositionX() + 15.0f * std::cos(angle);
                ...
                case EVENT_FLAMES_SPREAD:
                    {
                        ...
                        if( last )
                        {
                            ...
                            if (target && prevdist >= 4.0f) // no need to spread when player is standing in fire, check distance
                            {
                                ...
                                SpreadFlame(last->GetPositionX() + 7.0f * std::cos(angle), last->GetPositionY() + 7.0f * std::sin(angle));
                            }
                        }
                ...
                case EVENT_EMERGENCY_BOT_CHECK:
                    ...
                    if (Creature* flame = me->FindNearestCreature(NPC_FLAMES_SPREAD, 150.0f, true))
                    {
                        ...
                        if (dist <= 5.0f)
                            ...
                            me->GetMotionMaster()->MovePoint(1, me->GetPositionX() + (dist - 5.0f)*std::cos(me->GetOrientation()), me->GetPositionY() + (dist - 5.0f)*sin(me->GetOrientation()), 364.32f);
                    }
                    break;
