                ...
                case EVENT_SPAWN_FLAMES_INITIAL:
                    {
                        ...
                        for( uint8 i = 0; i < 3; ++i )
                            if( !pg.empty() )
                            {
                                ...
                                float z = 364.35f;
                                if (!player->IsWithinLOS(player->GetPositionX() + std::cos(angle) * 5.0f, player->GetPositionY() + sin(angle) * 5.0f, z))
                                {
                                    angle = player->GetAngle(2744.65f, 2569.46f);
                                }
                                me->CastSpell(player->GetPositionX() + std::cos(angle) * 5.0f, player->GetPositionY() + sin(angle) * 5.0f, z, SPELL_SUMMON_FLAMES_INITIAL, true);
                ...
                case EVENT_FLAMES_SPREAD:
                    {
                        ...
                        if( last )
                        {
                            ...
                            if (target && prevdist >= 4.0f) // no need to spread when player is standing in fire, check distance
                            {
                                ...
                                SpreadFlame(last->GetPositionX() + 7.0f * std::cos(angle), last->GetPositionY() + 7.0f * sin(angle));
                            }
                        }
