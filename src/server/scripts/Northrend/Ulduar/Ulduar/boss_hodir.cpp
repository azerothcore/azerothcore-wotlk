        ...
        void UpdateAI(uint32 diff) override
        {
            if (timer <= diff)
            {
                ...
                if (me->IsSummon())
                {
                    ...
                    {
                        if ((s->IsPlayer() && !s->HasAura(SPELL_FLASH_FREEZE_TRAPPED_PLAYER)) || (s->IsUnit() && !s->HasAura(SPELL_FLASH_FREEZE_TRAPPED_NPC)))
...
class spell_hodir_flash_freeze_aura : public AuraScript
{
    ...
    void HandleEffectPeriodic(AuraEffect const* aurEff)
    {
        if (aurEff->GetTotalTicks() > 0 && aurEff->GetTickNumber() == uint32(aurEff->GetTotalTicks()) - 1)
        {
            ...
            if (!target || !caster || !caster->IsUnit())
                return;
...
public:
    ...
    {
        return target && target->GetEntry() == NPC_HODIR && target->IsUnit() && target->ToCreature()->AI()->GetData(1);
    }
};
...
public:
    ...
    bool OnCheck(Player*  /*player*/, Unit* target, uint32 /*criteria_id*/) override
    {
        return target && target->GetEntry() == NPC_HODIR && target->IsUnit() && target->ToCreature()->AI()->GetData(2);
    }
};
...
public:
    ...
    bool OnCheck(Player*  /*player*/, Unit* target, uint32 /*criteria_id*/) override
    {
        return target && target->GetEntry() == NPC_HODIR && target->IsUnit() && target->ToCreature()->AI()->GetData(3);
    }
};
...
public:
    ...
    bool OnCheck(Player*  /*player*/, Unit* target, uint32 /*criteria_id*/) override
    {
        return target && target->GetEntry() == NPC_HODIR && target->IsUnit() && target->ToCreature()->AI()->GetData(4);
    }
};
