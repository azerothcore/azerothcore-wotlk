            ...
            case EVENT_MOVE_UNDERGROUND:
                {
                    ...
                    if( Creature* c = me->SummonCreature(NPC_WORLD_TRIGGER, *me, TEMPSUMMON_TIMED_DESPAWN, 6000) )
                    {
                        ...
                        c->GetMotionMaster()->MovePoint(0, Locs[LOC_CENTER].GetPositionX() + std::cos(angle)*dist, Locs[LOC_CENTER].GetPositionY() + std::sin(angle)*dist, me->GetPositionZ());
                    }
                    me->UpdatePosition(Locs[LOC_CENTER].GetPositionX() + std::cos(angle)*dist, Locs[LOC_CENTER].GetPositionY() + std::sin(angle)*dist, me->GetPositionZ(), me->GetOrientation(), true);
                ...
                case EVENT_JUMP_BACK:
                    {
                        ...
                        if( angle > 1.0f && angle < 2.0f ) // near main gate
                            dist = 46.0f;
                        destX = Locs[LOC_CENTER].GetPositionX() + std::cos(angle) * dist;
                        destY = Locs[LOC_CENTER].GetPositionY() + std::sin(angle) * dist;
                        destZ = Locs[LOC_CENTER].GetPositionZ() + 1.0f;
                        me->StopMoving();
                        me->GetMotionMaster()->MoveJump(Locs[LOC_CENTER].GetPositionX() + std::cos(jumpangle) * 35.0f, Locs[LOC_CENTER].GetPositionY() + std::sin(jumpangle) * 35.0f, Locs[LOC_CENTER].GetPositionZ() + 1.0f, 40.0f, 12.0f);
