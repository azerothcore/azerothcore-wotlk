    ...
    struct boss_anubarak_trialAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            for( uint8 i = 0; i < 10; ++i )
            {
                ...
                if( Creature* c = me->SummonCreature(NPC_SCARAB, AnubLocs[0].GetPositionX() + std::cos(angle) * dist, AnubLocs[0].GetPositionY() + std::sin(angle) * dist, AnubLocs[0].GetPositionZ(), 0.0f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000) )
                {
        ...
        void SpellHitTarget(Unit* target, const SpellInfo* spell) override
        {
            ...
            if( spell->Id == SPELL_SHADOW_STRIKE )
            {
                ...
                if( o >= M_PI )
                    ...
                    o += M_PI;
                me->NearTeleportTo(target->GetPositionX() + std::cos(o) * 5.0f, target->GetPositionY() + std::sin(o) * 5.0f, target->GetPositionZ() + 0.6f, target->GetOrientation());
                AttackStart(target);
