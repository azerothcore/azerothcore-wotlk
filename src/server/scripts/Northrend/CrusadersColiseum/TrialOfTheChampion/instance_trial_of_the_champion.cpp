                ...
                case EVENT_GRAND_GROUP_1_MOVE_MIDDLE:
                    {
                        ...
                        for( uint8 i = 0; i < 3; ++i )
                            if( Creature* c = instance->GetCreature(NPC_GrandChampionMinionsGUID[1][i]) )
                            {
                                ...
                                c->GetMotionMaster()->MovePoint(0, 748.309f + 3.0f * std::cos(angle), 619.448f + 3.0f * std::sin(angle), 411.3f);
                            }
                ...
                case EVENT_GRAND_CHAMPIONS_MOVE_MIDDLE:
                    {
                        for( uint8 i = 0; i < 3; ++i )
                            ...
                            {
                                float angle = rand_norm() * 2 * M_PI;
                                c->GetMotionMaster()->MovePoint(4, 748.309f + 3.0f * std::cos(angle), 619.448f + 3.0f * std::sin(angle), 411.3f);
                            }
