                    ...
                    if (pInstance)
                    {
                        ...
                        for (SummonList::iterator itr = summons.begin(); itr != summons.end(); ++itr)
                            if (Creature* c = pInstance->instance->GetCreature(*itr))
                            {
                                ...
                                c->GetMotionMaster()->MoveTakeoff(0, c->GetPositionX() + dist * std::cos(angle), c->GetPositionY() + dist * sin(angle), c->GetPositionZ() + 6.0f + (float)urand(0, 4), 1.5f + frand(0.0f, 1.5f));
                            }
                    }
                    ...
                    if (pInstance)
                    {
                        for (SummonList::iterator itr = summons.begin(); itr != summons.end(); ++itr)
                            if (Creature* c = pInstance->instance->GetCreature(*itr))
                            {
                                ...
                                c->SetHomePosition(me->GetPositionX() + dist * std::cos(angle), me->GetPositionY() + dist * sin(angle), me->GetPositionZ(), 0.0f);
                            }
                    }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    me->GetMotionMaster()->MoveTakeoff(10, me->GetPositionX() + 2.0f * std::cos(me->GetOrientation()), me->GetPositionY() + 2.0f * sin(me->GetOrientation()), me->GetPositionZ() + 30.0f, 7.0f);
                    break;
                case 6:
            ...
            else if (p == 2)
            {
                ...
                for (ObjectGuid const& guid : summons)
                    if (Creature* c = pInstance->instance->GetCreature(guid))
                    {
                        ...
                        c->GetMotionMaster()->MovePoint(0, TSSpawnPos.GetPositionX() + std::cos(ang)*dist, TSSpawnPos.GetPositionY() + sin(ang)*dist, 628.2f);
                    }
            }
            else if (p == 3)
            {
    ...
    struct npc_pos_leader_secondAI: public NullCreatureAI
    {
        ...
        void SpellHitTarget(Unit* target, const SpellInfo* spell) override
        {
            if ((spell->Id == SPELL_TELEPORT_JAINA || spell->Id == SPELL_TELEPORT_SYLVANAS) && target && target->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                target->ToPlayer()->NearTeleportTo(me->GetPositionX() + std::cos(angle)*dist, me->GetPositionY() + sin(angle)*dist, me->GetPositionZ(), me->GetOrientation());
            }
        }
