...
//     CastSpellExtraArgs(uint64 const& origCaster) : TriggerFlags(TRIGGERED_FULL_MASK), OriginalCaster(origCaster) {}
//     CastSpellExtraArgs(AuraEffect const* eff, uint64 const& origCaster) : TriggerFlags(TRIGGERED_FULL_MASK), TriggeringAura(eff), OriginalCaster(origCaster) {}
...
//     CastSpellExtraArgs& SetOriginalCaster(uint64 const& guid) { OriginalCaster = guid; return *this; }
...
//                         guardList.sort(acore::ObjectDistanceOrderPred(me));
//                         uint32 x = 1;
...
//                         _events.ScheduleEvent(EVENT_INTRO_ALLIANCE_4, 29500
//                         , 0, PHASE_INTRO_A);
//                         _instance->HandleGameObject(_instance->GetData64(GO_SAURFANG_S_DOOR), true);
//                         if (Creature* deathbringer = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_DEATHBRINGER_SAURFANG)))
