...
public:
    ...
    struct npc_valkyr_shadowguardAI : public NullCreatureAI
    {
        ...
        void GoSiphon()
        {
            ...
            _destPoint.Relocate(CenterPosition.GetPositionX() + dist * std::cos(angle), CenterPosition.GetPositionY() + dist * sin(angle), 855.0f + frand(0.0f, 4.0f), 0.0f);
...
public:
    ...
    class spell_the_lich_king_teleport_to_frostmourne_hc_SpellScript : public SpellScript
    {
        ...
        void ModDest(SpellEffIndex  /*effIndex*/)
        {
            ...
            Position const offset = {dist * std::cos(angle), dist * sin(angle), 0.0f, 0.0f};
            WorldLocation* dest = const_cast<WorldLocation*>(GetExplTargetDest());
            dest->RelocateOffset(offset);
            GetHitDest()->RelocateOffset(offset);
        }
                ...
                case EVENT_MOVE_TO_LICH_KING:
                    if (Creature* lichKing = ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_THE_LICH_KING)))
                        if (me->GetExactDist(lichKing) > 8.0f)
                        {
                            ...
                            Movement::PointsArray path;
                            path.push_back(G3D::Vector3(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ()));
                            path.push_back(G3D::Vector3(lichKing->GetPositionX() + dist * std::cos(angle), lichKing->GetPositionY() + dist * sin(angle), 843.0f));
                            me->GetMotionMaster()->MoveSplinePath(&path);
                        }
                    break;
