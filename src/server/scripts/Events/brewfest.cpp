            ...
            case EVENT_SPAWN_MOLE_MACHINE:
                {
                    if (me->GetMapId() == 1) // Kalimdor
                    {
                        ...
                        float x = 1201.8f + rand * std::cos(angle);
                    ...
                    else if (me->GetMapId() == 0) // EK
                    {
                        ...
                        float angle = rand_norm() * 2 * M_PI;
                        float x = -5157.1f + rand * std::cos(angle);
            ...
            case EVENT_BARTENDER_SAY:
                {
                    ...
                    if (thrown == 3)
                    {
                        ...
                        sayer->SummonCreature(NPC_SUPER_BREW_TRIGGER, sayer->GetPositionX() + 15 * std::cos(sayer->GetOrientation()), sayer->GetPositionY() + 15 * std::sin(sayer->GetOrientation()), sayer->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 30000);
                    }
                    else
                    {
                        if (urand(0, 1))
