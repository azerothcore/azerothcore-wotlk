            ...
            case EVENT_BARTENDER_SAY:
                {
                    ...
                    if (thrown == 3)
                    {
                        ...
                        sayer->SummonCreature(NPC_SUPER_BREW_TRIGGER, sayer->GetPositionX() + 15 * std::cos(sayer->GetOrientation()), sayer->GetPositionY() + 15 * sin(sayer->GetOrientation()), sayer->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 30000);
                    }
                    else
                    {
                        if (urand(0, 1))
