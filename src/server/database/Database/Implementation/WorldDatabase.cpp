...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_INS_SPELL_GO_FACTION,     "INSERT INTO spell_gameobject_faction (spell_id, team_id, comment) VALUES (?, ?, ?)", CONNECTION_SYNCH);
    PrepareStatement(WORLD_UPD_SPELL_GO_FACTION,     "UPDATE spell_gameobject_faction SET team_id = ?, comment = ? WHERE spell_id = ?", CONNECTION_SYNCH);
    PrepareStatement(WORLD_DEL_SPELL_GO_FACTION,     "DELETE FROM spell_gameobject_faction WHERE spell_id = ?", CONNECTION_SYNCH);
