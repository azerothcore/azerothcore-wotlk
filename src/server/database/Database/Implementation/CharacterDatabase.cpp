...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_CHARACTER, "SELECT guid, account, name, race, class, gender, level, xp, money, skin, face, hairStyle, hairColor, facialStyle, bankSlots, restState, playerFlags, "
                     ...
                     "knownTitles, actionBars, grantableLevels, innTriggerId, extraBonusTalentCount, UNIX_TIMESTAMP(creation_date), securitygroup FROM characters WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_CHARACTER, "UPDATE characters SET name=?,race=?,class=?,gender=?,level=?,xp=?,money=?,skin=?,face=?,hairStyle=?,hairColor=?,facialStyle=?,bankSlots=?,restState=?,playerFlags=?,"
                     ...
                     "equipmentCache=?,ammoId=?,knownTitles=?,actionBars=?,grantableLevels=?,innTriggerId=?,extraBonusTalentCount=?,online=?,securitygroup=? WHERE guid=?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_BUG_REPORT,
                     "INSERT INTO Bugs (Category, Title, Description) VALUES (?,?,?)",
                     CONNECTION_ASYNC);
