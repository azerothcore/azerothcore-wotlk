...
namespace Acore
{
    ...
    struct AIRelocationNotifier
    {
        Unit& i_unit;
        bool isCreature;
        explicit AIRelocationNotifier(Unit& unit) : i_unit(unit), isCreature(unit.IsUnit())  {}
        template<class T> void Visit(GridRefMgr<T>&) {}
        void Visit(CreatureMapType&);
    };
    ...
    public:
        ...
        bool operator()(Unit* u)
        {
            if (u->IsAlive() && !u->IsCritter() && i_obj->IsWithinDistInMap(u, i_range) && !i_funit->IsFriendlyTo(u) &&
                    (!i_funit->IsUnit() || !i_funit->ToCreature()->IsAvoidingAOE())) // pussywizard
                return true;
            else
                return false;
        }
    ...
    public:
        ...
        bool operator()(Unit* u)
        {
            ...
            if (u->IsUnit() && (u->ToCreature()->IsTotem() || u->ToCreature()->IsTrigger() || u->ToCreature()->IsAvoidingAOE())) // pussywizard: added IsAvoidingAOE()
                return false;
    ...
    public:
        ...
        bool operator()(Unit* u)
        {
            ...
            if (u->IsUnit() && u->ToCreature()->IsTotem())
            {
                return false;
            }
