...
{
public:                                                 // Constructors
    explicit MailReceiver(WOWGUID::LowType receiver_lowguid) : m_receiver(nullptr), m_receiver_lowguid(receiver_lowguid) {}
    MailReceiver(Player* receiver);
    MailReceiver(Player* receiver, WOWGUID::LowType receiver_lowguid);
public:                                                 // Accessors
    [[nodiscard]] Player* GetPlayer() const { return m_receiver; }
    [[nodiscard]] WOWGUID::LowType  GetPlayerGUIDLow() const { return m_receiver_lowguid; }
private:
    Player* m_receiver;
    WOWGUID::LowType  m_receiver_lowguid;
};

class MailDraft
{
    typedef std::map<WOWGUID, Item*> MailItemMap;
...
public:                                                 // finishers
    void SendReturnToSender(uint32 sender_acc, WOWGUID::LowType sender_guid, WOWGUID::LowType receiver_guid, CharacterDatabaseTransaction trans);
...
struct Mail
{
    ...
    void AddItem(WOWGUID::LowType itemGuidLow, uint32 item_template)
    {
        MailItemInfo mii;
    ...
    bool RemoveItem(WOWGUID::LowType item_guid)
    {
