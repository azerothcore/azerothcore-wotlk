...
void Warden::ApplyPenalty(uint16 checkId, std::string const& reason)
{
    ...
    if (checkId)
    {
        if (Player const* plr = _session->GetPlayer())
        {
            ...
            reportMsg = acore::StringFormat(reportFormat, plr->GetName().c_str(), plr->GetGUIDLow(), _session->GetAccountId(), checkId, checkData && (checkData->Comment.empty()) ? checkData->Comment.c_str() : "Unknown", GetWardenActionStr(action).c_str());
        }
        else
        {
