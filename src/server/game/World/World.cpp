...
World::World()
{
    ...
    _guidAlert = false;
    _warnDiff = 0;
    _warnShutdownTime = GameTime::GetGameTime().count();
}
...
void World::LoadConfigSettings(bool reload)
{
    ...
    // Respawn Settings
    _int_configs[CONFIG_RESPAWN_MINCHECKINTERVALMS] = sConfigMgr->GetOption<int32>("Respawn.MinCheckIntervalMS", 5000);
    _int_configs[CONFIG_RESPAWN_DYNAMICMODE] = sConfigMgr->GetOption<int32>("Respawn.DynamicMode", 0);
    if (_int_configs[CONFIG_RESPAWN_DYNAMICMODE] > 1)
    {
        LOG_ERROR("server.loading", "Invalid value for Respawn.DynamicMode (%u). Set to 0.", _int_configs[CONFIG_RESPAWN_DYNAMICMODE]);
        _int_configs[CONFIG_RESPAWN_DYNAMICMODE] = 0;
    }
    _bool_configs[CONFIG_RESPAWN_DYNAMIC_ESCORTNPC] = sConfigMgr->GetOption<bool>("Respawn.DynamicEscortNPC", true);
    _int_configs[CONFIG_RESPAWN_GUIDWARNLEVEL] = sConfigMgr->GetOption<int32>("Respawn.GuidWarnLevel", 12000000);
    if (_int_configs[CONFIG_RESPAWN_GUIDWARNLEVEL] > 16777215)
    {
        LOG_ERROR("server.loading", "Respawn.GuidWarnLevel (%u) cannot be greater than maximum GUID (16777215). Set to 12000000.", _int_configs[CONFIG_RESPAWN_GUIDWARNLEVEL]);
        _int_configs[CONFIG_RESPAWN_GUIDWARNLEVEL] = 12000000;
    }
    _int_configs[CONFIG_RESPAWN_GUIDALERTLEVEL] = sConfigMgr->GetOption<int32>("Respawn.GuidAlertLevel", 16000000);
    if (_int_configs[CONFIG_RESPAWN_GUIDALERTLEVEL] > 16777215)
    {
        LOG_ERROR("server.loading", "Respawn.GuidWarnLevel (%u) cannot be greater than maximum GUID (16777215). Set to 16000000.", _int_configs[CONFIG_RESPAWN_GUIDALERTLEVEL]);
        _int_configs[CONFIG_RESPAWN_GUIDALERTLEVEL] = 16000000;
    }
    _int_configs[CONFIG_RESPAWN_RESTARTQUIETTIME] = sConfigMgr->GetOption<int32>("Respawn.RestartQuietTime", 3);
    if (_int_configs[CONFIG_RESPAWN_RESTARTQUIETTIME] > 23)
    {
        LOG_ERROR("server.loading", "Respawn.RestartQuietTime (%u) must be an hour, between 0 and 23. Set to 3.", _int_configs[CONFIG_RESPAWN_RESTARTQUIETTIME]);
        _int_configs[CONFIG_RESPAWN_RESTARTQUIETTIME] = 3;
    }
    _float_configs[CONFIG_RESPAWN_DYNAMICRATE_CREATURE] = sConfigMgr->GetOption<float>("Respawn.DynamicRateCreature", 10.0f);
    if (_float_configs[CONFIG_RESPAWN_DYNAMICRATE_CREATURE] < 0.0f)
    {
        LOG_ERROR("server.loading", "Respawn.DynamicRateCreature (%f) must be positive. Set to 10.", _float_configs[CONFIG_RESPAWN_DYNAMICRATE_CREATURE]);
        _float_configs[CONFIG_RESPAWN_DYNAMICRATE_CREATURE] = 10.0f;
    }
    _int_configs[CONFIG_RESPAWN_DYNAMICMINIMUM_CREATURE] = sConfigMgr->GetOption<int32>("Respawn.DynamicMinimumCreature", 10);
    _float_configs[CONFIG_RESPAWN_DYNAMICRATE_GAMEOBJECT] = sConfigMgr->GetOption<float>("Respawn.DynamicRateGameObject", 10.0f);
    if (_float_configs[CONFIG_RESPAWN_DYNAMICRATE_GAMEOBJECT] < 0.0f)
    {
        LOG_ERROR("server.loading", "Respawn.DynamicRateGameObject (%f) must be positive. Set to 10.", _float_configs[CONFIG_RESPAWN_DYNAMICRATE_GAMEOBJECT]);
        _float_configs[CONFIG_RESPAWN_DYNAMICRATE_GAMEOBJECT] = 10.0f;
    }
    _int_configs[CONFIG_RESPAWN_DYNAMICMINIMUM_GAMEOBJECT] = sConfigMgr->GetOption<int32>("Respawn.DynamicMinimumGameObject", 10);
    _guidWarningMsg = sConfigMgr->GetOption<std::string>("Respawn.WarningMessage", "There will be an unscheduled server restart at 03:00. The server will be available again shortly after.");
    _alertRestartReason = sConfigMgr->GetOption<std::string>("Respawn.AlertRestartReason", "Urgent Maintenance");
    _int_configs[CONFIG_RESPAWN_GUIDWARNING_FREQUENCY] = sConfigMgr->GetOption<int32>("Respawn.WarningFrequency", 1800);
...
void World::Update(uint32 diff)
{
...

    {
        ...
        {
            _warnDiff += diff;
            if (GameTime::GetGameTime().count() >= _warnShutdownTime)
