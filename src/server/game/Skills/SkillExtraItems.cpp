...
void LoadSkillPerfectItemTable()
{
    ...
    if (!result)
    {
        LOG_ERROR("root", ">> Loaded 0 spell perfection definitions. DB table `skill_perfect_item_template` is empty.");
        LOG_INFO("root", "\n");
        return;
    }
    ...
    do /* fetch data and run sanity checks */
    {
        ...
        {
            LOG_ERROR("root", "Skill perfection data for spell %u has non-existent spell id in `skill_perfect_item_template`!", spellId);
            continue;
        }
        ...
        {
            LOG_ERROR("root", "Skill perfection data for spell %u has non-existent required specialization spell id %u in `skill_perfect_item_template`!", spellId, requiredSpecialization);
            continue;
        }
        ...
        {
            LOG_ERROR("root", "Skill perfection data for spell %u has impossibly low proc chance in `skill_perfect_item_template`!", spellId);
            continue;
        }
        ...
        {
            LOG_ERROR("root", "Skill perfection data for spell %u references non-existent perfect item id %u in `skill_perfect_item_template`!", spellId, perfectItemType);
            continue;
        }
    ...
    LOG_INFO("root", ">> Loaded %u spell perfection definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("root", "\n");
}
...
void LoadSkillExtraItemTable()
{
    ...
    if (!result)
    {
        LOG_ERROR("root", ">> Loaded 0 spell specialization definitions. DB table `skill_extra_item_template` is empty.");
        LOG_INFO("root", "\n");
        return;
    }
    ...
    do
    {
        ...
        if (!sSpellMgr->GetSpellInfo(spellId))
        {
            LOG_ERROR("root", "Skill specialization %u has non-existent spell id in `skill_extra_item_template`!", spellId);
            continue;
        }
        ...
        if (!sSpellMgr->GetSpellInfo(requiredSpecialization))
        {
            LOG_ERROR("root", "Skill specialization %u have not existed required specialization spell id %u in `skill_extra_item_template`!", spellId, requiredSpecialization);
            continue;
        }
        ...
        {
            LOG_ERROR("root", "Skill specialization %u has too low additional create chance in `skill_extra_item_template`!", spellId);
            continue;
        }
        ...
        if (!newMaxOrEntry)
        {
            LOG_ERROR("root", "Skill specialization %u has 0 max number of extra items in `skill_extra_item_template`!", spellId);
            continue;
        }
    ...
    LOG_INFO("root", ">> Loaded %u spell specialization definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("root", "\n");
}
