...
void WorldSession::HandleLearnPreviewTalents(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("server", "CMSG_LEARN_PREVIEW_TALENTS");
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "MSG_TALENT_WIPE_CONFIRM");
#endif
    uint64 guid;
    recvData >> guid;
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "WORLD: HandleTalentWipeConfirmOpcode - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(guid)));
#endif
        return;
    }
