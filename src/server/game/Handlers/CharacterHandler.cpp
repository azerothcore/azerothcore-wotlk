    ...
    switch (_charCreateCallback.GetStage())
    {
        ...
        case 3:
        {
            ...
            if (createInfo->Data.rpos() < createInfo->Data.wpos())
            {
                ...
                createInfo->Data >> unk;
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
                LOG_DEBUG("server", "Character creation %s (account %u) has unhandled tail data: [%u]", createInfo->Name.c_str(), GetAccountId(), unk);
#endif
            }
    ...
    if (WorldSession* sess = sWorld->FindOfflineSession(GetAccountId()))
    {
        ...
        else
        {
            ...
            while (sess->GetPlayer() && (sess->GetPlayer()->IsBeingTeleportedFar() || (sess->GetPlayer()->IsInWorld() && sess->GetPlayer()->IsBeingTeleportedNear())))
            {
                ...
                {
                    LOG_INFO("server", "HandlePlayerLoginOpcode A");
                    break;
                }
                while (sess->GetPlayer() && sess->GetPlayer()->IsBeingTeleportedFar())
                {
                    ...
                    {
                        LOG_INFO("server", "HandlePlayerLoginOpcode B");
                        break;
                    }
                ...
                while (sess->GetPlayer() && sess->GetPlayer()->IsInWorld() && sess->GetPlayer()->IsBeingTeleportedNear())
                {
                    ...
                    {
                        LOG_INFO("server", "HandlePlayerLoginOpcode C");
                        break;
                    }
...
void WorldSession::HandlePlayerLoginFromDB(LoginQueryHolder* holder)
{
    ...
    //LOG_DEBUG("server", "Player %s added to Map.", pCurrChar->GetName().c_str());
    ...
    std::string IP_str = GetRemoteAddress();
    LOG_DEBUG("server", "Account: %d (IP: %s) Login Character:[%s] (GUID: %u) Level: %d",
...
void WorldSession::HandleChangePlayerNameOpcodeCallBack(PreparedQueryResult result, std::string const& newName)
{
    ...
    LOG_DEBUG("server", "Account: %d (IP: %s), Character [%s] (guid: %u) Changed name to: %s", GetAccountId(), GetRemoteAddress().c_str(), oldName.c_str(), guidLow, newName.c_str());
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "CMSG_ALTER_APPEARANCE");
#endif
...
void WorldSession::HandleRemoveGlyph(WorldPacket& recvData)
{
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "Client sent wrong glyph slot number in opcode CMSG_REMOVE_GLYPH %u", slot);
#endif
        return;
    }
...
void WorldSession::HandleCharCustomize(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("server", "Account: %d (IP: %s), Character [%s] (guid: %u) Customized to: %s", GetAccountId(), GetRemoteAddress().c_str(), playerData->name.c_str(), GUID_LOPART(guid), newName.c_str());
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "CMSG_EQUIPMENT_SET_SAVE");
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "CMSG_EQUIPMENT_SET_DELETE");
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "CMSG_EQUIPMENT_SET_USE");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "Item " UI64FMTD ": srcbag %u, srcslot %u", itemGuid, srcbag, srcslot);
#endif
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("server", "Account: %d (IP: %s), Character [%s] (guid: %u) Changed Race/Faction to: %s", GetAccountId(), GetRemoteAddress().c_str(), playerData->name.c_str(), lowGuid, newname.c_str());
    ...
    std::string IP_str = GetRemoteAddress();
    //LOG_DEBUG("server", "%s (IP: %s) changed race from %u to %u", GetPlayerInfo().c_str(), IP_str.c_str(), oldRace, race);
