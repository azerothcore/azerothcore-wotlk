...
void WorldSession::HandleCharEnum(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            LOG_DEBUG("root", "Loading char guid %u from account %u.", guidlow, GetAccountId());
#endif
...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    if (!classEntry)
    {
        ...
        SendPacket(&data);
        LOG_ERROR("root", "Class (%u) not found in DBC while creating new char for account (ID: %u): wrong DBC files or cheater?", class_, GetAccountId());
        return;
    }
    ...
    if (!raceEntry)
    {
        data << (uint8)CHAR_CREATE_FAILED;
        SendPacket(&data);
        LOG_ERROR("root", "Race (%u) not found in DBC while creating new char for account (ID: %u): wrong DBC files or cheater?", race_, GetAccountId());
        return;
    }
    ...
    if (raceEntry->expansion > Expansion())
    {
        ...
        LOG_ERROR("root", "Expansion %u account:[%d] tried to Create character with expansion %u race (%u)", Expansion(), GetAccountId(), raceEntry->expansion, race_);
        SendPacket(&data);
        return;
    }
    ...
    if (classEntry->expansion > Expansion())
    {
        ...
        LOG_ERROR("root", "Expansion %u account:[%d] tried to Create character with expansion %u class (%u)", Expansion(), GetAccountId(), classEntry->expansion, class_);
        SendPacket(&data);
        return;
    }
    ...
    if (!normalizePlayerName(name))
    {
        ...
        SendPacket(&data);
        LOG_ERROR("root", "Account:[%d] but tried to Create character with empty [name] ", GetAccountId());
        return;
    }
    ...
    switch (_charCreateCallback.GetStage())
    {
        ...
        case 3:
        {
            ...
            if (createInfo->Data.rpos() < createInfo->Data.wpos())
            {
                ...
                createInfo->Data >> unk;
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
                LOG_DEBUG("root", "Character creation %s (account %u) has unhandled tail data: [%u]", createInfo->Name.c_str(), GetAccountId(), unk);
#endif
            }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            LOG_DEBUG("root", "Account: %d (IP: %s) Create Character:[%s] (GUID: %u)", GetAccountId(), IP_str.c_str(), createInfo->Name.c_str(), newChar.GetGUIDLow());
#endif
            LOG_DEBUG("root", "Account: %d (IP: %s) Create Character:[%s] (GUID: %u)", GetAccountId(), IP_str.c_str(), createInfo->Name.c_str(), newChar.GetGUIDLow());
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket& recvData)
{
    ...
    std::string IP_str = GetRemoteAddress();
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "Account: %d (IP: %s) Delete Character:[%s] (GUID: %u)", GetAccountId(), IP_str.c_str(), name.c_str(), GUID_LOPART(guid));
#endif
    LOG_DEBUG("root", "Account: %d (IP: %s) Delete Character:[%s] (GUID: %u)", GetAccountId(), IP_str.c_str(), name.c_str(), GUID_LOPART(guid));
...
void WorldSession::HandlePlayerLoginOpcode(WorldPacket & recvData)
{
    ...
    {
        LOG_ERROR("root", "Player tries to login again, AccountId = %d", GetAccountId());
    ...
    {
        LOG_ERROR("root", "Account (%u) can't login with that character (%u).", GetAccountId(), GUID_LOPART(playerGuid));
    ...
    if (WorldSession* sess = sWorld->FindOfflineSession(GetAccountId()))
    {
        ...
        else
        {
            ...
            while (sess->GetPlayer() && (sess->GetPlayer()->IsBeingTeleportedFar() || (sess->GetPlayer()->IsInWorld() && sess->GetPlayer()->IsBeingTeleportedNear())))
            {
                ...
                {
                    LOG_INFO("root", "HandlePlayerLoginOpcode A");
                    break;
                }
                while (sess->GetPlayer() && sess->GetPlayer()->IsBeingTeleportedFar())
                {
                    ...
                    {
                        LOG_INFO("root", "HandlePlayerLoginOpcode B");
                        break;
                    }
                ...
                while (sess->GetPlayer() && sess->GetPlayer()->IsInWorld() && sess->GetPlayer()->IsBeingTeleportedNear())
                {
                    ...
                    {
                        LOG_INFO("root", "HandlePlayerLoginOpcode C");
                        break;
                    }
...
void WorldSession::HandlePlayerLoginFromDB(LoginQueryHolder* holder)
{
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "WORLD: Sent server info");
#endif
    }
    ...
    if (uint32 guildId = Player::GetGuildIdFromStorage(pCurrChar->GetGUIDLow()))
    {
        ...
        {
            LOG_ERROR("root", "Player %s (GUID: %u) marked as member of not existing guild (id: %u), removing guild membership for player.", pCurrChar->GetName().c_str(), pCurrChar->GetGUIDLow(), guildId);
    ...
    //LOG_DEBUG("root", "Player %s added to Map.", pCurrChar->GetName().c_str());
    ...
    std::string IP_str = GetRemoteAddress();
    LOG_DEBUG("root", "Account: %d (IP: %s) Login Character:[%s] (GUID: %u) Level: %d",
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "WORLD: Received CMSG_SET_FACTION_ATWAR");
#endif
...
{
    LOG_ERROR("root", "WORLD SESSION: HandleSetFactionCheat, not expected call, please report.");
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "WORLD: Received CMSG_SET_WATCHED_FACTION");
#endif
    uint32 fact;
    recvData >> fact;
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "WORLD: Received CMSG_SET_FACTION_INACTIVE");
#endif
    uint32 replistid;
    uint8 inactive;
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "CMSG_SHOWING_HELM for %s", _player->GetName().c_str());
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "CMSG_SHOWING_CLOAK for %s", _player->GetName().c_str());
#endif
    recvData.read_skip<uint8>(); // unknown, bool?
...
void WorldSession::HandleChangePlayerNameOpcodeCallBack(PreparedQueryResult result, std::string const& newName)
{
    ...
    LOG_DEBUG("root", "Account: %d (IP: %s), Character [%s] (guid: %u) Changed name to: %s", GetAccountId(), GetRemoteAddress().c_str(), oldName.c_str(), guidLow, newName.c_str());
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "CMSG_ALTER_APPEARANCE");
#endif
...
void WorldSession::HandleRemoveGlyph(WorldPacket& recvData)
{
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "Client sent wrong glyph slot number in opcode CMSG_REMOVE_GLYPH %u", slot);
#endif
        return;
    }
...
void WorldSession::HandleCharCustomize(WorldPacket& recvData)
{
    ...
    {
        LOG_ERROR("root", "Account %u, IP: %s tried to customise character %u, but it does not belong to their account!",
    ...
    LOG_DEBUG("root", "Account: %d (IP: %s), Character [%s] (guid: %u) Customized to: %s", GetAccountId(), GetRemoteAddress().c_str(), playerData->name.c_str(), GUID_LOPART(guid), newName.c_str());
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "CMSG_EQUIPMENT_SET_SAVE");
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "CMSG_EQUIPMENT_SET_DELETE");
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "CMSG_EQUIPMENT_SET_USE");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "Item " UI64FMTD ": srcbag %u, srcslot %u", itemGuid, srcbag, srcslot);
#endif
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket& recvData)
{
    ...
    if (!IsLegitCharacterForAccount(GUID_LOPART(guid)))
    {
        LOG_ERROR("root", "Account %u, IP: %s tried to factionchange character %u, but it does not belong to their account!",
            GetAccountId(), GetRemoteAddress().c_str(), GUID_LOPART(guid));
        recvData.rfinish();
    ...
    LOG_DEBUG("root", "Account: %d (IP: %s), Character [%s] (guid: %u) Changed Race/Faction to: %s", GetAccountId(), GetRemoteAddress().c_str(), playerData->name.c_str(), lowGuid, newname.c_str());
    ...
    std::string IP_str = GetRemoteAddress();
    //LOG_DEBUG("root", "%s (IP: %s) changed race from %u to %u", GetPlayerInfo().c_str(), IP_str.c_str(), oldRace, race);
