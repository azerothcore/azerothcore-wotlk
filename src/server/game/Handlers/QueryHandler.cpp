...
void WorldSession::HandleCreatureQueryOpcode(WorldPacket & recvData)
{
    ...
    else
    {
        ...
        LOG_DEBUG("server", "WORLD: CMSG_CREATURE_QUERY - NO CREATURE INFO! (GUID: %u, ENTRY: %u)", GUID_LOPART(guid), entry);
#endif
        WorldPacket data(SMSG_CREATURE_QUERY_RESPONSE, 4);
        data << uint32(entry | 0x80000000);
        SendPacket(&data);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "WORLD: Sent SMSG_CREATURE_QUERY_RESPONSE");
#endif
    }
}
...
void WorldSession::HandleGameObjectQueryOpcode(WorldPacket & recvData)
{
    ...
    if (info)
    {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "WORLD: CMSG_GAMEOBJECT_QUERY '%s' - Entry: %u. ", info->name.c_str(), entry);
#endif
        ...
        SendPacket(&data);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "WORLD: Sent SMSG_GAMEOBJECT_QUERY_RESPONSE");
#endif
    }
    else
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "WORLD: CMSG_GAMEOBJECT_QUERY - Missing gameobject info for (GUID: %u, ENTRY: %u)",
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received MSG_CORPSE_QUERY");
#endif
...
void WorldSession::HandleNpcTextQueryOpcode(WorldPacket & recvData)
{
    ...
    recvData >> textID;
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: CMSG_NPC_TEXT_QUERY TextId: %u", textID);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Sent SMSG_NPC_TEXT_UPDATE");
#endif
}
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received CMSG_PAGE_TEXT_QUERY");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "WORLD: Sent SMSG_PAGE_TEXT_QUERY_RESPONSE");
#endif
    }
}
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Recv CMSG_CORPSE_MAP_POSITION_QUERY");
#endif
