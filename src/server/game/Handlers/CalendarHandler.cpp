...
bool validUtf8String(WorldPacket& recvData, std::string& s, std::string action, uint64 playerGUID)
{
    ...
    {
        sLog->outString("CalendarHandler: Player with guid %llu attempt to %s an event with invalid name or description (packet modification)", playerGUID, action.c_str());
        recvData.rfinish();
        return false;
    }
    return true;
