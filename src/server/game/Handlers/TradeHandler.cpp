...
void WorldSession::HandleIgnoreTradeOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    LOG_DEBUG("server", "WORLD: Ignore Trade %u", _player->GetGUIDLow());
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Busy Trade %u", _player->GetGUIDLow());
#endif
    // recvPacket.print_storage();
}
    ...
    for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        ...
        if (traderCanTrade && playerCanTrade)
        {
            ...
            if (myItems[i])
            {
                // logging
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
                LOG_DEBUG("server", "partner storing: %u", myItems[i]->GetGUIDLow());
#endif
            ...
            if (hisItems[i])
            {
                // logging
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
                LOG_DEBUG("server", "player storing: %u", hisItems[i]->GetGUIDLow());
#endif
