...
void WorldSession::HandleRepopRequestOpcode(WorldPacket & recv_data)
{
    ...
    LOG_DEBUG("server", "WORLD: Recvd CMSG_REPOP_REQUEST Message");
#endif
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "HandleRepopRequestOpcode: got request after player %s(%d) was killed and before he was updated", GetPlayer()->GetName().c_str(), GetPlayer()->GetGUIDLow());
#endif
...
void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket & recv_data)
{
    ...
    else if (IS_GAMEOBJECT_GUID(guid))
    {
        ...
        {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            LOG_DEBUG("server", "WORLD: HandleGossipSelectOptionOpcode - GameObject (GUID: %u) not found.", uint32(GUID_LOPART(guid)));
#endif
            return;
        }
    }
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "WORLD: HandleGossipSelectOptionOpcode - unsupported GUID type for highguid %u. lowpart %u.", uint32(GUID_HIPART(guid)), uint32(GUID_LOPART(guid)));
#endif
        return;
    }
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "WORLD: HandleGossipSelectOptionOpcode - Script reloaded while in use, ignoring and set new scipt id");
#endif
        if (unit)
...
{
    LOG_DEBUG("server", "WORLD: Recvd CMSG_WHO Message");
    ...
    for (uint32 i = 0; i < zones_count; ++i)
    {
        ...
        zoneids[i] = temp;
        LOG_DEBUG("server", "Zone %u: %u", i, zoneids[i]);
    }
    ...
    LOG_DEBUG("server", "Minlvl %u, maxlvl %u, name %s, guild %s, racemask %u, classmask %u, zones %u, strings %u", level_min, level_max, player_name.c_str(), guild_name.c_str(), racemask, classmask, zones_count, str_count);
    ...
    for (uint32 i = 0; i < str_count; ++i)
    {
        ...
        LOG_DEBUG("server", "String %u: %s", i, temp.c_str());
    }
    ...
    SendPacket(&data);
    ;// LOG_DEBUG("server", "WORLD: Send SMSG_WHO Message");
}
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Recvd CMSG_LOGOUT_REQUEST Message, security - %u", GetSecurity());
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Recvd CMSG_PLAYER_LOGOUT Message");
#endif
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Recvd ZONE_UPDATE: %u", newZone);
#endif
...
void WorldSession::HandleStandStateChangeOpcode(WorldPacket & recv_data)
{
    ...
    LOG_DEBUG("server", "WORLD: Received CMSG_STANDSTATECHANGE");
#endif
    uint32 animstate;
    recv_data >> animstate;
...
void WorldSession::HandleContactListOpcode(WorldPacket & recv_data)
{
    ...
    recv_data >> unk;
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received CMSG_CONTACT_LIST - Unk: %d", unk);
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received CMSG_ADD_FRIEND");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: %s asked to add friend : '%s'", GetPlayer()->GetName().c_str(), friendName.c_str());
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
                    LOG_DEBUG("server", "WORLD: %s's friend list is full.", GetPlayer()->GetName().c_str());
#endif
                }
            }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Sent (SMSG_FRIEND_STATUS)");
#endif
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received CMSG_DEL_FRIEND");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Sent motd (SMSG_FRIEND_STATUS)");
#endif
}
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received CMSG_ADD_IGNORE");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: %s asked to Ignore: '%s'", GetPlayer()->GetName().c_str(), ignoreName.c_str());
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received CMSG_DEL_IGNORE");
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "CMSG_SET_CONTACT_NOTES");
#endif
    uint64 guid;
    std::string note;
    recv_data >> guid >> note;
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    if (suggestion == 0)
        LOG_DEBUG("server", "WORLD: Received CMSG_BUG [Bug Report]");
    else
        LOG_DEBUG("server", "WORLD: Received CMSG_BUG [Suggestion]");

    LOG_DEBUG("server", "%s", type.c_str());
    LOG_DEBUG("server", "%s", content.c_str());
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received CMSG_RECLAIM_CORPSE");
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received CMSG_RESURRECT_RESPONSE");
#endif
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket& recv_data)
{
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "CMSG_AREATRIGGER. Trigger ID: %u", triggerId);
#endif
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) in flight, ignore Area Trigger ID:%u",
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) send unknown (by DBC) Area Trigger ID:%u",
            player->GetName().c_str(), player->GetGUIDLow(), triggerId);
#endif
        return;
    }
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) too far (trigger map: %u player map: %u), ignore Area Trigger ID: %u",
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received CMSG_UPDATE_ACCOUNT_DATA");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "UAD: type %u, time %u, decompressedSize %u", type, timestamp, decompressedSize);
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received CMSG_REQUEST_ACCOUNT_DATA");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "RAD: type %u", type);
#endif
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "RAD: Failed to compress account data");
#endif
        return;
    }
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received CMSG_SET_ACTION_BUTTON");
#endif
    uint8 button;
    uint32 packetData;
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received CMSG_COMPLETE_CINEMATIC");
#endif
}
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received CMSG_NEXT_CINEMATIC_CAMERA");
#endif
}
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: CMSG_MOVE_FEATHER_FALL_ACK");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: CMSG_FORCE_MOVE_UNROOT_ACK");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: CMSG_FORCE_MOVE_ROOT_ACK");
#endif
...
void WorldSession::HandleInspectOpcode(WorldPacket& recv_data)
{
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received CMSG_INSPECT");
#endif
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "CMSG_INSPECT: No player found from GUID: " UI64FMTD, guid);
#endif
        return;
    }
...
void WorldSession::HandleInspectHonorStatsOpcode(WorldPacket& recv_data)
{
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "MSG_INSPECT_HONOR_STATS: No player found from GUID: " UI64FMTD, guid);
#endif
        return;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: Received CMSG_WORLD_TELEPORT");
#endif

    if (GetPlayer()->IsInFlight())
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "Player '%s' (GUID: %u) in flight, ignore worldport command.", GetPlayer()->GetName().c_str(), GetPlayer()->GetGUIDLow());
#endif
        return;
    }

#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "CMSG_WORLD_TELEPORT: Player = %s, Time = %u, map = %u, x = %f, y = %f, z = %f, o = %f", GetPlayer()->GetName().c_str(), time, mapid, PositionX, PositionY, PositionZ, Orientation);
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Received opcode CMSG_WHOIS");
#endif
    std::string charname;
    recv_data >> charname;
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Received whois command from player %s for character %s", GetPlayer()->GetName().c_str(), charname.c_str());
#endif
}
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: CMSG_COMPLAIN");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "REPORT SPAM: type %u, guid %u, unk1 %u, unk2 %u, unk3 %u, unk4 %u, message %s", spam_type, GUID_LOPART(spammer_guid), unk1, unk2, unk3, unk4, description.c_str());
#endif
}
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "CMSG_REALM_SPLIT");
#endif
    ...
    data << split_date;
    SendPacket(&data);
    //LOG_DEBUG("server", "response sent %u", unk);
}

void WorldSession::HandleFarSightOpcode(WorldPacket& recvData)
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: CMSG_FAR_SIGHT");
#endif
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "Added FarSight " UI64FMTD " to player %u", _player->GetUInt64Value(PLAYER_FARSIGHT), _player->GetGUIDLow());
#endif
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "Player %u set vision to self", _player->GetGUIDLow());
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "CMSG_SET_TITLE");
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: CMSG_RESET_INSTANCES");
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "MSG_SET_DUNGEON_DIFFICULTY");
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "MSG_SET_RAID_DIFFICULTY");
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: CMSG_CANCEL_MOUNT_AURA");
#endif
...
void WorldSession::HandleMoveSetCanFlyAckOpcode(WorldPacket & recv_data)
{
    // fly mode on/off
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: CMSG_MOVE_SET_CAN_FLY_ACK");
#endif
...
void WorldSession::HandleRequestPetInfoOpcode(WorldPacket & /*recv_data */)
{
    /*
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "WORLD: CMSG_REQUEST_PET_INFO");
#endif
        recv_data.hexlike();
    */
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Client used \"/timetest %d\" command", mode);
#endif
}
...
void WorldSession::HandleWorldStateUITimerUpdate(WorldPacket& /*recv_data*/)
{
    // empty opcode
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: CMSG_WORLD_STATE_UI_TIMER_UPDATE");
#endif
...
void WorldSession::HandleReadyForAccountDataTimes(WorldPacket& /*recv_data*/)
{
    // empty opcode
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: CMSG_READY_FOR_ACCOUNT_DATA_TIMES");
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: CMSG_AREA_SPIRIT_HEALER_QUERY");
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: CMSG_AREA_SPIRIT_HEALER_QUEUE");
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: CMSG_UPDATE_MISSILE_TRAJECTORY");
#endif
