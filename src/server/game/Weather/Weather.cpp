    ...
    : m_zone(zone), m_weatherChances(weatherChances)
{
    ...
    LOG_DEBUG("root", "WORLD: Starting weather system for zone %u (change every %u minutes).", m_zone, (uint32)(m_timer.GetInterval() / (MINUTE*IN_MILLISECONDS)));
#endif
}
...
bool Weather::ReGenerate()
{
    ...
    LOG_DEBUG("root", "Generating a change in %s weather for zone %u.", seasonName[season], m_zone);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "Change the weather of zone %u to %s.", m_zone, wthstr);
#endif
