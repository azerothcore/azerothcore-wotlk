...
void TransportMgr::LoadTransportTemplates()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("entities.transport", "Transport {} (name: %s) has an invalid path specified in `gameobject_template`.`data0` ({}) field, skipped.", entry, goInfo->name.c_str(), goInfo->moTransport.taxiPathId);
            continue;
        }
...
MotionTransport* TransportMgr::CreateTransport(uint32 entry, ObjectGuid::LowType guid /*= 0*/, Map* map /*= nullptr*/)
{
    ...
    if (MapEntry const* mapEntry = sMapStore.LookupEntry(mapId))
    {
        ...
        {
            LOG_ERROR("entities.transport", "Transport {} (name: %s) attempted creation in instance map (id: {}) but it is not an instanced transport!", entry, trans->GetName().c_str(), mapId);
            delete trans;
            return nullptr;
        }
    }
