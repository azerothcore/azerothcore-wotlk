...
public:
    ...
    [[nodiscard]] bool IsWaterWalking(uint32 phaseMask, float x, float y, float z, float collisionHeight) const;
    [[nodiscard]] bool IsAboveWater(uint32 phaseMask, float x, float y, float z, float collisionHeight) const;
    ...
    float GetWaterOrGroundLevel(uint32 phasemask, float x, float y, float z, float* ground = nullptr, bool swim = false, float collisionHeight = DEFAULT_COLLISION_HEIGHT, float maxSearchDist = DEFAULT_MAX_SEARCH_DISTANCE) const;    
