...
void PoolGroup<Creature>::Spawn1Object(PoolObject* obj)
{
    if (CreatureData const* data = sObjectMgr->GetCreatureData(obj->guid))
    {
        ...
        if (!map->Instanceable() && map->IsGridLoaded(data->posX, data->posY))
        {
            ...
            //LOG_DEBUG("server", "Spawning creature %u", guid);
...
void PoolGroup<GameObject>::Spawn1Object(PoolObject* obj)
{
    if (GameObjectData const* data = sObjectMgr->GetGOData(obj->guid))
    {
        ...
        if (!map->Instanceable() && map->IsGridLoaded(data->posX, data->posY))
        {
            ...
            //LOG_DEBUG("server", "Spawning gameobject %u", guid);
...
void PoolGroup<Quest>::Spawn1Object(PoolObject* obj)
{
    ...
    for (PooledQuestRelation::iterator itr = qr.first; itr != qr.second; ++itr)
    {
        ...
        LOG_DEBUG("server", "PoolGroup<Quest>: Adding quest %u to creature %u", itr->first, itr->second);
#endif
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "PoolGroup<Quest>: Adding quest %u to GO %u", itr->first, itr->second);
#endif
        questMap->insert(QuestRelations::value_type(itr->second, itr->first));
    }
}
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "PoolGroup<Quest>: Spawning pool %u", poolId);
#endif
    // load state from db
    if (!triggerFrom)
    {
    ...
    LOG_INFO("server", "Loading Creatures Pooling Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (!data)
                {
                    LOG_ERROR("server", "`pool_creature` has a non existing creature spawn (GUID: %u) defined for pool id (%u), skipped.", guid, pool_id);
                    continue;
                }
                if (pool_id > max_pool_id)
                {
                    LOG_ERROR("server", "`pool_creature` pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.", pool_id);
                    continue;
                }
                if (chance < 0 || chance > 100)
                {
                    LOG_ERROR("server", "`pool_creature` has an invalid chance (%f) for creature guid (%u) in pool id (%u), skipped.", chance, guid, pool_id);
                    continue;
                }
                PoolTemplateData* pPoolTemplate = &mPoolTemplate[pool_id];
    ...
    LOG_INFO("server", "Loading Gameobject Pooling Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (!data)
                {
                    LOG_ERROR("server", "`pool_gameobject` has a non existing gameobject spawn (GUID: %u) defined for pool id (%u), skipped.", guid, pool_id);
                    continue;
                }
                ...
                {
                    LOG_ERROR("server", "`pool_gameobject` has a not lootable gameobject spawn (GUID: %u, type: %u) defined for pool id (%u), skipped.", guid, goinfo->type, pool_id);
                    continue;
                }

                if (pool_id > max_pool_id)
                {
                    LOG_ERROR("server", "`pool_gameobject` pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.", pool_id);
                    continue;
                }

                if (chance < 0 || chance > 100)
                {
                    LOG_ERROR("server", "`pool_gameobject` has an invalid chance (%f) for gameobject guid (%u) in pool id (%u), skipped.", chance, guid, pool_id);
                    continue;
                }
    ...
    LOG_INFO("server", "Loading Mother Pooling Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    LOG_ERROR("server", "`pool_pool` mother_pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.", mother_pool_id);
                    continue;
                }
                if (child_pool_id > max_pool_id)
                {
                    LOG_ERROR("server", "`pool_pool` included pool_id (%u) is out of range compared to max pool id in `pool_template`, skipped.", child_pool_id);
                    continue;
                }
                if (mother_pool_id == child_pool_id)
                {
                    LOG_ERROR("server", "`pool_pool` pool_id (%u) includes itself, dead-lock detected, skipped.", child_pool_id);
                    continue;
                }
                if (chance < 0 || chance > 100)
                {
                    LOG_ERROR("server", "`pool_pool` has an invalid chance (%f) for pool id (%u) in mother pool id (%u), skipped.", chance, child_pool_id, mother_pool_id);
                    continue;
                }
            ...
            for (uint32 i=0; i < max_pool_id; ++i)
            {
                ...
                for (SearchMap::iterator poolItr = mPoolSearchMap.find(i); poolItr != mPoolSearchMap.end(); poolItr = mPoolSearchMap.find(poolItr->second))
                {
                    ...
                    if (checkedPools.find(poolItr->second) != checkedPools.end())
                    {
                        ...
                        LOG_ERROR("server", "%s", ss.str().c_str());
    ...
    LOG_INFO("server", "Loading Quest Pooling Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (!quest)
                {
                    LOG_ERROR("server", "`pool_quest` has a non existing quest template (Entry: %u) defined for pool id (%u), skipped.", entry, pool_id);
                    continue;
                }

                if (pool_id > max_pool_id)
                {
                    LOG_ERROR("server", "`pool_quest` pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.", pool_id);
                    continue;
                }

                if (!quest->IsDailyOrWeekly())
                {
                    LOG_ERROR("server", "`pool_quest` has an quest (%u) which is not daily or weekly in pool id (%u), use ExclusiveGroup instead, skipped.", entry, pool_id);
                    continue;
                }
                ...
                {
                    LOG_ERROR("server", "`pool_quest` quest %u is %s but pool (%u) is specified for %s, mixing not allowed, skipped.",
                ...
                {
                    LOG_ERROR("server", "`pool_quest` lists entry (%u) as member of pool (%u) but is not started anywhere, skipped.", entry, pool_id);
                    continue;
                }
            ...
            do
            {
                ...
                if (!CheckPool(pool_entry))
                {
                    if (pool_pool_id)
                        ...
                        LOG_ERROR("server", "Pool Id %u has no equal chance pooled entites defined and explicit chance sum is not 100. This broken pool is a child pool of Id %u and cannot be safely removed.", pool_entry, fields[2].GetUInt32());
                    else
                        LOG_ERROR("server", "Pool Id %u has no equal chance pooled entites defined and explicit chance sum is not 100. The pool will not be spawned.", pool_entry);
                    continue;
                }
