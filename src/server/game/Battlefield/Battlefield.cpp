...
bool Battlefield::AddOrSetPlayerToCorrectBfGroup(Player* player)
{
    ...
    {
        LOG_INFO("server", "Battlefield::AddOrSetPlayerToCorrectBfGroup - player is already in %s group!", (player->GetGroup()->isBGGroup() ? "BG" : "BF"));
        return false;
    }
...
Creature* Battlefield::SpawnCreature(uint32 entry, float x, float y, float z, float o, TeamId teamId)
{
    ...
    if (!cinfo)
    {
        LOG_ERROR("server", "Battlefield::SpawnCreature: entry %u does not exist.", entry);
        return NULL;
    }
...
GameObject* Battlefield::SpawnGameObject(uint32 entry, float x, float y, float z, float o)
{
    ...
    {
        LOG_ERROR("server", "Battlefield::SpawnGameObject: Gameobject template %u not found in database! Battlefield not created!", entry);
...
bool BfCapturePoint::SetCapturePointData(GameObject* capturePoint)
{
    ...
    LOG_DEBUG("server", "Creating capture point %u", capturePoint->GetEntry());
#endif
