...
void Battlefield::InitStalker(uint32 entry, float x, float y, float z, float o)
{
    if (Creature* creature = SpawnCreature(entry, x, y, z, o, TEAM_NEUTRAL))
        ...
        LOG_ERROR("root", "Battlefield::InitStalker: could not spawn Stalker (Creature entry %u), zone messeges will be un-available", entry);
}
...
bool Battlefield::AddOrSetPlayerToCorrectBfGroup(Player* player)
{
    ...
    {
        LOG_INFO("root", "Battlefield::AddOrSetPlayerToCorrectBfGroup - player is already in %s group!", (player->GetGroup()->isBGGroup() ? "BG" : "BF"));
        return false;
    }
...
BfGraveyard* Battlefield::GetGraveyardById(uint32 id) const
{
    if (id < m_GraveyardList.size())
    {
        if (m_GraveyardList[id])
            ...
            LOG_ERROR("root", "Battlefield::GetGraveyardById Id:%u not existed", id);
    }
    else
        LOG_ERROR("root", "Battlefield::GetGraveyardById Id:%u cant be found", id);
...
void BfGraveyard::SetSpirit(Creature* spirit, TeamId team)
{
    if (!spirit)
    {
        LOG_ERROR("root", "BfGraveyard::SetSpirit: Invalid Spirit.");
        return;
    }
...
Creature* Battlefield::SpawnCreature(uint32 entry, float x, float y, float z, float o, TeamId teamId)
{
    ...
    if (!map)
    {
        LOG_ERROR("root", "Battlefield::SpawnCreature: Can't create creature entry: %u map not found", entry);
        return 0;
    }
    ...
    {
        LOG_ERROR("root", "Battlefield::SpawnCreature: Can't create creature entry: %u", entry);
        delete creature;
        return NULL;
    }
    ...
    if (!cinfo)
    {
        LOG_ERROR("root", "Battlefield::SpawnCreature: entry %u does not exist.", entry);
        return NULL;
    }
...
GameObject* Battlefield::SpawnGameObject(uint32 entry, float x, float y, float z, float o)
{
    ...
    {
        LOG_ERROR("root", "Battlefield::SpawnGameObject: Gameobject template %u not found in database! Battlefield not created!", entry);
        LOG_ERROR("root", "Battlefield::SpawnGameObject: Cannot create gameobject template %u! Battlefield not created!", entry);
        delete go;
        return NULL;
    }
...
bool BfCapturePoint::SetCapturePointData(GameObject* capturePoint)
{
    ...
    LOG_DEBUG("root", "Creating capture point %u", capturePoint->GetEntry());
#endif
    ...
    {
        LOG_ERROR("root", "OutdoorPvP: GO %u is not capture point!", capturePoint->GetEntry());
        return false;
    }
