...
struct DefaultTargetSelector : public Acore::unary_function<Unit*, bool>
{
    ...
    DefaultTargetSelector(Unit const* unit, float dist, bool playerOnly, bool withMainTank, int32 aura) : me(unit), m_dist(dist), except(withMainTank ? nullptr : me->GetThreatMgr().GetCurrentVictim()), m_playerOnly(playerOnly), m_aura(aura) {}
