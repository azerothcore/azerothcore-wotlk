...
{
    if (!me->m_spells[0])
        LOG_ERROR("root", "ArcherAI set for creature (entry = %u) with spell1=0. AI will do nothing", me->GetEntry());
...
{
    if (!me->m_spells[0])
        LOG_ERROR("root", "TurretAI set for creature (entry = %u) with spell1=0. AI will do nothing", me->GetEntry());
...
void VehicleAI::LoadConditions()
{
    ...
    if (!conditions.empty())
        LOG_DEBUG("root", "VehicleAI::LoadConditions: loaded %u conditions", uint32(conditions.size()));
#endif
}
