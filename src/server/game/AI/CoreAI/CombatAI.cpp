...
{
    if (!me->m_spells[0])
        LOG_ERROR("scripts.ai", "ArcherAI set for creature (entry = %u) with spell1=0. AI will do nothing", me->GetEntry());
...
{
    if (!me->m_spells[0])
        LOG_ERROR("scripts.ai", "TurretAI set for creature (entry = %u) with spell1=0. AI will do nothing", me->GetEntry());
...
void VehicleAI::LoadConditions()
{
    ...
    if (!conditions.empty())
        LOG_DEBUG("scripts.ai", "VehicleAI::LoadConditions: loaded %u conditions", uint32(conditions.size()));
#endif
}
