...
void PetAI::_stopAttack()
{
    if (!me->IsAlive())
    {
        ...
        LOG_DEBUG("root", "Creature stoped attacking cuz his dead [guid=%u]", me->GetGUIDLow());
#endif
...
void PetAI::UpdateAI(uint32 diff)
{
    ...
    if (me->GetVictim() && me->GetVictim()->IsAlive())
    {
        ...
        {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            LOG_DEBUG("root", "Pet AI stopped attacking [guid=%u]", me->GetGUIDLow());
#endif
            _stopAttack();
            return;
        }
...
bool PetAI::CanAttack(Unit* target, const SpellInfo* spellInfo)
{
    ...
    if (!me->GetCharmInfo())
    {
        LOG_INFO("root", "PetAI::CanAttack (A1) - %u, %u", me->GetEntry(), GUID_LOPART(me->GetOwnerGUID()));
        return false;
    }
