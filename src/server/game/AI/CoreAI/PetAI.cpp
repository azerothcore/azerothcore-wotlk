...
void PetAI::_stopAttack()
{
    if (!me->IsAlive())
    {
        LOG_DEBUG("scripts.ai.petai", "Creature stoped attacking cuz his dead [guid=%u]", me->GetGUIDLow());
...
void PetAI::UpdateAI(uint32 diff)
{
    ...
    if (me->GetVictim() && me->GetVictim()->IsAlive())
    {
        ...
        if (_needToStop())
        {
            LOG_DEBUG("scripts.ai.petai", "Pet AI stopped attacking [guid=%u]", me->GetGUIDLow());
            _stopAttack();
            return;
        }
...
bool PetAI::CanAttack(Unit* target, const SpellInfo* spellInfo)
{
    ...
    if (!me->GetCharmInfo())
    {
        LOG_INFO("scripts.ai.petai", "PetAI::CanAttack (A1) - %u, %u", me->GetEntry(), GUID_LOPART(me->GetOwnerGUID()));
        return false;
    }
