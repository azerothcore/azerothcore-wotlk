        ...
        for (uint8 i = 0; i < me->GetPetAutoSpellSize(); ++i)
        {
            ...
            if (spellInfo->IsPositive())
            {
                if (spellInfo->CanBeUsedInCombat())
                {
                    ...
                    if (!me->IsInCombat() && spellInfo->GetMaxDuration() != -1 && !me->GetCharmInfo()->IsCommandAttack())
                    {
                        continue;
                    }
                }
...
void PetAI::HandleReturnMovement()
{
    ...
    me->SetUnitFlag(UNIT_FLAG_PET_IN_COMBAT); // on player pets, this flag indicates that we're actively going after a target - we're returning, so remove it
}
