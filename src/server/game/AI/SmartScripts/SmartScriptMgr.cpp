...
bool SmartAIMgr::IsEventValid(SmartScriptHolder& e)
{
    ...
    switch (e.GetActionType())
    {
        ...
        {
            if (e.action.startClosestWaypoint.pathId1 == 0 || e.action.startClosestWaypoint.pathId2 == 0)
            {
                LOG_ERROR("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has invalid pathId1 or pathId2 set to 0",
                    e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
                return false;
            }
            if (e.action.startClosestWaypoint.repeat > 1 || e.action.startClosestWaypoint.run > 1)
            {
                LOG_ERROR("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has invalid repeat ({}) or run ({}) parameter, must be 0 or 1.",
                    e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
