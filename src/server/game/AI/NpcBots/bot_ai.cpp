    ...
    if (gr && IsOffTank())
    {
        if (_primaryIconTank >= 0 && BotMgr::GetOffTankTargetIconFlags() & (1u << _primaryIconTank))
        {
            if (ObjectGuid guid = gr->GetTargetIcons()[_primaryIconTank])
            {
                ...
                {
                    if (unit->IsVisible() && unit->isTargetableForAttack(false) && me->IsValidAttackTarget(unit) &&
                        (unit->IsInCombat() || me->IsInCombat()) && (CanSeeEveryone() || (me->CanSeeOrDetect(unit) && unit->InSamePhase(me))))
                    {
                        return { unit, unit };
                    }
                }
...
void bot_ai::InitBotCustomSpells()
{
    ...
    if (BotMgr::IsClassEnabled(BOT_CLASS_BM))
    {
        ...
        trigInfo->Attributes |= (SPELL_ATTR0_NOT_SHAPESHIFTED | SPELL_ATTR0_ALLOW_WHILE_SITTING);
        trigInfo->AttributesEx |= (SPELL_ATTR1_ALLOW_WHILE_STEALTHED);
    ...
    if (BotMgr::IsClassEnabled(BOT_CLASS_SPHYNX))
    {
        ...
        //sinfo->Attributes |= SPELL_ATTR0_DO_NOT_LOG | SPELL_ATTR0_DO_NOT_DISPLAY | SPELL_ATTR0_DO_NOT_SHEATH;
