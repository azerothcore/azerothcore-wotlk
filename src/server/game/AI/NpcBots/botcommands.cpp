...
public:
    ...
    static bool HandleNpcBotHideCommand(ChatHandler* handler)
    {
        ...
        if (!owner->IsAlive())
        {
            ChatHandler(handler->GetSession()).SendNotification("You are dead");
            handler->SetSentErrorMessage(true);
            return false;
        }
        if (owner->GetBotMgr()->IsPartyInCombat())
        {
            ChatHandler(handler->GetSession()).SendNotification(LANG_YOU_IN_COMBAT);
            handler->SetSentErrorMessage(true);
            return false;
        }
    ...
    static bool HandleNpcBotUnhideCommand(ChatHandler* handler)
    {
        ...
        {
            ChatHandler(handler->GetSession()).SendNotification("You can't do that while in PvP combat");
            handler->SetSentErrorMessage(true);
            return false;
        }
