...
private:
    ...
    WanderingBotsGenerator()
    {
        ...
        QueryResult result = CharacterDatabase.Query("SELECT value FROM worldstates WHERE entry = %u", uint32(BOT_GIVER_ENTRY));
        if (!result)
        {
            ...
            for (uint32 bot_cid : BotDataMgr::GetExistingNPCBotIds())
                if (bot_cid > next_bot_id)
                    next_bot_id = bot_cid;
            CharacterDatabase.DirectExecute("INSERT INTO worldstates (entry, value, comment) VALUES (%u, %u, '%s')",
...
public:
    ...
    bool GenerateWanderingBotsToSpawn(uint32 count, int32 map_id, int32 team, bool immediate, PvPDifficultyEntry const* bracketEntry, NpcBotRegistry* registry, uint32& spawned)
    {
        ...
        CharacterDatabase.Execute("UPDATE worldstates SET value = %u WHERE entry = %u", next_bot_id, uint32(BOT_GIVER_ENTRY));
