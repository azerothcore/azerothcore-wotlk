...
void FollowerAI::EnterEvadeMode()
{
    ...
    if (HasFollowState(STATE_FOLLOW_INPROGRESS))
    {
        ...
        LOG_DEBUG("root", "TSCR: FollowerAI left combat, returning to CombatStartPosition.");
#endif
...
void FollowerAI::UpdateAI(uint32 uiDiff)
{
    if (HasFollowState(STATE_FOLLOW_INPROGRESS) && !me->GetVictim())
    {
        if (m_uiUpdateFollowTimer <= uiDiff)
        {
            ...
            {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
                LOG_DEBUG("root", "TSCR: FollowerAI is set completed, despawns.");
#endif
            ...
            if (Player* player = GetLeaderForFollower())
            {
                ...
                {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
                    LOG_DEBUG("root", "TSCR: FollowerAI is returning to leader.");
#endif
            ...
            {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
                LOG_DEBUG("root", "TSCR: FollowerAI failed because player/group was to far away or not found");
#endif
                me->DespawnOrUnsummon();
                return;
            }
...
void FollowerAI::StartFollow(Player* player, uint32 factionForFollower, const Quest* quest)
{
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "TSCR: FollowerAI attempt to StartFollow while in combat.");
#endif
        return;
    }
    ...
    if (HasFollowState(STATE_FOLLOW_INPROGRESS))
    {
        LOG_ERROR("root", "TSCR: FollowerAI attempt to StartFollow while already following.");
        return;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "TSCR: FollowerAI start with WAYPOINT_MOTION_TYPE, set to MoveIdle.");
#endif
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "TSCR: FollowerAI start follow %s (GUID " UI64FMTD ")", player->GetName().c_str(), m_uiLeaderGUID);
#endif
}
        ...
        else
        {
            if (Group* group = player->GetGroup())
            {
                for (GroupReference* groupRef = group->GetFirstMember(); groupRef != NULL; groupRef = groupRef->next())
                {
                    ...
                    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
                        LOG_DEBUG("root", "TSCR: FollowerAI GetLeader changed and returned new leader.");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "TSCR: FollowerAI GetLeader can not find suitable leader.");
#endif
    return NULL;
}
