...
void ScriptedAI::DoPlaySoundToSet(WorldObject* source, uint32 soundId)
{
    ...
    {
        LOG_ERROR("entities.unit.ai", "ASCR Invalid soundId %u used in DoPlaySoundToSet (Source: %s)", soundId, source->GetGUID().ToString().c_str());
        return;
    }
...
void ScriptedAI::DoTeleportPlayer(Unit* unit, float x, float y, float z, float o)
{
    ...
    if (Player* player = unit->ToPlayer())
        ...
        LOG_ERROR("entities.unit.ai", "ASCR Creature %s Tried to teleport non-player unit %s to x: %f y:%f z: %f o: %f. Aborted.",
