...
void ScriptedAI::DoPlaySoundToSet(WorldObject* source, uint32 soundId)
{
    ...
    {
        LOG_ERROR("root", "TSCR: Invalid soundId %u used in DoPlaySoundToSet (Source: TypeId %u, GUID %u)", soundId, source->GetTypeId(), source->GetGUIDLow());
        return;
    }
...
void ScriptedAI::DoResetThreat()
{
    ...
    {
        LOG_ERROR("root", "DoResetThreat called for creature that either cannot have threat list or has empty threat list (me entry = %d)", me->GetEntry());
        return;
    }
...
void ScriptedAI::DoTeleportPlayer(Unit* unit, float x, float y, float z, float o)
{
    ...
    if (Player* player = unit->ToPlayer())
        ...
        LOG_ERROR("root", "TSCR: Creature " UI64FMTD " (Entry: %u) Tried to teleport non-player unit (Type: %u GUID: " UI64FMTD ") to x: %f y:%f z: %f o: %f. Aborted.", me->GetGUID(), me->GetEntry(), unit->GetTypeId(), unit->GetGUID(), x, y, z, o);
}
