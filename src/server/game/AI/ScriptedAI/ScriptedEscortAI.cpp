...
void npc_escortAI::EnterEvadeMode()
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        ...
        ReturnToLastPoint();
        LOG_DEBUG("scripts.ai.escortai", "TSCR: EscortAI has left combat and is now returning to last point");
    }
    else
    {
...
void npc_escortAI::MovementInform(uint32 moveType, uint32 pointId)
{
    ...
    if (moveType == POINT_MOTION_TYPE)
    {
        ...
        {
            LOG_DEBUG("scripts.ai.escortai", "TSCR: EscortAI has returned to original position before combat");
        ...
        else if (pointId == POINT_HOME)
        {
            ...
            LOG_DEBUG("scripts.ai.escortai", "TSCR: EscortAI has returned to original home location and will continue from beginning of waypoint list.");
...
void npc_escortAI::SetRun(bool on)
{
    if (on)
    {
        if (!m_bIsRunning)
            ...
            LOG_DEBUG("scripts.ai.escortai", "TSCR: EscortAI attempt to set run mode, but is already running.");
    }
    else
    {
        if (m_bIsRunning)
            ...
            LOG_DEBUG("scripts.ai.escortai", "TSCR: EscortAI attempt to set walk mode, but is already walking.");
    }
...
void npc_escortAI::Start(bool isActiveAttacker /* = true*/, bool run /* = false */, uint64 playerGUID /* = 0 */, Quest const* quest /* = NULL */, bool instantRespawn /* = false */, bool canLoopPath /* = false */, bool resetWaypoints /* = true */)
{
    if (me->GetVictim())
    {
        LOG_ERROR("scripts.ai.escortai", "TSCR ERROR: EscortAI (script: %s, creature entry: %u) attempts to Start while in combat", me->GetScriptName().c_str(), me->GetEntry());
        return;
    }

    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        LOG_ERROR("scripts.ai.escortai", "TSCR: EscortAI (script: %s, creature entry: %u) attempts to Start while already escorting", me->GetScriptName().c_str(), me->GetEntry());
        return;
    }
    ...
    if (WaypointList.empty())
    {
        LOG_ERROR("scripts.ai.escortai", "TSCR: EscortAI (script: %s, creature entry: %u) starts with 0 waypoints (possible missing entry in script_waypoint. Quest: %u).",
    ...
    if (m_bCanReturnToStart && m_bCanInstantRespawn)
        LOG_DEBUG("scripts.ai.escortai", "TSCR: EscortAI is set to return home after waypoint end and instant respawn at waypoint end. Creature will never despawn.");
    ...
    if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
    {
        ...
        LOG_DEBUG("scripts.ai.escortai", "TSCR: EscortAI start with WAYPOINT_MOTION_TYPE, changed to MoveIdle.");
    }
    ...
    LOG_DEBUG("scripts.ai.escortai", "TSCR: EscortAI started with " UI64FMTD " waypoints. ActiveAttacker = %d, Run = %d, PlayerGUID = " UI64FMTD "", uint64(WaypointList.size()), m_bIsActiveAttacker, m_bIsRunning, m_uiPlayerGUID);
