...
void npc_escortAI::EnterEvadeMode()
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        ...
        ReturnToLastPoint();
        LOG_DEBUG("scripts.ai", "ASCR EscortAI has left combat and is now returning to last point");
    }
    else
    {
...
void npc_escortAI::MovementInform(uint32 moveType, uint32 pointId)
{
    ...
    if (moveType == POINT_MOTION_TYPE)
    {
        ...
        {
            LOG_DEBUG("scripts.ai", "ASCR EscortAI has returned to original position before combat");
        ...
        {
            LOG_DEBUG("scripts.ai", "ASCR EscortAI has returned to original home location and will continue from beginning of waypoint list.");
...
void npc_escortAI::SetRun(bool on)
{
    if (on)
    {
        if (!m_bIsRunning)
            ...
            LOG_DEBUG("scripts.ai", "ASCR EscortAI attempt to set run mode, but is already running.");
    }
    else
    {
        if (m_bIsRunning)
            ...
            LOG_DEBUG("scripts.ai", "ASCR EscortAI attempt to set walk mode, but is already walking.");
    }
...
void npc_escortAI::Start(bool isActiveAttacker /* = true*/, bool run /* = false */, ObjectGuid playerGUID /* = ObjectGuid::Empty */, Quest const* quest /* = nullptr */, bool instantRespawn /* = false */, bool canLoopPath /* = false */, bool resetWaypoints /* = true */)
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        LOG_ERROR("entities.unit.ai", "ASCR EscortAI (script: %s, creature entry: %u) attempts to Start while already escorting", me->GetScriptName().c_str(), me->GetEntry());
        return;
    }
    ...
    if (WaypointList.empty())
    {
        LOG_ERROR("sql.sql", "ASCR EscortAI (script: %s, creature entry: %u) starts with 0 waypoints (possible missing entry in script_waypoint. Quest: %u).",
    ...
    if (m_bCanReturnToStart && m_bCanInstantRespawn)
        LOG_DEBUG("scripts.ai", "ASCR EscortAI is set to return home after waypoint end and instant respawn at waypoint end. Creature will never despawn.");

    if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
    {
        me->GetMotionMaster()->MovementExpired();
        me->GetMotionMaster()->MoveIdle();
        LOG_DEBUG("scripts.ai", "ASCR EscortAI start with WAYPOINT_MOTION_TYPE, changed to MoveIdle.");
    }
    ...
    LOG_DEBUG("scripts.ai", "ASCR EscortAI started with " UI64FMTD " waypoints. ActiveAttacker = %d, Run = %d, PlayerGUID = %s",
