...
void BattlegroundAV::HandleKillUnit(Creature* unit, Player* killer)
{
    ...
    LOG_DEBUG("server", "bg_av HandleKillUnit %i", unit->GetEntry());
#endif
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "BG_AV Quest %i completed", questid);
#endif
    switch (questid)
    {
        ...
        case AV_QUEST_H_SCRAPS2:
            ...
            {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
                LOG_DEBUG("server", "BG_AV Quest %i completed starting with unit upgrading..", questid);
#endif
        ...
        case AV_QUEST_H_COMMANDER1:
            ...
            if (m_Team_QuestStatus[teamId][1] == 30)
                LOG_DEBUG("server", "BG_AV Quest %i completed (need to implement some events here", questid);
#endif
            break;
        ...
        case AV_QUEST_H_NEAR_MINE:
            ...
            if (m_Team_QuestStatus[teamId][5] == 28)
            {
                ...
                if (m_Team_QuestStatus[teamId][6] == 7)
                    LOG_DEBUG("server", "BG_AV Quest %i completed (need to implement some events here - ground assault ready", questid);
#endif
            }
            break;
        ...
        case AV_QUEST_H_RIDER_HIDE:
            ...
            if (m_Team_QuestStatus[teamId][7] == 25)
            {
                ...
                if (m_Team_QuestStatus[teamId][8] == 25)
                    LOG_DEBUG("server", "BG_AV Quest %i completed (need to implement some events here - rider assault ready", questid);
#endif
            }
            break;
        ...
        default:
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            LOG_DEBUG("server", "BG_AV Quest %i completed but is not interesting at all", questid);
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "BG_AV: start spawning mine stuff");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "bg_av: player destroyed point node %i object %i", node, object);
#endif
...
void BattlegroundAV::ChangeMineOwner(uint8 mine, TeamId teamId, bool initial)
{
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "bg_av depopulating mine %i (0=north, 1=south)", mine);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "bg_av populating mine %i (0=north, 1=south)", mine);
#endif
    uint16 miner;
    ...
    else
    {
       ...
       //vermin
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "spawning vermin");
#endif
        if (teamId == TEAM_ALLIANCE)
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "bg_AV getnodethroughobject %i", object);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "bg_AV GetObjectThroughNode %i", node);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "player defends point object: %i node: %i", object, node);
#endif
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object)
{
    ...
    TeamId teamId  = player->GetTeamId();
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "bg_av: player assaults point object %i node %i", object, node);
#endif
...
bool BattlegroundAV::SetupBattleground()
{
    ...
    {
        LOG_ERROR("server", "BatteGroundAV: Failed to spawn some object Battleground not created!1");
        return false;
    }
    ...
    uint16 i;
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Alterac Valley: entering state STATUS_WAIT_JOIN ...");
#endif
    // Initial Nodes
    ...
    //creatures
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "BG_AV start poputlating nodes");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "BG_AV: start spawning static creatures");
#endif
    for (i = 0; i < AV_STATICCPLACE_MAX; i++)
        AddAVCreature(0, i + AV_CPLACE_MAX);
    //mainspiritguides:
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "BG_AV: start spawning spiritguides creatures");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "BG_AV: start spawning marshal creatures");
#endif
