...
void BattlegroundAV::HandleKillUnit(Creature* unit, Player* killer)
{
    ...
    LOG_DEBUG("root", "bg_av HandleKillUnit %i", unit->GetEntry());
#endif
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
    ...
    */
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_CAPTAIN])
    {
        if (!m_CaptainAlive[0])
        {
            LOG_ERROR("root", "Killed a Captain twice, please report this bug, if you haven't done \".respawn\"");
            return;
        }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "BG_AV Quest %i completed", questid);
#endif
    switch (questid)
    {
        ...
        case AV_QUEST_H_SCRAPS2:
            ...
            {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
                LOG_DEBUG("root", "BG_AV Quest %i completed starting with unit upgrading..", questid);
#endif
        ...
        case AV_QUEST_H_COMMANDER1:
            ...
            if (m_Team_QuestStatus[teamId][1] == 30)
                LOG_DEBUG("root", "BG_AV Quest %i completed (need to implement some events here", questid);
#endif
            break;
        ...
        case AV_QUEST_H_NEAR_MINE:
            ...
            if (m_Team_QuestStatus[teamId][5] == 28)
            {
                ...
                if (m_Team_QuestStatus[teamId][6] == 7)
                    LOG_DEBUG("root", "BG_AV Quest %i completed (need to implement some events here - ground assault ready", questid);
#endif
            }
            break;
        ...
        case AV_QUEST_H_RIDER_HIDE:
            ...
            if (m_Team_QuestStatus[teamId][7] == 25)
            {
                ...
                if (m_Team_QuestStatus[teamId][8] == 25)
                    LOG_DEBUG("root", "BG_AV Quest %i completed (need to implement some events here - rider assault ready", questid);
#endif
            }
            break;
        ...
        default:
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            LOG_DEBUG("root", "BG_AV Quest %i completed but is not interesting at all", questid);
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "BG_AV: start spawning mine stuff");
#endif
...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "bg_av: player destroyed point node %i object %i", node, object);
#endif
    ...
    if (IsTower(node))
    {
        ...
        if (BgCreatures[AV_CPLACE_A_MARSHAL_SOUTH + tmp])
            ...
            LOG_ERROR("root", "BG_AV: playerdestroyedpoint: marshal %i doesn't exist", AV_CPLACE_A_MARSHAL_SOUTH + tmp);
        //spawn destroyed aura
        for (uint8 i=0; i <= 9; i++)
...
void BattlegroundAV::ChangeMineOwner(uint8 mine, TeamId teamId, bool initial)
{
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "bg_av depopulating mine %i (0=north, 1=south)", mine);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "bg_av populating mine %i (0=north, 1=south)", mine);
#endif
    uint16 miner;
    ...
    else
    {
       ...
       //vermin
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "spawning vermin");
#endif
        if (teamId == TEAM_ALLIANCE)
...
void BattlegroundAV::PopulateNode(BG_AV_Nodes node)
{
    ...
    else
    {
        ...
        if (!AddSpiritGuide(node, BG_AV_CreaturePos[node][0], BG_AV_CreaturePos[node][1], BG_AV_CreaturePos[node][2], BG_AV_CreaturePos[node][3], ownerId))
            LOG_ERROR("root", "AV: couldn't spawn spiritguide at node %i", node);
    }
    for (uint8 i=0; i<4; i++)
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "bg_AV getnodethroughobject %i", object);
#endif
    ...
    LOG_ERROR("root", "BattlegroundAV: ERROR! GetPlace got a wrong object :(");
    ASSERT(false);
...
{ //this function is the counterpart to GetNodeThroughObject()
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "bg_AV GetObjectThroughNode %i", node);
#endif
    ...
    LOG_ERROR("root", "BattlegroundAV: Error! GetPlaceNode couldn't resolve node %i", node);
    ASSERT(false);
    return 0;
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "player defends point object: %i node: %i", object, node);
#endif
    if (m_Nodes[node].PrevOwnerId != teamId)
    {
        LOG_ERROR("root", "BG_AV: player defends point which doesn't belong to his team %i", node);
        return;
    }
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object)
{
    ...
    TeamId teamId  = player->GetTeamId();
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "bg_av: player assaults point object %i node %i", object, node);
#endif
...
uint8 BattlegroundAV::GetWorldStateType(uint8 state, TeamId teamId) //this is used for node worldstates and returns values which fit good into the worldstatesarray
{
    ...
    LOG_ERROR("root", "BG_AV: should update a strange worldstate state:%i team:%i", state, teamId);
...
bool BattlegroundAV::SetupBattleground()
{
    ...
    {
        LOG_ERROR("root", "BatteGroundAV: Failed to spawn some object Battleground not created!1");
        return false;
    }
    ...
    for (uint8 i = BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX; ++i)
    {
        if (i <= BG_AV_NODES_FROSTWOLF_HUT)
        {
            ...
            {
                LOG_ERROR("root", "BatteGroundAV: Failed to spawn some object Battleground not created!2");
                return false;
            }
        }
        else //towers
        {
            if (i <= BG_AV_NODES_STONEHEART_BUNKER) //alliance towers
            {
                ...
                {
                    LOG_ERROR("root", "BatteGroundAV: Failed to spawn some object Battleground not created!3");
                    return false;
                }
            }
            else //horde towers
            {
                ...
                {
                    LOG_ERROR("root", "BatteGroundAV: Failed to spawn some object Battleground not created!4");
                    return false;
                }
            }
            for (uint8 j=0; j <= 9; j++) //burning aura
            {
                ...
                {
                    LOG_ERROR("root", "BatteGroundAV: Failed to spawn some object Battleground not created!5.%i", i);
                    return false;
                }
            }
        }
    ...
    for (uint8 i=0; i<2; i++) //burning aura for buildings
    {
        for (uint8 j=0; j <= 9; j++)
        {
            if (j<5)
            {
                ...
                {
                    LOG_ERROR("root", "BatteGroundAV: Failed to spawn some object Battleground not created!6.%i", i);
                    return false;
                }
            }
            else
            {
                ...
                {
                    LOG_ERROR("root", "BatteGroundAV: Failed to spawn some object Battleground not created!7.%i", i);
                    return false;
                }
            }
        }
    ...
    for (uint16 i= 0; i <= (BG_AV_OBJECT_MINE_SUPPLY_N_MAX-BG_AV_OBJECT_MINE_SUPPLY_N_MIN); i++)
    {
        ...
        {
            LOG_ERROR("root", "BatteGroundAV: Failed to spawn some mine supplies Battleground not created!7.5.%i", i);
            return false;
        }
    }
    for (uint16 i= 0; i <= (BG_AV_OBJECT_MINE_SUPPLY_S_MAX-BG_AV_OBJECT_MINE_SUPPLY_S_MIN); i++)
    {
        ...
        {
            LOG_ERROR("root", "BatteGroundAV: Failed to spawn some mine supplies Battleground not created!7.6.%i", i);
            return false;
        }
    }
    ...
    {
        LOG_ERROR("root", "BatteGroundAV: Failed to spawn some object Battleground not created!8");
        return false;
    }
    for (uint8 i = 0; i < 4; i++)
    {
        ...
        {
            LOG_ERROR("root", "BatteGroundAV: Failed to spawn some object Battleground not created!9.%i", i);
            return false;
        }
    }
    ...
    uint16 i;
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "Alterac Valley: entering state STATUS_WAIT_JOIN ...");
#endif
    // Initial Nodes
    ...
    //creatures
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "BG_AV start poputlating nodes");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "BG_AV: start spawning static creatures");
#endif
    for (i = 0; i < AV_STATICCPLACE_MAX; i++)
        AddAVCreature(0, i + AV_CPLACE_MAX);
    //mainspiritguides:
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "BG_AV: start spawning spiritguides creatures");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "BG_AV: start spawning marshal creatures");
#endif
...
char const* BattlegroundAV::GetNodeName(BG_AV_Nodes node)
{
    switch (node)
    {
        ...
        default:
            LOG_ERROR("root", "tried to get name for node %u", node);
            break;
    }
...
void BattlegroundAV::AssaultNode(BG_AV_Nodes node, TeamId teamId)
{
    ...
    {
        LOG_FATAL("root", "Assaulting team is TotalOwner of node");
        ASSERT(false);
    }
    if (m_Nodes[node].OwnerId == teamId)
    {
        LOG_FATAL("root", "Assaulting team is owner of node");
        ASSERT(false);
    }
    if (m_Nodes[node].State == POINT_DESTROYED)
    {
        LOG_FATAL("root", "Destroyed node is being assaulted");
        ASSERT(false);
    }
    ...
    {
        LOG_FATAL("root", "Assault on an not assaulted node with total owner");
        ASSERT(false);
    }
