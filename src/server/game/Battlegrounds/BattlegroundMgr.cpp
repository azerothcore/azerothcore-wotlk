...
void BattlegroundMgr::BuildPvpLogDataPacket(WorldPacket* data, Battleground* bg)
{
    ...
    *data << uint32(scoreCount);                            // placeholder
    ...
    for (Battleground::BattlegroundScoreMap::const_iterator itr = itr2; itr != bg->GetPlayerScoresEnd();)
    {
        ...
        {
            LOG_ERROR("bg.battleground", "Player " UI64FMTD " has scoreboard entry for battleground %u but is not in battleground!", itr->first, bg->GetBgTypeID());
            continue;
        }
        ...
        *data << uint32(itr2->second->HealingDone);             // healing done
        ...
        {
            LOG_INFO("bg.battleground", "Battleground %u scoreboard has more entries (%u) than allowed players in this bg (%u)", bg->GetBgTypeID(), bg->GetPlayerScoresSize(), bg->GetMaxPlayersPerTeam()*2);
            break;
        }
    }
...
void BattlegroundMgr::CreateInitialBattlegrounds()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("sql.sql", "Table `battleground_template` for id %u has bad values for MinPlayersPerTeam (%u) and MaxPlayersPerTeam(%u)",
        ...
        {
            LOG_ERROR("sql.sql", "Table `battleground_template` for id %u has bad values for LevelMin (%u) and LevelMax(%u)",
        ...
        else
        {
            ...
            {
                LOG_ERROR("sql.sql", "Table `battleground_template` for id %u have non-existed `game_graveyard` table id %u in field `AllianceStartLoc`. BG not created.", data.bgTypeId, startId);
                continue;
            }
            ...
            {
                LOG_ERROR("sql.sql", "Table `battleground_template` for id %u have non-existed `game_graveyard` table id %u in field `HordeStartLoc`. BG not created.", data.bgTypeId, startId);
                continue;
            }
        }
...
void BattlegroundMgr::InitAutomaticArenaPointDistribution()
{
    ...
    LOG_INFO("server.loading", "Battleground: Initializing Automatic Arena Point Distribution");
    
    if (wstime < curtime)
    {
        m_NextAutoDistributionTime = curtime;           // reset will be called in the next update
        LOG_INFO("server.loading", "Battleground: Next arena point distribution time in the past, reseting it now.");
    }
    else
        m_NextAutoDistributionTime = wstime;
 	
    LOG_INFO("server.loading", "Battleground: Automatic Arena Point Distribution initialized.");
}
