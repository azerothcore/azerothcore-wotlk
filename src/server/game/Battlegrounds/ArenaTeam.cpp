...
bool ArenaTeam::AddMember(uint64 playerGuid)
{
    ...
    {
        LOG_ERROR("root", "Arena: Player %s (guid: %u) already has an arena team of type %u", playerName.c_str(), GUID_LOPART(playerGuid), GetType());
        return false;
    }
...
bool ArenaTeam::LoadMembersFromDB(QueryResult result)
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("root", "ArenaTeam %u has member with empty name - probably player %u doesn't exist, deleting him from memberlist!", arenaTeamId, GUID_LOPART(newMember.Guid));
    ...
    if (Empty() || !captainPresentInTeam)
    {
        ...
        LOG_DEBUG("root", "ArenaTeam %u does not have any members or its captain is not in team, disbanding it...", TeamId);
#endif
        return false;
    }
...
void ArenaTeam::SetCaptain(uint64 guid)
{
    ...
    if (Player* newCaptain = ObjectAccessor::FindPlayerInOrOutOfWorld(guid))
    {
        ...
        /*if (oldCaptain)
        {
            LOG_DEBUG("root","Player: %s [GUID: %u] promoted player: %s [GUID: %u] to leader of arena team [Id: %u] [Type: %u].",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "WORLD: Sent SMSG_ARENA_TEAM_ROSTER");
#endif
}
...
void ArenaTeam::Query(WorldSession* session)
{
    ...
    session->SendPacket(&data);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "WORLD: Sent SMSG_ARENA_TEAM_QUERY_RESPONSE");
#endif
}
...
void ArenaTeam::BroadcastEvent(ArenaTeamEvents event, uint64 guid, uint8 strCount, std::string const& str1, std::string const& str2, std::string const& str3)
{
    ...
    switch (strCount)
    {
        ...
        case 3:
            ...
            break;
        default:
            LOG_ERROR("root", "Unhandled strCount %u in ArenaTeam::BroadcastEvent", strCount);
            return;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "WORLD: Sent SMSG_ARENA_TEAM_EVENT");
#endif
}
...
uint8 ArenaTeam::GetSlotByType(uint32 type)
{
    ...
    LOG_ERROR("root", "FATAL: Unknown arena team type %u for some arena team", type);
    return 0xFF;
}
