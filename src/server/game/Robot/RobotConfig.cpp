...
bool RobotConfig::LoadInitial(std::string const& file)
{
    ASSERT(file.c_str());
...
bool RobotConfig::LoadMore(std::string const& file)
{
    ASSERT(file.c_str());
    ASSERT(_config);
...
bool RobotConfig::LoadData(std::string const& file)
{
...
std::string RobotConfig::GetStringDefault(std::string const& name, const std::string& def, bool logUnused /*= true*/)
{
    ACE_TString val;
...
bool RobotConfig::GetBoolDefault(std::string const& name, bool def, bool logUnused /*= true*/)
{
    ACE_TString val;
...
int RobotConfig::GetIntDefault(std::string const& name, int def, bool logUnused /*= true*/)
{
    ACE_TString val;
...
float RobotConfig::GetFloatDefault(std::string const& name, float def, bool logUnused /*= true*/)
{
    ACE_TString val;
...
void RobotConfig::SetConfigList(std::string const& fileName, std::string const& modulesConfigList /*= ""*/)
{
...
bool RobotConfig::LoadAppConfigs(std::string const& applicationName /*= "worldserver"*/)
{
    // #1 - Load init config file .conf.dist
    if (!sRobotConfig->LoadInitial(_initConfigFile + ".dist"))
    {
    ...
    // #2 - Load .conf file
    if (!sRobotConfig->LoadMore(_initConfigFile))
    {
        sLog->outString();
...
bool RobotConfig::LoadModulesConfigs()
{
    ...
    for (auto const& itr : _modulesConfigFiles)
    {
        ...
        if (!sRobotConfig->LoadMore(ConfigFileDist.c_str()))
        {
        ...
        // Load .conf config
        if (!sRobotConfig->LoadMore(defaultConfig.c_str()))
