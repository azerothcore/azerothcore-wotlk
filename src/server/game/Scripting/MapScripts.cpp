...
{ 
    Player* player = NULL;
    if (!source && !target)
        LOG_ERROR("server", "%s source and target objects are NULL.", scriptInfo->GetDebugInfo().c_str());
    else
    {
...
{ 
    Unit* unit = NULL;
    if (!obj)
        LOG_ERROR("server", "%s %s object is NULL.", scriptInfo->GetDebugInfo().c_str(), isSource ? "source" : "target");
...
{ 
    ...
    if (!obj)
        LOG_ERROR("server", "%s %s object is NULL.",
            scriptInfo->GetDebugInfo().c_str(), isSource ? "source" : "target");
    else
    {
...
{ 
    ...
    if (!guid)
        ...
        LOG_ERROR("server", "%s source object is NULL.", scriptInfo->GetDebugInfo().c_str());
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= sWorld->GetGameTime()))
    {
        ...
        switch (step.script->command)
        {
            ...
            case SCRIPT_COMMAND_QUEST_EXPLORED:
            {
                if (!source)
                {
                    LOG_ERROR("server", "%s source object is NULL.", step.script->GetDebugInfo().c_str());
                    break;
                }
                if (!target)
                {
                    LOG_ERROR("server", "%s target object is NULL.", step.script->GetDebugInfo().c_str());
                    break;
                }
            ...
            case SCRIPT_COMMAND_CAST_SPELL:
            {
                ...
                if (!source && !target)
                {
                    LOG_ERROR("server", "%s source and target objects are NULL.", step.script->GetDebugInfo().c_str());
                    break;
                }
