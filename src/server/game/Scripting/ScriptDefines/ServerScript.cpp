...
void ScriptMgr::OnSocketOpen(std::shared_ptr<WowConnection> socket)
{
    ASSERT(socket);
...
void ScriptMgr::OnSocketClose(std::shared_ptr<WowConnection> socket)
{
    ASSERT(socket);
...
bool ScriptMgr::CanPacketSend(User* session, WDataStore const& packet)
{
    ASSERT(session);
