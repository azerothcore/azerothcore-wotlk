...
namespace
{
    ...
    inline void ForeachMaps(Map* map, std::function<void(WorldMapScript*)> executeHook)
    {
        ...
        if (ScriptRegistry<WorldMapScript>::Instance()->GetScripts().empty())
            return;

        for (auto const& [scriptID, script] : ScriptRegistry<WorldMapScript>::Instance()->GetScripts())
        {
            MapEntry const* mapEntry = script->GetEntry();
            if (!mapEntry || mapEntry->MapID != map->GetId())
                continue;
    ...
    inline void ForeachMaps(Map* map, std::function<void(InstanceMapScript*)> executeHook)
    {
        ...
        if (ScriptRegistry<InstanceMapScript>::Instance()->GetScripts().empty())
            return;

        for (auto const& [scriptID, script] : ScriptRegistry<InstanceMapScript>::Instance()->GetScripts())
        {
            MapEntry const* mapEntry = script->GetEntry();
            if (!mapEntry || mapEntry->MapID != map->GetId())
                continue;
    ...
    inline void ForeachMaps(Map* map, std::function<void(BattlegroundMapScript*)> executeHook)
    {
        ...
        if (ScriptRegistry<BattlegroundMapScript>::Instance()->GetScripts().empty())
            return;

        for (auto const& [scriptID, script] : ScriptRegistry<BattlegroundMapScript>::Instance()->GetScripts())
        {
            MapEntry const* mapEntry = script->GetEntry();
            if (!mapEntry || mapEntry->MapID != map->GetId())
                continue;
