...
void SystemMgr::LoadScriptWaypoints()
{
    ...
    LOG_INFO("server.loading", "ASCR Loading Script Waypoints for " UI64FMTD " creature(s)...", uiCreatureCount);
    ...
    do
    {
        ...
        if (!pCInfo)
        {
            LOG_ERROR("sql.sql", "ASCR DB table script_waypoint has waypoint for non-existant creature entry %u", temp.uiCreatureEntry);
            continue;
        }

        if (!pCInfo->ScriptID)
            LOG_ERROR("sql.sql", "ASCR DB table script_waypoint has waypoint for creature entry %u, but creature does not have ScriptName defined and then useless.", temp.uiCreatureEntry);
