...
bool _SpellScript::_Validate(SpellInfo const* entry)
{
    if (!Validate(entry))
    {
        LOG_ERROR("spells.scripts", "_SpellScript::_Validate: Spell `%u` did not pass Validate() function of script `%s` - script will not be added to the spell", entry->Id, m_scriptName->c_str());
        return false;
    }
    return true;
}
...
bool _SpellScript::_ValidateSpellInfo(uint32 const* begin, uint32 const* end)
{
    ...
    while (begin != end)
    {
        ...
        {
            LOG_ERROR("spells.scripts", "_SpellScript::_ValidateSpellInfo: Spell %u does not exist.", *begin);
            allValid = false;
        }
