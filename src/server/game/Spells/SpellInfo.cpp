...
SpellCastResult SpellInfo::CheckShapeshift(uint32 form) const
{
    ...
    if (form > 0)
    {
        ...
        if (!shapeInfo)
        {
            LOG_ERROR("root", "GetErrorAtShapeshiftedCast: unknown shapeshift %u", form);
            return SPELL_CAST_OK;
        }
...
int32 SpellInfo::CalcPowerCost(Unit const* caster, SpellSchoolMask schoolMask, Spell* spell) const
{
    ...
    if (AttributesEx & SPELL_ATTR1_DRAIN_ALL_POWER)
    {
        ...
        LOG_ERROR("root", "SpellInfo::CalcPowerCost: Unknown power type '%d' in spell %d", PowerType, Id);
        return 0;
    }
    ...
    if (ManaCostPercentage)
    {
        switch (PowerType)
        {
            ...
            case POWER_RUNIC_POWER:
                ...
                LOG_DEBUG("root", "CalculateManaCost: Not implemented yet!");
#endif
                break;
            default:
                LOG_ERROR("root", "CalculateManaCost: Unknown power type '%d' in spell %d", PowerType, Id);
                return 0;
        }
    }
