...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/)
{
    ...
    switch (m_spellInfo->Id)
    {
        case 70746: // Teleport Into Sunwell (for Battered Hilt)
            if (Player* target = unitTarget->ToPlayer())
            {
                ...
                target->Teleport(mapid, x, y, z, orientation, TELE_TO_GM_MODE); // skip PlayerCannotEnter check
            }
            return;
    }
    ...
    if (mapid == unitTarget->GetMapId())
    {
        ...
        unitTarget->ToPlayer()->Teleport(mapid, x, y, z, orientation, unitTarget == m_caster ? TELE_TO_SPELL : 0);
    else
    {
...
void Spell::EffectProficiency(SpellEffIndex /*effIndex*/)
{
    ...
    if (m_spellInfo->EquippedItemClass == ITEM_WEAPON && !(p_target->GetWeaponProficiency() & subClassMask))
    {
        p_target->AddWeaponProficiency(subClassMask);
        p_target->SendProficiency(ITEM_WEAPON, p_target->GetWeaponProficiency());
    }
    if (m_spellInfo->EquippedItemClass == ITEM_ARMOR && !(p_target->GetArmorProficiency() & subClassMask))
    {
        p_target->AddArmorProficiency(subClassMask);
        p_target->SendProficiency(ITEM_ARMOR, p_target->GetArmorProficiency());
    }
}
...
void Spell::EffectDuel(SpellEffIndex effIndex)
{
    ...
    if (caster->duel || target->duel || !target->GetSocial() || target->FriendListPtr()->IsIgnored(caster->GetGUID()))
        return;
