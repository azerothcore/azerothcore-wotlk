...
void Spell::EffectNULL(SpellEffIndex /*effIndex*/)
{
    ...
    LOG_DEBUG("root", "WORLD: Spell Effect DUMMY");
#endif
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "Spell ScriptStart spellid %u in EffectDummy(%u)", m_spellInfo->Id, effIndex);
#endif
...
void Spell::EffectTriggerSpell(SpellEffIndex effIndex)
{
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "Spell::EffectTriggerSpell spell %u tried to trigger unknown spell %u", m_spellInfo->Id, triggered_spell_id);
#endif
        return;
    }
...
void Spell::EffectTriggerMissileSpell(SpellEffIndex effIndex)
{
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "Spell::EffectTriggerMissileSpell spell %u tried to trigger unknown spell %u", m_spellInfo->Id, triggered_spell_id);
#endif
        return;
    }
...
void Spell::EffectForceCast(SpellEffIndex effIndex)
{
    ...
    if (!spellInfo)
    {
        LOG_ERROR("root", "Spell::EffectForceCast of spell %u: triggering unknown spell id %i", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectTriggerRitualOfSummoning(SpellEffIndex effIndex)
{
    ...
    if (!spellInfo)
    {
        LOG_ERROR("root", "EffectTriggerRitualOfSummoning of spell %u: triggering unknown spell id %i", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/)
{
    ...
    if (!m_targets.HasDst())
    {
        LOG_ERROR("root", "Spell::EffectTeleportUnits - does not have destination for spell ID %u\n", m_spellInfo->Id);
        return;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "Spell::EffectTeleportUnits - teleport unit to %u %f %f %f %f\n", mapid, x, y, z, orientation);
#endif
    ...
    {
        LOG_ERROR("root", "Spell::EffectTeleportUnits - spellId %u attempted to teleport creature to a different map.", m_spellInfo->Id);
        return;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "Spell: Player %u has unlearned spell %u from NpcGUID: %u", player->GetGUIDLow(), spellToUnlearn, m_caster->GetGUIDLow());
#endif
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "Spell ScriptStart %u for spellid %u in EffectSendEvent ", m_spellInfo->Effects[effIndex].MiscValue, m_spellInfo->Id);
#endif
...
void Spell::EffectHeal(SpellEffIndex /*effIndex*/)
{
    ...
    if (unitTarget && unitTarget->IsAlive() && damage >= 0)
    {
        ...
        else if (m_spellInfo->TargetAuraState == AURA_STATE_SWIFTMEND && unitTarget->HasAuraState(AURA_STATE_SWIFTMEND, m_spellInfo, m_caster))
        {
            ...
            if (!targetAura)
            {
                LOG_ERROR("root", "Target(GUID:" UI64FMTD ") has aurastate AURA_STATE_SWIFTMEND but no matching aura.", unitTarget->GetGUID());
                return;
            }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "HealthLeech :%i", damage);
#endif
...
void Spell::SendLoot(uint64 guid, LootType loottype)
{
    ...
    if (gameObjTarget)
    {
        ...
        {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            LOG_ERROR("root", "Possible hacking attempt: Player %s [guid: %u] tried to loot a gameobject [entry: %u id: %u] which is on respawn time without being in GM mode!", player->GetName().c_str(), player->GetGUIDLow(), gameObjTarget->GetEntry(), gameObjTarget->GetGUIDLow());
#endif
            return;
        }
...
void Spell::EffectOpenLock(SpellEffIndex effIndex)
{
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "WORLD: Open Lock - No Player Caster!");
#endif
        return;
    }
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "WORLD: Open Lock - No GameObject/Item Target!");
#endif
        return;
    }
...
void Spell::EffectSummonType(SpellEffIndex effIndex)
{
    ...
    if (!properties)
    {
        LOG_ERROR("root", "EffectSummonType: Unhandled summon type %u", m_spellInfo->Effects[effIndex].MiscValueB);
        return;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "Spell: Player %u has learned spell %u from NpcGUID=%u", player->GetGUIDLow(), spellToLearn, m_caster->GetGUIDLow());
#endif
}
...
void Spell::EffectTeleUnitsFaceCaster(SpellEffIndex  /*effIndex*/)
{
    ...
    if (!m_targets.HasDst())
    {
        LOG_ERROR("root", "Spell::EffectTeleUnitsFaceCaster - does not have destination for spell ID %u\n", m_spellInfo->Id);
        return;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "SpellEffect::AddHonor (spell_id %u) rewards %d honor points (item %u) for player: %u", m_spellInfo->Id, damage/10, m_CastItem->GetEntry(), unitTarget->ToPlayer()->GetGUIDLow());
#endif
        return;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "SpellEffect::AddHonor (spell_id %u) rewards %u honor points (scale) to player: %u", m_spellInfo->Id, honor_reward, unitTarget->ToPlayer()->GetGUIDLow());
#endif
    }
    else
    {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "SpellEffect::AddHonor (spell_id %u) rewards %u honor points (non scale) for player: %u", m_spellInfo->Id, damage, unitTarget->ToPlayer()->GetGUIDLow());
#endif
    }
}
...
void Spell::EffectEnchantItemPrismatic(SpellEffIndex effIndex)
{
    ...
    // support only enchantings with add socket in this slot
    {
        ...
        if (!add_socket)
        {
            LOG_ERROR("root", "Spell::EffectEnchantItemPrismatic: attempt apply enchant spell %u with SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC (%u) but without ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET (%u), not suppoted yet.",
...
void Spell::EffectEnchantItemTmp(SpellEffIndex effIndex)
{
    ...
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_SHAMAN && m_spellInfo->SpellFamilyFlags[0] & 0x400000)
    {
        ...
        switch (damage)
        {
            ...
            default:
                LOG_ERROR("root", "Spell::EffectEnchantItemTmp: Damage %u not handled in S'RW", damage);
                return;
        }
        ...
        if (!spellInfo)
        {
            LOG_ERROR("root", "Spell::EffectEnchantItemTmp: unknown spell id %i", spell_id);
            return;
    ...
    if (!enchant_id)
    {
        LOG_ERROR("root", "Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) have 0 as enchanting id", m_spellInfo->Id, effIndex);
        return;
    }
    ...
    if (!pEnchant)
    {
        LOG_ERROR("root", "Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) have not existed enchanting id %u ", m_spellInfo->Id, effIndex, enchant_id);
        return;
    }
...
void Spell::EffectScriptEffect(SpellEffIndex effIndex)
{
            ...
            switch (m_spellInfo->Id)
            {
                ...
                case 60123: // Lightwell
                {
                    ...
                    switch (m_caster->GetEntry())
                    {
                        ...
                        default:
                            LOG_ERROR("root", "Unknown Lightwell spell caster %u", m_caster->GetEntry());
                            return;
                    }
    ...
    // normal DB scripted effect
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "Spell ScriptStart spellid %u in EffectScriptEffect(%u)", m_spellInfo->Id, effIndex);
#endif
    m_caster->GetMap()->ScriptsStart(sSpellScripts, uint32(m_spellInfo->Id | (effIndex << 24)), m_caster, unitTarget);
}
...
void Spell::EffectTransmitted(SpellEffIndex effIndex)
{
    ...
    if (!goinfo)
    {
        LOG_ERROR("root", "Gameobject (Entry: %u) not exist and not created at spell (ID: %u) cast", name_id, m_spellInfo->Id);
        return;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "AddObject at SpellEfects.cpp EffectTransmitted");
#endif
    //m_caster->AddGameObject(pGameObj);
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "WORLD: SkillEFFECT");
#endif
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "Effect: SkinPlayerCorpse");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "Effect: StealBeneficialBuff");
#endif
...
void Spell::EffectPlayMusic(SpellEffIndex effIndex)
{
    ...
    {
        LOG_ERROR("root", "EffectPlayMusic: Sound (Id: %u) not exist in spell %u.", soundid, m_spellInfo->Id);
        return;
    }
...
void Spell::EffectPlaySound(SpellEffIndex effIndex)
{
    ...
    if (!sSoundEntriesStore.LookupEntry(soundId))
    {
        LOG_ERROR("root", "EffectPlayerSound: Sound (Id: %u) not exist in spell %u.", soundId, m_spellInfo->Id);
        return;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "EffectBind: New homebind X: %f, Y: %f, Z: %f, MapId: %u, AreaId: %u",
