...
{
    if (!sScriptMgr->OnBeforePlayerResurrect(target, 100, false))
    {
        LOG_DEBUG("entities.player", "Player::ResurrectPlayer: OnBeforePlayerResurrect returned false for player {} ({})", target->GetName(), target->GetGUID().ToString());
        return;
    }
