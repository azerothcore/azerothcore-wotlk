...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    ...
    if ((m_spellInfo->Speed || getState() == SPELL_STATE_DELAYED) &&
            ((m_damage > 0 && unit->IsImmunedToDamage(unitCaster, m_spellInfo)) || unit->IsImmunedToSpell(m_spellInfo, this)))
    {
...
void Spell::_cast(bool skipCheck)
{
    ...
    if (batchDelay)
    {
        ...
        // leave the caster's own entry at 0, as AddUnitTarget() does for a projectile; it still
        // resolves in the first delayed pass, together with the victims
        for (TargetInfo& targetInfo : m_UniqueTargetInfo)
            if (targetInfo.targetGUID != m_caster->GetGUID())
