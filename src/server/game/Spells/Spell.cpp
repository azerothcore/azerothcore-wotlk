...
void Spell::SendSpellGo()
{
    ...
    if ((m_caster->GetTypeId() == TYPEID_PLAYER)
            && (m_caster->IsClass(CLASS_DEATH_KNIGHT))
...
{
    if (m_caster->GetTypeId() != TYPEID_PLAYER || !m_caster->IsClass(CLASS_DEATH_KNIGHT))
        return;
            ...
            case SPELL_EFFECT_SUMMON_PET:
                {
                    ...
                    if (m_caster->GetTypeId() == TYPEID_PLAYER && m_caster->IsClass(CLASS_WARLOCK) && strict)
                        if (Pet* pet = m_caster->ToPlayer()->GetPet())
