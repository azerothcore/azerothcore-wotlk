...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    if (m_caster)
    {
        ...
        {
            m_caster->AtTargetAttacked(effectUnit, m_spellInfo->HasInitialAggro());
...
void Spell::_cast(bool skipCheck)
{
    ...
    if (Unit* target = m_targets.GetUnitTarget())
        if (m_originalCaster && GetDelayMoment() > 0 && !m_originalCaster->IsFriendlyTo(target) && !m_spellInfo->HasAura(SPELL_AURA_BIND_SIGHT) && (!m_spellInfo->IsPositive() || m_spellInfo->HasEffect(SPELL_EFFECT_DISPEL)))
            if (m_spellInfo->HasInitialAggro() || target->IsEngaged())
