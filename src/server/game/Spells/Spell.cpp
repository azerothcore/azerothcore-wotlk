        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_CHARGE:
            {
                ...
                if (MMAP::MMapFactory::IsPathfindingEnabled(m_caster->FindMap()) && m_spellInfo->NeedsExplicitUnitTarget())
                {
                    ...
                    float maxdist = MELEE_RANGE + m_caster->GetCombatReach() + target->GetCombatReach();
...
SpellCastResult Spell::CheckRange(bool strict)
{
    ...
    // xinef: dont check maxRange to strictly after cast
    if (!(m_spellInfo->RangeEntry->type & SPELL_RANGE_MELEE) && !strict)
    ...
    if (target && target != m_caster)
    {
        ...
        if (minRange && !(m_spellInfo->RangeEntry->type & SPELL_RANGE_RANGED) && m_caster->IsWithinCombatRange(target, minRange)) // skip this check if minRange = 0
