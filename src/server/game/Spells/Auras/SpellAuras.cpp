...
_flags(AFLAG_NONE), _effectsToApply(effMask), _needClientUpdate(false), _disableMask(0)
{
    ...
    if (GetBase()->CanBeSentToClient())
    {
        ...
        if (slot < MAX_AURAS)
        {
            ...
            LOG_DEBUG("root", "Aura: %u Effect: %d put to unit visible auras slot: %u", GetBase()->GetId(), GetEffectMask(), slot);
#endif
        }
        else {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            LOG_DEBUG("root", "Aura: %u Effect: %d could not find empty unit visible slot", GetBase()->GetId(), GetEffectMask());
#endif
        }
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "AuraApplication::_HandleEffect: %u, apply: %u: amount: %u", aurEff->GetAuraType(), apply, aurEff->GetAmount());
#endif
...
void Aura::_UnapplyForTarget(Unit* target, Unit* caster, AuraApplication * auraApp)
{
    ...
    {
        LOG_ERROR("root", "Aura::_UnapplyForTarget, target:%u, caster:%u, spell:%u was not found in owners application map!",
    ...
    for (std::map<Unit*, uint8>::iterator itr = targets.begin(); itr!= targets.end();)
    {
        ...
        else
        {
            ...
            if (!GetOwner()->IsSelfOrInSameMap(itr->first))
            {
                ...
                LOG_FATAL("root", "Aura %u: Owner %s (map %u) is not in the same map as target %s (map %u).", GetSpellInfo()->Id,
...
void Aura::UnregisterSingleTarget()
{
    ...
    if (!caster)
    {
        LOG_INFO("root", "Aura::UnregisterSingleTarget (A1) - %u, %u, %u, %s", GetId(), GetOwner()->GetTypeId(), GetOwner()->GetEntry(), GetOwner()->GetName().c_str());
        //ASSERT(caster);
    }
    else
                    ...
                    case 12043: // Presence of Mind
                        ...
                        if (AuraEffect const* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_MAGE, 2120, 0))
                        {
                            ...
                            switch (aurEff->GetId())
                            {
                                ...
                                default:
                                    LOG_ERROR("root", "Aura::HandleAuraSpecificMods: Unknown rank of Arcane Potency (%d) found", aurEff->GetId());
                            }
                            if (spellId)
                ...
                if (GetSpellInfo()->SpellFamilyFlags[2] & 0x2)
                {
                    ...
                    if (aurEff)
                    {
                        ...
                        switch (aurEff->GetId())
                        {
                            ...
                            default:
                                LOG_ERROR("root", "Aura::HandleAuraSpecificMods: Unknown rank of Crypt Fever/Ebon Plague (%d) found", aurEff->GetId());
                        }
                ...
                if (GetSpellInfo()->SpellFamilyFlags[1] & 0x00000400)
                {
                    if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_WARLOCK, 98, 0))
                    {
                        ...
                        switch (aurEff->GetId())
                        {
                            ...
                            default:
                                LOG_ERROR("root", "Aura::HandleAuraSpecificMods: Unknown rank of Improved Fear (%d) found", aurEff->GetId());
                        }
                        if (spellId)
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "Aura::LoadScripts: Script `%s` for aura `%u` is loaded now", (*itr)->_GetScriptName()->c_str(), m_spellInfo->Id);
#endif
        (*itr)->Register();
        ++itr;
    }
