...
void MoveSpline::init_spline(const MoveSplineInitArgs& args)
{
    ...
    {
        //LOG_ERROR("root", "MoveSpline::init_spline: zero length spline, wrong input data?"); // ZOMG! temp comment to avoid console spam from transports
...
#define CHECK(exp) \
    if (!(exp))\
    {\
        LOG_ERROR("root", "MoveSplineInitArgs::Validate: expression '%s' failed for GUID: %u Entry: %u", #exp, unit->GetTypeId() == TYPEID_PLAYER ? unit->GetGUIDLow() : unit->ToCreature()->GetDBTableGUIDLow(), unit->GetEntry());\
        return false;\
    }
    CHECK(path.size() > 1);
...
bool MoveSplineInitArgs::_checkPathBounds() const
{
    if (!(flags & MoveSplineFlag::Mask_CatmullRom) && path.size() > 2)
    {
        ...
        for (uint32 i = 1; i < path.size()-1; ++i)
        {
            ...
            {
                LOG_ERROR("root", "MoveSplineInitArgs::_checkPathBounds check failed");
                return false;
            }
        }
    }
