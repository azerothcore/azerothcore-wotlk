        ...
        else // ground
        {
            ...
            if (result && !(_pathGenerator->GetPathType() & PATHFIND_NOPATH))
            {
                ...
                for (; itrNext != finalPath.end(); ++itr, ++itrNext)
                {
                    ...
                    if (!map->isInLineOfSight((*itr).x, (*itr).y, (*itr).z + 2.f, (*itrNext).x, (*itrNext).y, (*itrNext).z + 2.f, creature->GetPhaseMask(), LINEOFSIGHT_ALL_CHECKS, VMAP::ModelIgnoreFlags::Nothing))
                    {
                        _validPointsVector[_currentPoint].erase(randomIter);
                        _preComputedPaths.erase(pathIdx);
                        return;
