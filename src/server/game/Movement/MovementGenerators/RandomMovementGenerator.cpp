...
void RandomMovementGenerator<Creature>::DoInitialize(Creature* creature)
{
    ...
    if (G3D::fuzzyEq(_initialPosition.GetExactDist2d(0.0f, 0.0f), 0.0f))
    {
        ...
        for (uint8 i = 0; i < RANDOM_POINTS_NUMBER; ++i)
        {
            ...
            _destinationPoints.push_back(G3D::Vector3(_initialPosition.GetPositionX() + _wanderDistance * std::cos(angle)*factor, _initialPosition.GetPositionY() + _wanderDistance * std::sin(angle)*factor, _initialPosition.GetPositionZ()));
        }
    }
