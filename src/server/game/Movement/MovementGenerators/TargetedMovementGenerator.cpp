...
void FollowMovementGenerator<Creature>::_updateSpeed(Creature* owner)
{
    ...
    if (!owner->GetOwnerGUID().IsPlayer() || !owner->IsInWorld() || !i_target.isValid() || i_target->GetGUID() != owner->GetOwnerGUID() || !(owner->GetOwner()->GetAuraEffectsByType(SPELL_AURA_MOD_DECREASE_SPEED).empty()))
        return;
