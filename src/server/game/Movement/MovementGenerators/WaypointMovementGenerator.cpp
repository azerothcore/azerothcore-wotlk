...
void WaypointMovementGenerator<Creature>::LoadPath(Creature* creature)
{
    ...
    if (!i_path)
    {
        ...
        sLog->outErrorDb("WaypointMovementGenerator::LoadPath: creature %s (%s) doesn't have waypoint path id: %u",
...
void WaypointMovementGenerator<Creature>::OnArrived(Creature* creature)
{
    ...
    if (i_path->at(i_currentNode)->event_id && urand(0, 99) < i_path->at(i_currentNode)->event_chance)
    {
        ...
        sLog->outDebug(LOG_FILTER_MAPSCRIPTS, "Creature movement start script %u at point %u for %s.",
...
bool FlightPathMovementGenerator::DoUpdate(Player* player, uint32 /*diff*/)
{
    ...
    if (pointId > i_currentNode)
    {
        ...
        do
        {
            if (i_currentNode >= i_path.size())
            {
                sLog->outMisc("TAXI NODE WAS GREATER THAN PATH SIZE, %s, POINTID: %u, NODESIZE: %lu, CURRENT: %u",
