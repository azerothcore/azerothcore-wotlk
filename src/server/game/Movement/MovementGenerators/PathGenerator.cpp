...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    // take ~80% of the original length
        ...
        if (!suffixPolyLength || dtStatusFailed(dtResult))
        {
            ...
            LOG_ERROR("server", "PathGenerator::BuildPolyPath: Path Build failed\n%lu", _source->GetGUID());
        }
    ...
    else
    {
        ...
        if (!_polyLength || dtStatusFailed(dtResult))
        {
            ...
            LOG_ERROR("server", "PathGenerator::BuildPolyPath: %lu Path Build failed: 0 length path", _source->GetGUID());
            BuildShortcut();
            _type = PATHFIND_NOPATH;
            return;
        }
...
void PathGenerator::BuildPointPath(const float* startPoint, const float* endPoint)
{
    ...
    if (_useRaycast)
    {
        ...
        LOG_ERROR("server", "PathGenerator::BuildPointPath() called with _useRaycast for unit %lu", _source->GetGUID());
        BuildShortcut();
        _type = PATHFIND_NOPATH;
        return;
    }
    ...
    *smoothPathSize = 0;
    ...
    while (npolys && nsmoothPath < maxSmoothPathSize)
    {
        ...
        if (dtStatusFailed(_navMeshQuery->getPolyHeight(polys[0], result, &result[1])))
            LOG_DEBUG("maps", "PathGenerator::FindSmoothPath: Cannot find height at position X: %f Y: %f Z: %f for %lu", result[2], result[0], result[1], _source->GetGUID());
        result[1] += 0.5f;
