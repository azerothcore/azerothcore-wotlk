    ...
    _navMeshQuery(NULL)
{
    ...
    //if (MMAP::MMapFactory::IsPathfindingEnabled(_sourceUnit->FindMap())) // pussywizard: checked before creating new PathGenerator
    {
        ...
        boost::shared_lock<boost::shared_mutex> lock(mmap->GetManagerLock());
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest)
{
    ...
    boost::shared_mutex& mmapLock = (base ? base->GetMMapLock() : MMAP::MMapFactory::createOrGetMMapManager()->GetMMapGeneralLock());
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos, boost::shared_mutex& mmapLock)
{
    bool endInWaterFar = false;
    bool cutToFirstHigher = false;

    {
    MutexReleaser<boost::shared_mutex> mutexReleaser(mmapLock);
