...
void MotionMaster::MoveTargetedHome(bool walk /*= false*/)
{
    ...
    if (_owner->IsUnit() && !_owner->ToCreature()->GetCharmerOrOwnerGUID())
    {
    ...
    else if (_owner->IsUnit() && _owner->ToCreature()->GetCharmerOrOwnerGUID())
    {
...
void MotionMaster::MoveFall(uint32 id /*=0*/, bool addFlagForNPC)
{
    ...
    else if (_owner->IsUnit() && addFlagForNPC) // pussywizard
    {
