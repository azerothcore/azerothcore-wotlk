...
public:
    ...
    void MoveCharge(float x, float y, float z, float speed = SPEED_CHARGE, uint32 id = EVENT_CHARGE, const Movement::PointsArray* path = nullptr, bool generatePath = false, float orientation = 0.0f, WOWGUID targetGUID = WOWGUID::Empty);
    void MoveCharge(PathGenerator const& path, float speed = SPEED_CHARGE, WOWGUID targetGUID = WOWGUID::Empty);
