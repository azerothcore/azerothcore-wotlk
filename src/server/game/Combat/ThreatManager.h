...
public:
    ...
    int32 GetTempModifierForTesting(Unit* target) const;
    float GetSchoolModifierForTesting(uint8 school) const;
#endif
    ...
    // TODO: Remove after QAston proc system merge — spell scripts should use
    // Trinity-style redirect handling (RegisterRedirectThreat/UnregisterRedirectThreat)
    // and store redirect targets on the aura script instead.
