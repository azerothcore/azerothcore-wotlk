...
int32 ThreatManager::GetTempModifierForTesting(Unit* target) const
{
    auto it = _threatenedByMe.find(target->GetGUID());
    if (it != _threatenedByMe.end())
        return it->second->_tempModifier;
    return 0;
}

float ThreatManager::GetSchoolModifierForTesting(uint8 school) const
{
    return _singleSchoolModifiers[school];
}
#endif
...
// TODO: Remove after QAston proc system merge
