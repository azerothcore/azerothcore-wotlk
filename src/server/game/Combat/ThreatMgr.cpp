...
HostileReference* ThreatContainer::selectNextVictim(Creature* attacker, HostileReference* currentVictim) const
{
    ...
    if (currentVictim)
    {
        ...
        if (!attacker->CanIgnoreFeignDeath() || !attacker->CanCreatureAttack(cvUnit)) // pussywizard: if currentVictim is not valid => don't compare the threat with it, just take the highest threat valid target
    ...
    for (ThreatContainer::StorageType::const_iterator iter = iThreatList.begin(); iter != iThreatList.end() && !found;)
    {
        ...
        if (attacker->CanIgnoreFeignDeath() && attacker->CanCreatureAttack(target))
        {
