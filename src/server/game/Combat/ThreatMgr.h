...
// Please check Game/Combat/ThreatMgr.h for documentation on how this class works!
class ThreatMgr
{
public:
    ...
    // identical to ThreatMgr::CanHaveThreatList(GetOwner())
    ...
    Acore::IteratorPair<ThreatListIterator> GetUnsortedThreatList() const { return { _myThreatListEntries.begin(), _myThreatListEntries.end() }; }
...
private:
    ...
    std::vector<std::pair<ObjectGuid, uint32>> _redirectInfo; // current redirection targets and percentages (updated from registry in ThreatMgr::UpdateRedirectInfo)
...
private:
    ...
    ThreatReference(ThreatMgr* mgr, Unit* victim, float amount) :
    ...
    OnlineState _online;
    float _baseAmount;
    int32 _tempModifier; // Temporary effects (auras with SPELL_AURA_MOD_TOTAL_THREAT) - set from victim's threatmanager in ThreatMgr::UpdateMyTempModifiers
    TauntState _taunted;
