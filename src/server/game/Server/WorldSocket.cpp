...
struct ServerPktHeader
{
    ...
    ServerPktHeader(uint32 size, uint16 cmd) : size(size)
    {
        ...
        if (isLargePacket())
        {
            LOG_DEBUG("server", "initializing large server to client packet. Size: %u, cmd: %u", size, cmd);
...
{
    if (!reason.empty())
        LOG_DEBUG("server", "Socket closed because of: %s", reason.c_str());
...
int WorldSocket::ProcessIncoming(WorldPacket* new_pct)
{
    ...
    catch (ByteBufferException &)
    {
        ...
        // if (sLog->IsOutDebug())
        // {
        //     LOG_DEBUG("server", "Dumping error causing packet:");
        //     new_pct->hexlike();
        // }
...
int WorldSocket::HandleAuthSession(WorldPacket& recvPacket)
{
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WorldSocket::HandleAuthSession: client %u, loginServerID %u, account %s, loginServerType %u, clientseed %u", BuiltNumberClient, loginServerID, account.c_str(), loginServerType, clientSeed);
#endif
    ...
    if (fields[3].GetUInt8() == 1) // if ip is locked
    {
        if (strcmp (fields[2].GetCString(), address.c_str()))
        {
            ...
            LOG_DEBUG("server", "WorldSocket::HandleAuthSession: Sent Auth Response (Account IP differs. Original IP: %s, new IP: %s).", fields[2].GetCString(), address.c_str());
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Allowed Level: %u Player Level %u", allowedAccountType, AccountTypes(security));
#endif
