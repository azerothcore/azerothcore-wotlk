...
public:
    ...
    static uint32 IncreaseSavingMaxValue(uint32 inc)            { std::lock_guard<std::mutex> lock(_savingLock); return (m_savingMaxValueAssigned += inc); }
    static void InsertToSavingSkipListIfNeeded(uint32 id)       { if (id > m_savingCurrentValue) { std::lock_guard<std::mutex> lock(_savingLock); m_savingSkipList.push_back(id); } }
