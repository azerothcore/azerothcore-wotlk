...
public:
    ...
    static uint32 IncreaseSavingMaxValue(uint32 inc)            { TRINITY_GUARD(std::mutex, _savingLock); return (m_savingMaxValueAssigned += inc); }
    static void InsertToSavingSkipListIfNeeded(uint32 id)       { if (id > m_savingCurrentValue) { TRINITY_GUARD(std::mutex, _savingLock); m_savingSkipList.push_back(id); } }
