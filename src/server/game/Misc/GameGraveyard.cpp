...
void Graveyard::LoadGraveyardFromDB()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("root", "Wrong UTF8 name for id %u in `game_graveyard` table, ignoring.", ID);
            continue;
        }
...
GraveyardStruct const* Graveyard::GetClosestGraveyard(float x, float y, float z, uint32 MapId, TeamId teamId)
{
    ...
    if (!zoneId)
    {
        if (z > -500)
        {
            LOG_ERROR("root", "ZoneId not found for map %u coords (%f, %f, %f)", MapId, x, y, z);
    ...
    {
        LOG_ERROR("root", "Table `graveyard_zone` incomplete: Zone %u Team %u does not have a linked graveyard.", zoneId, teamId);
        return GetDefaultGraveyard(teamId);
    }
    ...
    for (; range.first != range.second; ++range.first)
    {
        ...
        if (!entry)
        {
            LOG_ERROR("root", "Table `graveyard_zone` has record for not existing `game_graveyard` table %u, skipped.", data.safeLocId);
            continue;
        }
...
void Graveyard::LoadGraveyardZones()
{
    ...
    do
    {
        ...
        if (!entry)
        {
            LOG_ERROR("root", "Table `graveyard_zone` has a record for not existing `game_graveyard` table %u, skipped.", safeLocId);
            continue;
        }
        ...
        if (!areaEntry)
        {
            LOG_ERROR("root", "Table `graveyard_zone` has a record for not existing zone id (%u), skipped.", zoneId);
            continue;
        }

        if (areaEntry->zone != 0)
        {
            LOG_ERROR("root", "Table `graveyard_zone` has a record for subzone id (%u) instead of zone, skipped.", zoneId);
            continue;
        }

        if (team != 0 && team != HORDE && team != ALLIANCE)
        {
            LOG_ERROR("root", "Table `graveyard_zone` has a record for non player faction (%u), skipped.", team);
            continue;
        }

        if (!AddGraveyardLink(safeLocId, zoneId, teamId, false))
            LOG_ERROR("root", "Table `graveyard_zone` has a duplicate record for Graveyard (ID: %u) and Zone (ID: %u), skipped.", safeLocId, zoneId);
