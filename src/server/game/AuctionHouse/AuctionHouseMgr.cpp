...
void AuctionHouseMgr::LoadAuctionItems()
{
    ...
    if (!result)
    {
        LOG_WARN("server.loading", ">> Loaded 0 auction items. DB table `auction_house` or `item_instance` is empty!");
...
void AuctionHouseMgr::LoadAuctions()
{
    ...
    if (!result)
    {
        LOG_WARN("server.loading", ">> Loaded 0 auctions. DB table `auction_house` is empty.");
        LOG_INFO("server.loading", " ");
        return;
    }
