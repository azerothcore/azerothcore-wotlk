...
void AuctionHouseMgr::LoadAuctionItems()
{
    ...
    if (!result)
    {
        LOG_INFO("auctionHouse", ">> Loaded 0 auction items. DB table `auctionhouse` or `item_instance` is empty!");
    ...
    LOG_INFO("auctionHouse", ">> Loaded %u auction items in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", "");
}
...
void AuctionHouseMgr::LoadAuctions()
{
    ...
    if (!result)
    {
        LOG_INFO("auctionHouse", ">> Loaded 0 auctions. DB table `auctionhouse` is empty.");
        LOG_INFO("server.loading", "");
        return;
    }
    ...
    LOG_INFO("auctionHouse", ">> Loaded %u auctions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", "");
}
...
bool AuctionEntry::BuildAuctionInfo(WorldPacket& data) const
{
    ...
    if (!item)
    {
        LOG_ERROR("auctionHouse", "AuctionEntry::BuildAuctionInfo: Auction %u has a non-existent item: %u", Id, item_guidlow);
        return false;
    }
    data << uint32(Id);
...
bool AuctionEntry::LoadFromDB(Field* fields)
{
    ...
    if (!auctioneerData)
    {
        LOG_ERROR("auctionHouse", "Auction %u has not a existing auctioneer (GUID : %u)", Id, auctioneer);
        return false;
    }
    ...
    if (!auctioneerInfo)
    {
        LOG_ERROR("auctionHouse", "Auction %u has not a existing auctioneer (GUID : %u Entry: %u)", Id, auctioneer, auctioneerData->id);
        return false;
    }
    ...
    {
        LOG_ERROR("auctionHouse", "Auction %u has auctioneer (GUID : %u Entry: %u) with wrong faction %u", Id, auctioneer, auctioneerData->id, factionTemplateId);
        return false;
    }
    ...
    {
        LOG_ERROR("auctionHouse", "Auction %u has not a existing item : %u", Id, item_guidlow);
        return false;
    }
    return true;
}
...
bool AuctionEntry::LoadFromFieldList(Field* fields)
{
    ...
    if (!auctioneerData)
    {
        LOG_ERROR("auctionHouse", "AuctionEntry::LoadFromFieldList() - Auction %u has not a existing auctioneer (GUID : %u)", Id, auctioneer);
        return false;
    }
    ...
    if (!auctioneerInfo)
    {
        LOG_ERROR("auctionHouse", "AuctionEntry::LoadFromFieldList() - Auction %u has not a existing auctioneer (GUID : %u Entry: %u)", Id, auctioneer, auctioneerData->id);
        return false;
    }
    ...
    if (!auctionHouseEntry)
    {
        LOG_ERROR("auctionHouse", "AuctionEntry::LoadFromFieldList() - Auction %u has auctioneer (GUID : %u Entry: %u) with wrong faction %u", Id, auctioneer, auctioneerData->id, factionTemplateId);
        return false;
    }
