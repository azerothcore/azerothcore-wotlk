...
{
    {
        ...
        do
        {
            ...
            if (event_id == 0)
            {
                LOG_ERROR("root", "`game_event` game event entry 0 is reserved and can't be used.");
                continue;
            }
            ...
            {
                LOG_ERROR("root", "`game_event` game event id (%i) isn't a world event and has length = 0, thus it can't be used.", event_id);
                continue;
            }
            ...
            if (pGameEvent.holiday_id != HOLIDAY_NONE)
            {
                ...
                {
                    LOG_ERROR("root", "`game_event` game event id (%i) have not existed holiday id %u.", event_id, pGameEvent.holiday_id);
    ...
    LOG_INFO("root", "Loading Game Event Saves Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    LOG_ERROR("root", "`game_event_save` game event entry (%i) is out of range compared to max event entry in `game_event`", event_id);
                    continue;
                }
                ...
                {
                    LOG_ERROR("root", "game_event_save includes event save for non-worldevent id %u", event_id);
                    continue;
                }
    ...
    LOG_INFO("root", "Loading Game Event Prerequisite Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    LOG_ERROR("root", "`game_event_prerequisite` game event id (%i) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
                ...
                if (mGameEvent[event_id].state != GAMEEVENT_NORMAL && mGameEvent[event_id].state != GAMEEVENT_INTERNAL)
                {
                    ...
                    {
                        LOG_ERROR("root", "`game_event_prerequisite` game event prerequisite id (%i) is out of range compared to max event id in `game_event`", prerequisite_event);
                        continue;
                    }
                ...
                {
                    LOG_ERROR("root", "game_event_prerequisiste includes event entry for non-worldevent id %u", event_id);
                    continue;
                }
    ...
    LOG_INFO("root", "Loading Game Event Creature Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    LOG_ERROR("root", "`game_event_creature` game event id (%i) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
    ...
    LOG_INFO("root", "Loading Game Event GO Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    LOG_ERROR("root", "`game_event_gameobject` game event id (%i) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
    ...
    LOG_INFO("root", "Loading Game Event Model/Equipment Change Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    LOG_ERROR("root", "`game_event_model_equip` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
                ...
                if (newModelEquipSet.equipment_id > 0)
                {
                    ...
                    {
                        LOG_ERROR("root", "Table `game_event_model_equip` have creature (Guid: %u) with equipment_id %u not found in table `creature_equip_template`, set to no equipment.",
    ...
    LOG_INFO("root", "Loading Game Event Quest Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    LOG_ERROR("root", "`game_event_creature_quest` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
    ...
    LOG_INFO("root", "Loading Game Event GO Quest Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    LOG_ERROR("root", "`game_event_gameobject_quest` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
    ...
    LOG_INFO("root", "Loading Game Event Quest Condition Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    LOG_ERROR("root", "`game_event_quest_condition` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
    ...
    LOG_INFO("root", "Loading Game Event Condition Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    LOG_ERROR("root", "`game_event_condition` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
    ...
    LOG_INFO("root", "Loading Game Event Condition Save Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    LOG_ERROR("root", "`game_event_condition_save` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
                ...
                {
                    LOG_ERROR("root", "game_event_condition_save contains not present condition evt id %u cond id %u", event_id, condition);
                    continue;
                }
    ...
    LOG_INFO("root", "Loading Game Event NPCflag Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    LOG_ERROR("root", "`game_event_npcflag` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
    ...
    LOG_INFO("root", "Loading Game Event Seasonal Quest Relations...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
            ...
            do
            {
                ...
                if (!questTemplate)
                {
                    LOG_ERROR("root", "`game_event_seasonal_questrelation` quest id (%u) does not exist in `quest_template`", questId);
                    continue;
                }

                if (eventEntry >= mGameEvent.size())
                {
                    LOG_ERROR("root", "`game_event_seasonal_questrelation` event id (%u) is out of range compared to max event in `game_event`", eventEntry);
                    continue;
                }
    ...
    LOG_INFO("root", "Loading Game Event Vendor Additions Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    LOG_ERROR("root", "`game_event_npc_vendor` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
    ...
    LOG_INFO("root", "Loading Game Event Battleground Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    LOG_ERROR("root", "`game_event_battleground_holiday` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
    ...
    LOG_INFO("root", "Loading Game Event Pool Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    LOG_ERROR("root", "`game_event_pool` game event id (%i) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }

                if (!sPoolMgr->CheckPool(entry))
                {
                    LOG_ERROR("root", "Pool Id (%u) has all creatures or gameobjects with explicit chance sum <>100 and no equal chance defined. The pool system cannot pick one to spawn.", entry);
                    continue;
                }
...
void GameEventMgr::LoadHolidayDates()
{
    ...
    if (!result)
    {
        LOG_INFO("root", ">> Loaded 0 holiday dates. DB table `holiday_dates` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!entry)
        {
            LOG_ERROR("root", "holiday_dates entry has invalid holiday id %u.", holidayId);
            continue;
        }
        uint8 dateId = fields[1].GetUInt8();
        if (dateId >= MAX_HOLIDAY_DATES)
        {
            LOG_ERROR("root", "holiday_dates entry has out of range date_id %u.", dateId);
            continue;
        }
    ...
    LOG_INFO("root", ">> Loaded %u holiday dates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void GameEventMgr::StartArenaSeason()
{
    ...
    if (!result)
    {
        LOG_ERROR("root", "ArenaSeason (%u) must be an existant Arena Season", season);
        return;
    }
    ...
    {
        LOG_ERROR("root", "EventEntry %u for ArenaSeason (%u) does not exists", eventId, season);
        return;
    }
...
uint32 GameEventMgr::Update()                               // return the next event delay in ms
{
    ...
    for (uint16 itr = 1; itr < mGameEvent.size(); ++itr)
    {
        ...
        // so first queue it
        //LOG_ERROR("root", "Checking event %u", itr);
        if (CheckOneGameEvent(itr))
        {
            ...
            //LOG_DEBUG("root", "GameEvent %u is active", itr->first);
        ...
        {
            //LOG_DEBUG("root", "GameEvent %u is not active", itr->first);
            if (IsActiveEvent(itr))
            {
    ...
    LOG_DEBUG("root", "Next game event check in %u seconds.", nextEventDelay + 1);
#endif
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "GameEvent %u \"%s\" removed.", event_id, mGameEvent[event_id].description.c_str());
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "GameEvent %u \"%s\" started.", event_id, mGameEvent[event_id].description.c_str());
#endif
...
void GameEventMgr::GameEventSpawn(int16 event_id)
{
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventCreatureGuids.size()))
    {
        LOG_ERROR("root", "GameEventMgr::GameEventSpawn attempt access to out of range mGameEventCreatureGuids element %i (size: " SIZEFMTD ")",
    ...
    for (GuidList::iterator itr = mGameEventCreatureGuids[internal_event_id].begin(); itr != mGameEventCreatureGuids[internal_event_id].end(); ++itr)
    {
        ...
        if (CreatureData const* data = sObjectMgr->GetCreatureData(*itr))
        {
            ...
            if (!map->Instanceable() && map->IsGridLoaded(data->posX, data->posY))
            {
                ...
                //LOG_DEBUG("root", "Spawning creature %u", *itr);
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventGameobjectGuids.size()))
    {
        LOG_ERROR("root", "GameEventMgr::GameEventSpawn attempt access to out of range mGameEventGameobjectGuids element %i (size: " SIZEFMTD ")",
    ...
    for (GuidList::iterator itr = mGameEventGameobjectGuids[internal_event_id].begin(); itr != mGameEventGameobjectGuids[internal_event_id].end(); ++itr)
    {
        ...
        if (GameObjectData const* data = sObjectMgr->GetGOData(*itr))
        {
            ...
            if (!map->Instanceable() && map->IsGridLoaded(data->posX, data->posY))
            {
                ...
                //LOG_DEBUG("root", "Spawning gameobject %u", *itr);
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventPoolIds.size()))
    {
        LOG_ERROR("root", "GameEventMgr::GameEventSpawn attempt access to out of range mGameEventPoolIds element %u (size: " SIZEFMTD ")",
...
void GameEventMgr::GameEventUnspawn(int16 event_id)
{
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventCreatureGuids.size()))
    {
        LOG_ERROR("root", "GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventCreatureGuids element %i (size: " SIZEFMTD ")",
            internal_event_id, mGameEventCreatureGuids.size());
        return;
    }
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventGameobjectGuids.size()))
    {
        LOG_ERROR("root", "GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventGameobjectGuids element %i (size: " SIZEFMTD ")",
            internal_event_id, mGameEventGameobjectGuids.size());
        return;
    }
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventPoolIds.size()))
    {
        LOG_ERROR("root", "GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventPoolIds element %u (size: " SIZEFMTD ")", internal_event_id, mGameEventPoolIds.size());
        return;
    }
...
void GameEventMgr::SetHolidayEventTime(GameEventData& event)
{
    ...
    {
        LOG_ERROR("root", "Missing date or duration for holiday %u.", event.holiday_id);
        return;
    }
    ...
    LOG_INFO("root", "No suitable start date found for holiday %u.", event.holiday_id);
}
