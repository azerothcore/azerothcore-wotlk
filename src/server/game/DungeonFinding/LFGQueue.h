...
namespace lfg
{
    ...
    */
    ...
    public:
        ...
        auto GetJoinTime(ObjectGuid guid) -> time_t;

        // Find new group
        auto FindGroups() -> uint8;
    ...
    private:
        ...
        auto FindBestCompatibleInQueue(LfgQueueDataContainer::iterator itrQueue) -> uint32;
        void UpdateBestCompatibleInQueue(LfgQueueDataContainer::iterator itrQueue, Lfg5Guids const& key);

        auto FindNewGroups(const ObjectGuid& newGuid) -> LfgCompatibility;
        auto CheckCompatibility(Lfg5Guids const& checkWith, const ObjectGuid& newGuid, uint64& foundMask, uint32& foundCount, const std::set<Lfg5Guids>& currentCompatibles) -> LfgCompatibility;
