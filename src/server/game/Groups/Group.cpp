...
Group::~Group()
{
    if (m_bgGroup)
    {
        ...
        LOG_DEBUG("root", "Group::~Group: battleground group being deleted.");
#endif
        ...
        else LOG_ERROR("root", "Group::~Group: battleground group is not linked to the correct battleground.");
    }
    Rolls::iterator itr;
    while (!RollId.empty())
    {
...
void Group::GroupLoot(Loot* loot, WorldObject* pLootedObject)
{
    ...
    for (i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        if (!item)
        {
            //LOG_DEBUG("root", "Group::GroupLoot: missing item prototype for item with id: %d", i->itemid);
            continue;
        }
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "Group::MasterLoot (SMSG_LOOT_MASTER_LIST, 330)");
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            LOG_DEBUG("root", "-- Forced group value update for '%s'", pp->GetName().c_str());
#endif
        }
    }
}
