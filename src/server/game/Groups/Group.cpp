...
Group::~Group()
{
    if (m_bgGroup)
    {
        ...
        LOG_DEBUG("server", "Group::~Group: battleground group being deleted.");
#endif
...
void Group::GroupLoot(Loot* loot, WorldObject* pLootedObject)
{
    ...
    for (i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        if (!item)
        {
            //LOG_DEBUG("server", "Group::GroupLoot: missing item prototype for item with id: %d", i->itemid);
            continue;
        }
...
{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Group::MasterLoot (SMSG_LOOT_MASTER_LIST, 330)");
#endif
