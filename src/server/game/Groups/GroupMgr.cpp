...
uint32 GroupMgr::GenerateGroupId()
{
    ...
    {
        LOG_ERROR("root", "Group ID overflow!! Can't continue, shutting down server.");
...
{
    {
        ...
        if (!result)
        {
            LOG_INFO("root", ">> Loaded 0 group definitions. DB table `groups` is empty!");
            LOG_INFO("root", "\n");
        }
        else
        {
            ...
            LOG_INFO("root", ">> Loaded %u group definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("root", "\n");
        }
    }

    LOG_INFO("root", "Loading Group members...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            LOG_INFO("root", ">> Loaded 0 group members. DB table `group_member` is empty!");
            LOG_INFO("root", "\n");
        }
        else
        {
            ...
            do
            {
                ...
                //else
                //    LOG_ERROR("root", "GroupMgr::LoadGroups: Consistency failed, can't find group (storage id: %u)", fields[0].GetUInt32());

                ++count;
            }
            while (result->NextRow());

            LOG_INFO("root", ">> Loaded %u group members in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("root", "\n");
        }
    }
}
