...
void TicketMgr::LoadTickets()
{
    ...
    if (!result)
    {
        LOG_INFO("root", ">> Loaded 0 GM tickets. DB table `gm_ticket` is empty!");
    ...
    LOG_INFO("root", ">> Loaded %u GM tickets in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
void TicketMgr::LoadSurveys()
{
    ...
    LOG_INFO("root", ">> Loaded GM Survey count from database in %u ms", GetMSTimeDiffToNow(oldMSTime));
