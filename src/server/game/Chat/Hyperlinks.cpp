...
struct LinkValidator<LinkTags::item>
{
    static bool IsTextValid(ItemLinkData const& data, std::string_view text)
    {
        ...
        if (data.RandomProperty)
            randomSuffixes = reinterpret_cast<const std::array<const char*, 16>*>(&data.RandomProperty->nameSuffix);
        else if (data.RandomSuffix)
            randomSuffixes = reinterpret_cast<const std::array<const char*, 16>*>(&data.RandomSuffix->nameSuffix);
        ...
        for (uint8 i = 0; i < TOTAL_LOCALES; ++i)
        {
            ...
            std::string_view name = (i == DEFAULT_LOCALE) ? data.Item->Name1 : locale->Name[i];
            if (name.empty())
                continue;
            if (randomSuffixes)
            {
...
struct LinkValidator<LinkTags::quest>
{
    static bool IsTextValid(QuestLinkData const& data, std::string_view text)
    {
        ...
        for (uint8 i = 0; i < TOTAL_LOCALES; ++i)
        {
            ...
            std::string_view name = locale->Title[i];
