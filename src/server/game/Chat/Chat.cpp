    ...
    for (uint32 i = 0; i < table.size(); ++i)
    {
        ...
        if ((table[i].Handler)(this, table[i].Name[0] != '\0' ? text : oldtext))
        {
            ...
            if (!AccountMgr::IsPlayerAccount(m_session->GetSecurity()))
            {
                ...
                LOG_GM(m_session->GetAccountId(), "Command: %s [Player: %s (%ul) (Account: %u) X: %f Y: %f Z: %f Map: %u (%s) Area: %u (%s) Zone: %s Selected: %s (%ul)]",
...
bool ChatHandler::SetDataForCommandInTable(std::vector<ChatCommand>& table, char const* text, uint32 security, std::string const& help, std::string const& fullcommand)
{
    ...
    for (uint32 i = 0; i < table.size(); i++)
    {
        ...
        {
            LOG_ERROR("root", "Table `command` have unexpected subcommand '%s' in command '%s', skip.", text, fullcommand.c_str());
            return false;
        }

        //if (table[i].SecurityLevel != security)
        //    LOG_DEBUG("root", "Table `command` overwrite for command '%s' default security (%u) by %u", fullcommand.c_str(), table[i].SecurityLevel, security);
    ...
    if (!cmd.empty())
    {
        if (&table == &getCommandTable())
            LOG_ERROR("root", "Table `command` have non-existing command '%s', skip.", cmd.c_str());
        else
            LOG_ERROR("root", "Table `command` have non-existing subcommand '%s' in command '%s', skip.", cmd.c_str(), fullcommand.c_str());
    }
