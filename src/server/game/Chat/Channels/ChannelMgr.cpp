...
void ChannelMgr::LoadChannels()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 channels for %s", _teamId == TEAM_ALLIANCE ? "Alliance" : "Horde");
        return;
    }
    ...
    LOG_INFO("server.loading", ">> Loaded %u channels for %s in %ums", count, _teamId == TEAM_ALLIANCE ? "Alliance" : "Horde", GetMSTimeDiffToNow(oldMSTime));
