...
class Channel
{
    struct PlayerInfo
    {
        ...
        [[nodiscard]] auto HasFlag(uint8 flag) const -> bool { return flags & flag; }
        void SetFlag(uint8 flag) { if (!HasFlag(flag)) flags |= flag; }
        [[nodiscard]] auto IsOwner() const -> bool { return flags & MEMBER_FLAG_OWNER; }
        void SetOwner(bool state)
        {
            if (state) flags |= MEMBER_FLAG_OWNER;
            else flags &= ~MEMBER_FLAG_OWNER;
        }
        [[nodiscard]] auto IsOwnerGM() const -> bool { return _gmStatus; }
        void SetOwnerGM(bool on) { _gmStatus = on; }
        [[nodiscard]] auto IsModerator() const -> bool { return flags & MEMBER_FLAG_MODERATOR; }
        void SetModerator(bool state)
        {
            if (state) flags |= MEMBER_FLAG_MODERATOR;
            else flags &= ~MEMBER_FLAG_MODERATOR;
        }
        [[nodiscard]] auto IsMuted() const -> bool { return flags & MEMBER_FLAG_MUTED; }
        void SetMuted(bool state)
        {
            if (state) flags |= MEMBER_FLAG_MUTED;
            else flags &= ~MEMBER_FLAG_MUTED;
        }
        auto IsAllowedToSpeak(uint64 speakDelay) -> bool // pussywizard
        {
...
public:
    ...
    [[nodiscard]] auto GetName() const -> std::string const& { return _name; }
    [[nodiscard]] auto GetChannelId() const -> uint32 { return _channelId; }
    [[nodiscard]] auto IsConstant() const -> bool { return _channelId != 0; }
    [[nodiscard]] auto IsAnnounce() const -> bool { return _announce; }
    [[nodiscard]] auto IsLFG() const -> bool { return GetFlags() & CHANNEL_FLAG_LFG; }
    [[nodiscard]] auto GetPassword() const -> std::string const& { return _password; }
    void SetPassword(std::string const& npassword) { _password = npassword; }
    [[nodiscard]] auto GetNumPlayers() const -> uint32 { return playersStore.size(); }
    [[nodiscard]] auto GetFlags() const -> uint8 { return _flags; }
    [[nodiscard]] auto HasFlag(uint8 flag) const -> bool { return _flags & flag; }
...
private:
    ...
    [[nodiscard]] auto IsOn(ObjectGuid who) const -> bool { return playersStore.find(who) != playersStore.end(); }
    [[nodiscard]] auto IsBanned(ObjectGuid guid) const -> bool;
    ...
    [[nodiscard]] auto GetPlayerFlags(ObjectGuid guid) const -> uint8
    {
