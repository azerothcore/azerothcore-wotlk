...
class Channel
{
    ...
    public:
        Channel(uint32 channelId, TeamId teamId = 0, AreaTableEntry const* zoneEntry = nullptr);  // built-in channel ctor
        Channel(std::string const& name, TeamId teamId, std::string const& banList = "");         // custom player channel ctor
        ...
        void Say(uint64 guid, std::string const& what, uint32 lang) const;
        void Invite(Player const* player, std::string const& newp);
        void Voice(uint64 guid1, uint64 guid2) const;
        void DeVoice(uint64 guid1, uint64 guid2) const;
        void JoinNotify(uint64 guid) const;                                       // invisible notify
        void LeaveNotify(uint64 guid) const;                                      // invisible notify
        void SetOwnership(bool ownership) { _ownershipEnabled = ownership; }

    private:

        template<class Builder>
        void SendToAll(Builder&, uint64 guid = uint64::Empty) const;

        template<class Builder>
        void SendToAllButOne(Builder& builder, uint64 who) const;

        template<class Builder>
        void SendToOne(Builder& builder, uint64 who) const;

        bool IsOn(uint64 who) const { return _playersStore.find(who) != _playersStore.end(); }
        bool IsBanned(uint64 guid) const { return _bannedStore.find(guid) != _bannedStore.end(); }
