...
Creature::Creature(bool isWorldObject): Unit(isWorldObject), MovableMapObject(), m_groupLootTimer(0), lootingGroupLowGUID(0), m_PlayerDamageReq(0), m_lootRecipientGroup(0),
    ...
    m_homePosition(), m_transportHomePosition(), m_creatureInfo(nullptr), m_creatureData(nullptr), m_detectionDistance(20.0f), m_waypointID(0), m_path_id(0), m_formation(nullptr), m_respawnCompatibilityMode(false), _lastDamagedTime(nullptr), m_cannotReachTarget(false), m_cannotReachTimer(0),
...
void Creature::Update(uint32 diff)
{
    ...
    switch (m_deathState)
    {
        ...
        case JUST_DIED:
            ...
            LOG_ERROR("entities.unit", "Creature ({}) in wrong state: JUST_DIED (1)", GetGUID().ToString());
            break;
        case DEAD:
            {
                if (!m_respawnCompatibilityMode)
...
bool Creature::Create(ObjectGuid::LowType guidlow, Map* map, uint32 phaseMask, uint32 Entry, uint32 vehId, Position const& pos, const CreatureData* data)
{
    ASSERT(map);
    SetMap(map);
    ...
    if (!IsPositionValid())
    {
        LOG_ERROR("entities.unit", "Creature::Create(): given coordinates for creature (guidlow {}, entry {}) are not valid (X: {}, Y: {}, Z: {}, O: {})", guidlow, Entry, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), pos.GetOrientation());
        return false;
    }
...
bool Creature::LoadFromDB(ObjectGuid::LowType spawnId, Map* map, bool addToMap, bool gridLoad, bool allowDuplicate /*= false*/)
{
    if (!allowDuplicate)
    {
...
void Creature::DeleteFromDB()
{
    ...
    stmt->SetData(0, uint8(SPAWN_TYPE_CREATURE));
    stmt->SetData(1, m_spawnId);
    trans->Append(stmt);
...
void Creature::SaveRespawnTime(uint32 forceDelay)
{
    ...
    time_t thisRespawnTime = forceDelay ? GameTime::GetGameTime().count() + forceDelay : m_respawnTime;
    GetMap()->SaveRespawnTime(SPAWN_TYPE_CREATURE, m_spawnId, GetEntry(), thisRespawnTime, Acore::ComputeGridCoord(GetHomePosition().GetPositionX(), GetHomePosition().GetPositionY()).GetId());
}
