...
void Player::Update(uint32 p_time)
{
  ...
  // Xinef: update charm AI only if we are controlled by creature or non-posses
  // player charm
  ...
  for(uint32 i = 0; i < sChatChannelsStore.GetNumRows(); ++i)
  {
    if(ChatChannelsEntry const *channel = sChatChannelsStore.LookupEntry(i))
    {
      ...
      if(CanJoinConstantChannelInZone(channel, current_zone))
      {
        if(!(channel->flags & CHANNEL_DBC_FLAG_GLOBAL))
        {
          ...
          if(channel->flags & CHANNEL_DBC_FLAG_CITY_ONLY)
            currentNameExt =
                sObjectMgr->GetAcoreStringForDBCLocale(LANG_CHANNEL_CITY);
          else
      ...
      if(removeChannel)
      {
        ...
        std::string name =
            removeChannel->GetName(); // Store name, (*i)erase in LeftChannel
...
void Player::UpdateRating(CombatRating cr)
{
  ...
  case CR_CRIT_TAKEN_RANGED:
    break;
  case CR_CRIT_TAKEN_SPELL: // Implemented in Unit::SpellCriticalBonus (only for
                            // chance to crit)
    break;
...
{
  for(int cr = 0; cr < MAX_COMBAT_RATING; ++cr) UpdateRating(CombatRating(cr));
}
...
void Player::UpdateWeaponSkill(Unit *victim, WeaponAttackType attType)
{
  ...
  if(!tmpitem && attType == BASE_ATTACK)
  {
    ...
    UpdateSkill(SKILL_FIST_WEAPONS, weapon_skill_gain);
          ...
          tmpitem->GetTemplate()->SubClass != ITEM_SUBCLASS_WEAPON_FISHING_POLE)
  {
...
void Player::UpdateSkillsForLevel()
{
      ...
      itr != mSkillStatus.end(); ++itr)
  {
    ...
    if(max != 1)
    {
      ...
      else if(max != maxconfskill) /// update max skill value if current max
                                   /// skill not maximized
      {
...
void Player::UpdateHomebindTime(uint32 time)
{
  ...
  else
  {
    ...
    LOG_DEBUG(
        "maps",
        "PLAYER: Player '%s' (%s) will be teleported to homebind in 60 seconds",
...
void Player::UpdateFFAPvPState(bool reset /*= true*/)
{
  ...
  else if(IsFFAPvP())
  {
    ...
    else
    {
      ...
      {
        pvpInfo.FFAPvPEndTimer =
            sWorld->GetGameTime() + sWorld->getIntConfig(CONFIG_FFA_PVP_TIMER);
      }
    }
  }
}
...
void Player::UpdatePotionCooldown(Spell *spell)
{
  ...
  if(!spell)
  {
    // spell/item pair let set proper cooldown (except not existed charged spell
    // cooldown spellmods for potions)
...
void Player::UpdateCinematicLocation(uint32 /*diff*/)
{
  ...
  for(FlyByCamera cam : *m_cinematicCamera)
  {
    ...
    {
      nextPosition = Position(cam.locations.x, cam.locations.y, cam.locations.z,
                              cam.locations.w);
      nextTimestamp = cam.timeStamp;
      break;
    }
    lastPosition  = Position(cam.locations.x, cam.locations.y, cam.locations.z,
                            cam.locations.w);
  ...
  // Get an iterator to the last entry in the cameras, to make sure we don't go
  // beyond the end
  ...
  // Interpolate the position for this moment in time (or the adjusted moment in
  // time)
...
void Player::UpdateForQuestWorldObjects()
{
      ...
      itr != m_clientGUIDs.end(); ++itr)
  {
    ...
    else if((*itr).IsCreatureOrVehicle())
    {
      ...
      {
        //! This code doesn't look right, but it was logically converted to
        //! condition system to do the exact same thing it did before. It
        //! definitely needs to be overlooked for intended functionality.
...
void Player::UpdateAreaDependentAuras(uint32 newArea)
{
      ...
      iter != m_ownedAuras.end();)
  {
    ...
    if(iter->second->GetSpellInfo()->CheckLocation(
           GetMapId(), m_zoneUpdateId, newArea, this, false) != SPELL_CAST_OK)
      ...
      if(controlled && !controlled->IsPet())
      {
            ...
            auraIter != tAuras.end();)
        {
          if(auraIter->second->GetSpellInfo()->CheckLocation(
                 GetMapId(), m_zoneUpdateId, newArea, nullptr) != SPELL_CAST_OK)
...
void Player::UpdateUnderwaterState(Map *m, float x, float y, float z)
{
  ...
  ZLiquidStatus res = m->getLiquidStatus(x, y, z, MAP_ALL_LIQUIDS,
                                         &liquid_status, GetCollisionHeight());
  if(!res)
  {
  ...
  if((liquid_status.type_flags & MAP_LIQUID_TYPE_DARK_WATER) && !IsInFlight() &&
     !GetTransport())
...
void Player::UpdateCharmedAI()
{
     ...
     charmer->ToCreature()->IsInEvadeMode())
  {
    ...
    for(AuraEffectList::const_iterator iter = auras.begin();
      ...
      {
        Unit::DealDamage(charmer, this, GetHealth(), nullptr, DIRECT_DAMAGE,
  ...
  if(charmer->GetTypeId() == TYPEID_PLAYER)
  {
    ...
    else if(target && GetMotionMaster()->GetCurrentMovementGeneratorType() !=
                          CHASE_MOTION_TYPE)
        ...
        if(urand(0, 1))
        {
          if(m_charmAISpells[SPELL_T_STUN] &&
            ...
            CastSpell(target, m_charmAISpells[SPELL_T_STUN], false);
                  ...
                  !HasSpellCooldown(m_charmAISpells[SPELL_ROOT_OR_FEAR]))
        ...
        else
        {
          ...
          case 0:
            if(m_charmAISpells[SPELL_INSTANT_DAMAGE + rnd] &&
               !HasSpellCooldown(m_charmAISpells[SPELL_INSTANT_DAMAGE + rnd]))
              CastSpell(target, m_charmAISpells[SPELL_INSTANT_DAMAGE + rnd],
                        false);
            break;
          case 1:
      ...
      else
      {
        ...
        case 0:
          if(m_charmAISpells[SPELL_HIGH_DAMAGE1 + rnd] &&
             !HasSpellCooldown(m_charmAISpells[SPELL_HIGH_DAMAGE1 + rnd]))
