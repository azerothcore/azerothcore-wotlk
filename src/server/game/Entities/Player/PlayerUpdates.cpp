...
{
    kickScheduler.Schedule(delayTime, [this, kickFrom, reasonStr](TaskContext /*context*/)
        {
...
{
    kickScheduler.Schedule(delayTime, [this, duration, author, accountName, reasonStr](TaskContext /*context*/)
        {
            if (sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD))
            {
                bool IsPermanetly = true;

                if (TimeStringToSecs(duration.c_str()) > 0)
                    IsPermanetly = false;

                if (!IsPermanetly)
                    sWorld->SendWorldText(LANG_BAN_ACCOUNT_YOUBANNEDMESSAGE_WORLD, author.c_str(), accountName.c_str(), secsToTimeString(TimeStringToSecs(duration.c_str()), true).c_str(), reasonStr.c_str());
                else
...
{
    kickScheduler.Schedule(delayTime, [this, duration, author, ip, reasonStr](TaskContext /*context*/)
        {
            if (sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD))
            {
                bool IsPermanetly = true;
                ...
                if (IsPermanetly)
                    ...
                    sWorld->SendWorldText(LANG_BAN_IP_YOUBANNEDMESSAGE_WORLD, author.c_str(), ip.c_str(), secsToTimeString(TimeStringToSecs(duration.c_str()), true).c_str(), reasonStr.c_str());
            }
