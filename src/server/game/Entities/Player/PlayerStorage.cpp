...
InventoryResult Player::CanTakeMoreSimilarItems(uint32 entry, uint32 count, Item* item, uint32* no_space_count /*= nullptr*/, uint32* itemLimitCategory /*= nullptr*/) const
{
    ...
    if (pProto->ItemLimitCategory)
    {
        ...
        if (limitEntry->mode == ITEM_LIMIT_CATEGORY_MODE_HAVE)
        {
            ...
            if (curcount + count > uint32(limitEntry->maxCount))
            {
                ...
                if (itemLimitCategory) // TODO: Update this to return correct value
