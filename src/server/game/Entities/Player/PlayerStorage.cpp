...
BAG_RESULT Player::CanUnequipItems(uint32 item, uint32 count) const
{
    uint32 tempcount = 0;

    BAG_RESULT res = BAG_OK;

    for (uint8 i = EQUIPMENT_SLOT_START; i < INVENTORY_SLOT_BAG_END; ++i)
        if (Item* pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, i))
            if (pItem->GetEntry() == item)
            {
                BAG_RESULT ires = CanUnequipItem(INVENTORY_SLOT_BAG_0 << 8 | i, false);
                if (ires == BAG_OK)
                {
...
Item* Player::GetWeaponForAttack(WeaponAttackType attackType, bool useable /*= false*/) const
{
    ...
    if (!item || item->GetTemplate()->Class != ITEM_WEAPON)
        return nullptr;
...
Item* Player::GetShield(bool useable) const
{
    ...
    if (!item || item->GetTemplate()->Class != ITEM_ARMOR)
        return nullptr;
...
BAG_RESULT Player::CanTakeMoreSimilarItems(uint32 entry, uint32 count, Item* pItem, uint32* no_space_count) const
{
...
BAG_RESULT Player::CanStoreItem_InSpecificSlot(uint8 bag, uint8 slot, ItemPosCountVec& dest, ItemTemplate const* pProto, uint32& count, bool swap, Item* pSrcItem) const
{
    ...
    else
    {
        ...
        BAG_RESULT res  = pItem2->CanBeMergedPartlyWith(pProto);
        if (res != BAG_OK)
            return res;
...
BAG_RESULT Player::CanStoreItem_InBag(uint8 bag, ItemPosCountVec& dest, ItemTemplate const* pProto, uint32& count, bool merge, bool non_specialized, Item* pSrcItem, uint8 skip_bag, uint8 skip_slot) const
{
    ...
    if (non_specialized != (pBagProto->Class == ITEM_CONTAINER && pBagProto->SubClass == ITEM_SUBCLASS_CONTAINER))
        return EQUIP_ERR_ITEM_DOESNT_GO_INTO_BAG;
...
BAG_RESULT Player::CanStoreItem_InInventorySlots(uint8 slot_begin, uint8 slot_end, ItemPosCountVec& dest, ItemTemplate const* pProto, uint32& count, bool merge, Item* pSrcItem, uint8 skip_bag, uint8 skip_slot) const
{
...
BAG_RESULT Player::CanStoreItem(uint8 bag, uint8 slot, ItemPosCountVec& dest, uint32 entry, uint32 count, Item* pItem, bool swap, uint32* no_space_count) const
{
    ...
    BAG_RESULT res = CanTakeMoreSimilarItems(entry, count, pItem, &no_similar_count);
    if (res != BAG_OK)
    {
...
//////////////////////////////////////////////////////////////////////////
BAG_RESULT Player::CanStoreItems(Item** pItems, int count) const
{
    Item* pItem2;
    ...
    for (int k = 0; k < count; ++k)
    {
        ...
        BAG_RESULT res = CanTakeMoreSimilarItems(pItem);
        if (res != BAG_OK)
            return res;
        ...
        if (pProto->Stackable != 1)
        {
            ...
            for (uint8 t = KEYRING_SLOT_START; t < KEYRING_SLOT_END; ++t)
            {
                ...
                if (pItem2 && pItem2->CanBeMergedPartlyWith(pProto) == BAG_OK && inv_keys[t - KEYRING_SLOT_START] + pItem->GetCount() <= pProto->GetMaxStackSize())
                {
            ...
            {
                pItem2 = GetItemByPos(INVENTORY_SLOT_BAG_0, t);
                if (pItem2 && pItem2->CanBeMergedPartlyWith(pProto) == BAG_OK && inv_tokens[t - CURRENCYTOKEN_SLOT_START] + pItem->GetCount() <= pProto->GetMaxStackSize())
                {
            ...
            {
                pItem2 = GetItemByPos(INVENTORY_SLOT_BAG_0, t);
                if (pItem2 && pItem2->CanBeMergedPartlyWith(pProto) == BAG_OK && inv_slot_items[t - INVENTORY_SLOT_ITEM_START] + pItem->GetCount() <= pProto->GetMaxStackSize())
                {
                ...
                if (CGBag* bag = GetBagByPos(t))
                {
                    if (ItemCanGoIntoBag(pItem->GetTemplate(), bag->GetTemplate()))
                    {
                        for (uint32 j = 0; j < bag->GetBagSize(); j++)
                        {
                            ...
                            if (pItem2 && pItem2->CanBeMergedPartlyWith(pProto) == BAG_OK && inv_bags[t - INVENTORY_SLOT_BAG_START][j] + pItem->GetCount() <= pProto->GetMaxStackSize())
                            {
        ...
        if (pProto->BagFamily)
        {
            ...
            for (int t = INVENTORY_SLOT_BAG_START; !b_found && t < INVENTORY_SLOT_BAG_END; ++t)
            {
                if (CGBag* bag = GetBagByPos(t))
                {
                    ...
                    if (pBagProto && (pBagProto->Class != ITEM_CONTAINER || pBagProto->SubClass != ITEM_SUBCLASS_CONTAINER) &&
...
//////////////////////////////////////////////////////////////////////////
BAG_RESULT Player::CanEquipNewItem(uint8 slot, uint16& dest, uint32 item, bool swap) const
{
    dest = 0;
    Item* pItem = Item::CreateItem(item, 1, this);
    if (pItem)
    {
        BAG_RESULT result = CanEquipItem(slot, dest, pItem, swap);
        delete pItem;
        return result;
    }
...
BAG_RESULT Player::CanEquipItem(uint8 slot, uint16& dest, Item* pItem, bool swap, bool not_loading) const
{
    dest = 0;
    if (pItem)
    {
        ...
        if (pProto)
        {
            ...
            if (not_loading)
            {
                ...
                if (IsInCombat() && (pProto->Class == ITEM_WEAPON || pProto->InventoryType == INVTYPE_RELIC) && m_weaponChangeTimer != 0)
            ...
            BAG_RESULT res2 = CanEquipUniqueItem(pItem, swap ? ignore : uint8(NULL_SLOT));
            if (res2 != BAG_OK)
                return res2;
            ...
            if (type == INVTYPE_2HWEAPON)
            {
                ...
                if (!CanTitanGrip())
                {
                    ...
                    if (offItem && (!not_loading ||
                                    CanUnequipItem(uint16(INVENTORY_SLOT_BAG_0) << 8 | EQUIPMENT_SLOT_OFFHAND, false) != BAG_OK ||
                                    CanStoreItem(NULL_BAG, NULL_SLOT, off_dest, offItem, false) != BAG_OK))
...
BAG_RESULT Player::CanUnequipItem(uint16 pos, bool swap) const
{
...
BAG_RESULT Player::CanBankItem(uint8 bag, uint8 slot, ItemPosCountVec& dest, Item* pItem, bool swap, bool not_loading) const
{
    if (!pItem)
        return swap ? EQUIP_ERR_ITEMS_CANT_BE_SWAPPED : EQUIP_ERR_ITEM_NOT_FOUND;
...
BAG_RESULT Player::CanUseItem(Item* pItem, bool not_loading) const
{
    if (pItem)
    {
        ...
        if (pProto)
        {
            ...
            BAG_RESULT res = CanUseItem(pProto);
            if (res != BAG_OK)
                return res;
            ...
            if (pItem->GetSkill() != 0)
            {
                ...
                if (pProto->Quality == ITEM_QUALITY_HEIRLOOM && pProto->Class == ITEM_ARMOR && !HasSkill(itemSkill))
                {
...
BAG_RESULT Player::CanUseItem(ItemTemplate const* proto) const
{
...
BAG_RESULT Player::CanRollForItemInLFG(ItemTemplate const* proto, WorldObject const* lootedObject) const
{
    if (!GetGroup() || !GetGroup()->isLFGGroup(true))
        return BAG_OK;    // not in LFG group
    ...
    if (proto->Class == ITEM_WEAPON && GetSkillValue(item_weapon_skills[proto->SubClass]) == 0)
        return EQUIP_ERR_NO_REQUIRED_PROFICIENCY;
    ...
    if (proto->Class == ITEM_ARMOR && proto->SubClass > ITEM_SUBCLASS_ARMOR_MISC && proto->SubClass < ITEM_SUBCLASS_ARMOR_BUCKLER &&
...
BAG_RESULT Player::CanUseAmmo(uint32 item) const
{
...
Item* Player::EquipItem(uint16 pos, Item* pItem, bool update)
{
    ...
    if (!pItem2)
    {
        ...
        if (IsAlive())
        {
            ...
            if (pProto && IsInCombat() && (pProto->Class == ITEM_WEAPON || pProto->InventoryType == INVTYPE_RELIC) && m_weaponChangeTimer == 0)
            {
    ...
    for (uint8 i = EQUIPMENT_SLOT_START; i < INVENTORY_SLOT_BAG_END; i++)
    {
        if (Item* item = GetItemByPos(INVENTORY_SLOT_BAG_0, i))
        {
            if (item && item->GetEntry() == itemEntry && !item->IsInTrade())
            {
                if (item->GetCount() + remcount <= count)
                {
                    if (!unequip_check || CanUnequipItem(INVENTORY_SLOT_BAG_0 << 8 | i, false) == BAG_OK)
                    {
                        remcount += item->GetCount();
                        DestroyItem(INVENTORY_SLOT_BAG_0, i, update);
...
void Player::SplitItem(uint16 src, uint16 dst, uint32 count)
{
    ...
    if (IsInventoryPos(dst))
    {
        ...
        BAG_RESULT msg = CanStoreItem(dstbag, dstslot, dest, pNewItem, false);
        if (msg != BAG_OK)
        {
            delete pNewItem;
            pSrcItem->SetCount(pSrcItem->GetCount() + count);
    ...
    else if (IsBankPos(dst))
    {
        ...
        ItemPosCountVec dest;
        BAG_RESULT msg = CanBankItem(dstbag, dstslot, dest, pNewItem, false);
        if (msg != BAG_OK)
        {
            delete pNewItem;
            pSrcItem->SetCount(pSrcItem->GetCount() + count);
    ...
    else if (IsEquipmentPos(dst))
    {
        ...
        uint16 dest;
        BAG_RESULT msg = CanEquipItem(dstslot, dest, pNewItem, false);
        if (msg != BAG_OK)
        {
            delete pNewItem;
            pSrcItem->SetCount(pSrcItem->GetCount() + count);
...
bool Player::LoadFromDB(WOWGUID playerGuid, CharacterDatabaseQueryHolder const& holder)
{
    ...
    // 70          71               72            73                     74                             75
    //"actionBars, grantableLevels, innTriggerId, extraBonusTalentCount, UNIX_TIMESTAMP(creation_date), securitygroup FROM characters WHERE guid = '{}'", guid);
    ...
    m_securityGroup = static_cast<CHAR_SECURITY_GROUP>(fields[75].Get<uint32_t>());
    return true;
}
