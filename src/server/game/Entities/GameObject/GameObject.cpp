...
{ 
    ...
    LOG_FATAL("root", "Delete GameObject (GUID: %u Entry: %u SpellId %u LinkedGO %u) that lost references to owner (GUID %u Type '%s') GO list. Crash possible later.",
...
void GameObject::Update(uint32 diff)
{
    ...
    if (AI())
        ...
        LOG_ERROR("root", "Could not initialize GameObjectAI");
...
{ 
    ...
    if (!data)
    {
        LOG_ERROR("root", "GameObject::SaveToDB failed, cannot get gameobject data!");
        return;
    }
...
{ 
    ...
    if (!data)
    {
        LOG_ERROR("root", "Gameobject (GUID: %u) not found in table `gameobject`, can't load. ", guid);
        return false;
    }
...
void GameObject::Use(Unit* user)
{
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_GOOBER:                        //10
        {
            ...
            if (user->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                if (info->goober.eventId)
                {
                    ...
                    LOG_DEBUG("root", "Goober ScriptStart id %u for GO entry %u (GUID %u).", info->goober.eventId, GetEntry(), GetDBTableGUIDLow());
#endif
        ...
        case GAMEOBJECT_TYPE_FISHINGNODE:                   //17
        {
            ...
            switch (getLootState())
            {
                case GO_READY:                              // ready for loot
                {
                    ...
                    if (!zone_skill)
                        LOG_ERROR("root", "Fishable areaId %u are not properly defined in `skill_fishing_base_level`.", subzone);
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
                    LOG_DEBUG("root", "Fishing check (skill: %i zone min skill: %i chance %i roll: %i", skill, zone_skill, chance, roll);
#endif
        ...
        default:
            if (GetGoType() >= MAX_GAMEOBJECT_TYPE)
                LOG_ERROR("root", "GameObject::Use(): unit (type: %u, guid: %u, name: %s) tries to use object (guid: %u, entry: %u, name: %s) of unknown type (%u)",
    ...
    if (!spellInfo)
    {
        if (user->GetTypeId() != TYPEID_PLAYER || !sOutdoorPvPMgr->HandleCustomSpell(user->ToPlayer(), spellId, this))
            LOG_ERROR("root", "WORLD: unknown spell id %u at use action for gameobject (Entry: %u GoType: %u)", spellId, GetEntry(), GetGoType());
        else
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            LOG_DEBUG("root", "WORLD: %u non-dbc spell was handled by OutdoorPvP", spellId);
#endif
        return;
    }
