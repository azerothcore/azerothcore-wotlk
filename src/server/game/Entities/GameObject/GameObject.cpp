...
{ 
    ...
    if (!data)
    {
        LOG_ERROR("server", "Gameobject (GUID: %u) not found in table `gameobject`, can't load. ", guid);
        return false;
    }
        ...
        case GAMEOBJECT_TYPE_GOOBER:                        //10
        {
            ...
            if (user->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                if (info->goober.eventId)
                {
                    ...
                    LOG_DEBUG("server", "Goober ScriptStart id %u for GO entry %u (GUID %u).", info->goober.eventId, GetEntry(), GetDBTableGUIDLow());
#endif
        ...
        case GAMEOBJECT_TYPE_FISHINGNODE:                   //17
        {
            ...
            switch (getLootState())
            {
                case GO_READY:                              // ready for loot
                {
                    ...
                    if (!zone_skill)
                        LOG_ERROR("server", "Fishable areaId %u are not properly defined in `skill_fishing_base_level`.", subzone);
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            LOG_DEBUG("server", "WORLD: %u non-dbc spell was handled by OutdoorPvP", spellId);
#endif
        return;
    }
