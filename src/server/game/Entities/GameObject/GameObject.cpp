...
bool GameObject::LoadGameObjectFromDB(ObjectGuid::LowType spawnId, Map* map, bool addToMap)
{
    ...
    if (!data)
    {
        sLog->outErrorDb("Gameobject (GUID: %u) not found in table `gameobject`, can't load. ", spawnId);
        return false;
    }
        ...
        case GAMEOBJECT_TYPE_GOOBER:                        //10
            {
                ...
                if (user->GetTypeId() == TYPEID_PLAYER)
                {
                    ...
                    if (info->goober.eventId)
                    {
                        ...
                        sLog->outDebug(LOG_FILTER_MAPSCRIPTS, "Goober ScriptStart id %u for GO entry %u (spawnId %u).", info->goober.eventId, GetEntry(), m_spawnId);
#endif
...
void GameObject::ModifyHealth(int32 change, Unit* attackerOrHealer /*= nullptr*/, uint32 spellId /*= 0*/)
{
    ...
    if (player)
    {
        ...
        data << attackerOrHealer->GetGUID().WriteAsPacked();
        data << player->GetGUID().WriteAsPacked();
