...
{ 
    ...
    LOG_DEBUG("server", "Pet::CreateBaseForTamed");
#endif
...
{ 
    ...
    if (IsPet() && m_owner->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        else if (m_owner->getClass() == CLASS_HUNTER)
        {
            ...
            LOG_ERROR("server", "Unknown type pet %u is summoned by player class %u", GetEntry(), m_owner->getClass());
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            LOG_DEBUG("server", "Pet (Number: %u) spell %u cooldown loaded (%u secs).", m_charmInfo->GetPetNumber(), spell_id, uint32(db_time-curTime));
#endif
        }
...
{ 
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Loading auras for pet %u", GetGUIDLow());
#endif
