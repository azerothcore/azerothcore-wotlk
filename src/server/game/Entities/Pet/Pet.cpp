    ...
    switch (m_deathState)
    {
        ...
        case ALIVE:
        {
            ...
            if (isControlled())
            {
                ...
                {
                    LOG_ERROR("root", "Pet %u is not pet of owner %s, removed", GetEntry(), m_owner->GetName().c_str());
...
{ 
    ...
    if (!IsPositionValid())
    {
        LOG_ERROR("root", "Pet (guidlow %d, entry %d) not created base at creature. Suggested coordinates isn't valid (X: %f Y: %f)",
    ...
    if (!cinfo)
    {
        LOG_ERROR("root", "CreateBaseAtCreature() failed, creatureInfo is missing!");
        return false;
    }
...
{ 
    ...
    LOG_DEBUG("root", "Pet::CreateBaseForTamed");
#endif
...
{ 
    ...
    if (IsPet() && m_owner->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        else if (m_owner->getClass() == CLASS_HUNTER)
        {
            ...
            LOG_ERROR("root", "Unknown type pet %u is summoned by player class %u", GetEntry(), m_owner->getClass());
    }
...
{ 
    ...
    if (result)
    {
        ...
        do
        {
            ...
            {
                LOG_ERROR("root", "Pet %u have unknown spell %u in `pet_spell_cooldown`, skipping.", m_charmInfo->GetPetNumber(), spell_id);
                continue;
            }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            LOG_DEBUG("root", "Pet (Number: %u) spell %u cooldown loaded (%u secs).", m_charmInfo->GetPetNumber(), spell_id, uint32(db_time-curTime));
#endif
        }
...
{ 
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "Loading auras for pet %u", GetGUIDLow());
#endif
    ...
    if (result)
    {
        do
        {
            ...
            if (!spellInfo)
            {
                LOG_ERROR("root", "Unknown aura (spellid %u), ignore.", spellid);
                continue;
            }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
                LOG_DEBUG("root", "Added aura spellid %u, effectmask %u", spellInfo->Id, effmask);
#endif
            }
        }
        while (result->NextRow());
...
{ 
    ...
    if (!spellInfo)
    {
        ...
        {
            LOG_ERROR("root", "Pet::addSpell: Non-existed in SpellStore spell #%u request, deleting for all pets in `pet_spell`.", spellId);
            ...
            LOG_ERROR("root", "Pet::addSpell: Non-existed in SpellStore spell #%u request.", spellId);
...
void Pet::HandleAsynchLoadFailed(AsynchPetSummon* info, Player* player, uint8 asynchLoadType, uint8 loadResult)
{
    ...
    else if (loadResult == PET_LOAD_NO_RESULT && info && (asynchLoadType == PET_LOAD_SUMMON_PET || asynchLoadType == PET_LOAD_SUMMON_DEAD_PET))
    {
        ...
        {
            LOG_ERROR("root", "no such creature entry %u", info->m_entry);
            delete pet;
            return;
        }
