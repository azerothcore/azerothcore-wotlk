...
Pet::Pet(Player* owner, PetType type) :
    ...
    m_usedTalentCount(0), m_removed(false), m_happinessTimer(PET_LOSE_HAPPINES_INTERVAL), m_petType(type), m_duration(0),
    m_auraRaidUpdateMask(0), m_loading(false), m_petRegenTimer(PET_FOCUS_REGEN_INTERVAL), m_tempspellTarget(nullptr), 
    m_tempoldTarget(nullptr), m_tempspellIsPositive(false), m_tempspell(0)
{
    ASSERT(GetOwner());
...
bool Pet::LoadPetFromDB(Player* owner, uint32 petEntry, uint32 petnumber, bool current)
{
    ...
    if (IsCritter())
    {
        ...
        if (!IsPositionValid())
        {
            LOG_DEBUG("entities.pet", "Pet%s not loaded. Suggested coordinates isn't valid (X: %f Y: %f)",
    ...
    switch (getPetType())
    {
        case SUMMON_PET:
            ...
            // enables popup window (pet dismiss, cancel)
            break;
        case HUNTER_PET:
            ...
            SetUInt32Value(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED);
            // enables popup window (pet abandon, cancel)
    ...
    // fake summon spell cast for correct cooldown application spells Example: 46584 
    // SOMEONE MUST FIX IT LATER: pets should be summoned from real cast instead of just faking it?
