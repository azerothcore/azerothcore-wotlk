...
void Pet::AddToWorld()
{
    ...
    if (GetOwnerGUID().IsPlayer())
    {
        ...
        {
            if (getPetType() == SUMMON_PET && owner->IsClass(CLASS_WARLOCK, CLASS_CONTEXT_ABILITY))
            {
...
bool Pet::LoadPetFromDB(Player* owner, uint32 petEntry, uint32 petnumber, bool current, uint32 healthPct /*= 0*/)
{
    ...
    if (!forceLoadFromDB && (owner->IsClass(CLASS_DEATH_KNIGHT, CLASS_CONTEXT_ABILITY) && !owner->CanSeeDKPet())) // DK Pet exception
        return false;
...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    if (owner->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (IsPet())
        {
            if (petType == MAX_PET_TYPE)
            {
                ...
                if (owner->IsClass(CLASS_WARLOCK, CLASS_CONTEXT_ABILITY) ||
                        owner->IsClass(CLASS_SHAMAN, CLASS_CONTEXT_ABILITY) ||          // Fire Elemental
                        owner->IsClass(CLASS_DEATH_KNIGHT, CLASS_CONTEXT_ABILITY) ||    // Risen Ghoul
                        owner->IsClass(CLASS_MAGE, CLASS_CONTEXT_ABILITY))              // Water Elemental with glyph
                    petType = SUMMON_PET;
                else if (owner->IsClass(CLASS_HUNTER, CLASS_CONTEXT_ABILITY))
                {
                    petType = HUNTER_PET;
                }
            }
    ...
    if (!owner->IsClass(CLASS_HUNTER, CLASS_CONTEXT_ABILITY) && cinfo->BaseAttackTime >= 1000)
...
bool Pet::IsPermanentPetFor(Player* owner) const
{
    switch (getPetType())
    {
        case SUMMON_PET:
            ...
            if (owner->IsClass(CLASS_DEATH_KNIGHT, CLASS_CONTEXT_ABILITY))
                return GetCreatureTemplate()->type == CREATURE_TYPE_UNDEAD;
            if (owner->IsClass(CLASS_MAGE, CLASS_CONTEXT_ABILITY))
