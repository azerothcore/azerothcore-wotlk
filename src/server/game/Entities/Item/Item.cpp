...
void AddItemsSetItem(Player* player, Item* item)
{
    ...
    if (!set)
    {
        LOG_INFO("sql.sql", "Item set %u for item (id %u) not found, mods not applied.", setid, proto->ItemId);
        return;
    }
...
void RemoveItemsSetItem(Player* player, ItemTemplate const* proto)
{
    ...
    if (!set)
    {
        LOG_INFO("sql.sql", "Item set #%u for item #%u not found, mods not removed.", setid, proto->ItemId);
        return;
    }
...
int32 Item::GenerateItemRandomPropertyId(uint32 item_id)
{
    ...
    if ((itemProto->RandomProperty) && (itemProto->RandomSuffix))
    {
        LOG_INFO("sql.sql", "Item template %u have RandomProperty == %u and RandomSuffix == %u, but must have one from field =0", itemProto->ItemId, itemProto->RandomProperty, itemProto->RandomSuffix);
        return 0;
    }
    ...
    if (itemProto->RandomProperty)
    {
        ...
        if (!random_id)
        {
            LOG_INFO("sql.sql", "Enchantment id #%u used but it doesn't have records in 'ItemRandomProperties.dbc'", randomPropId);
            return 0;
        }
    ...
    else
    {
        ...
        if (!random_id)
        {
            LOG_INFO("sql.sql", "Enchantment id #%u used but it doesn't have records in sItemRandomSuffixStore.", randomPropId);
            return 0;
        }
