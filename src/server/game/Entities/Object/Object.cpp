...
{ 
    ...
    //LOG_DEBUG("server", "BuildCreateUpdate: update-type: %u, object-type: %u got flags: %X, flags2: %X", updatetype, m_objectTypeId, flags, flags2);
...
void MovementInfo::OutDebug()
{
    ...
    LOG_INFO("server", "time %u current time " UI64FMTD "", flags2, uint64(::time(NULL)));
...
{ 
    ...
    if (!goinfo)
    {
        LOG_ERROR("server", "Gameobject template %u not found in database!", entry);
        return NULL;
    }
