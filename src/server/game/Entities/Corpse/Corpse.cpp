...
{ 
    ...
    if (!IsPositionValid())
    {
        LOG_ERROR("root", "Corpse (guidlow %d, owner %s) not created. Suggested coordinates isn't valid (X: %f Y: %f)",
...
{ 
    ...
    if (!IsPositionValid())
    {
        LOG_ERROR("root", "Corpse (guid: %u, owner: %u) is not created, given coordinates are not valid (X: %f, Y: %f, Z: %f)",
