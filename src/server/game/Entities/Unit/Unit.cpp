...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown, uint32 procPhase, ProcEventInfo& eventInfo)
{
    ...
    if (auraSpellInfo->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT)
    {
        ...
        {
            if (GetTypeId() != TYPEID_PLAYER || !IsClass(CLASS_DEATH_KNIGHT))
                return false;
        ...
        {
            Player* plr = ToPlayer();
            if (!plr || !plr->IsClass(CLASS_DEATH_KNIGHT) || !procSpell)
                return false;
        ...
        if (procExtra & (PROC_EX_CRITICAL_HIT | PROC_EX_PARRY | PROC_EX_DODGE | PROC_EX_BLOCK))
        {
            ...
            if (isVictim)
            {
                ...
                if (procExtra & PROC_EX_DODGE)
                {
                    ...
                    if (!IsClass(CLASS_ROGUE)) // skip Rogue Riposte
                    {
...
void Unit::ClearAllReactives()
{
    ...
    if (IsClass(CLASS_HUNTER) && HasAuraState(AURA_STATE_HUNTER_PARRY))
        ModifyAuraState(AURA_STATE_HUNTER_PARRY, false);
    if (IsClass(CLASS_WARRIOR) && GetTypeId() == TYPEID_PLAYER)
        ClearComboPoints();
}
...
void Unit::Kill(Unit* killer, Unit* victim, bool durabilityLoss, WeaponAttackType attackType, SpellInfo const* spellProto, Spell const* spell /*= nullptr*/)
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER && victim->IsClass(CLASS_PRIEST) && !victim->ToPlayer()->HasPlayerFlag(PLAYER_FLAGS_IS_OUT_OF_BOUNDS))
    {
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
    ...
    if (GetTypeId() == TYPEID_UNIT)
    {
        ...
        if (charmer->GetTypeId() == TYPEID_PLAYER && charmer->IsClass(CLASS_WARLOCK) && ToCreature()->GetCreatureTemplate()->type == CREATURE_TYPE_DEMON)
        {
    ...
    if (playerCharmer)
    {
        switch (type)
        {
            ...
            case CHARM_TYPE_CHARM:
                if (GetTypeId() == TYPEID_UNIT && charmer->IsClass(CLASS_WARLOCK))
                {
...
bool Unit::IsClass(Classes unitClass) const
{
