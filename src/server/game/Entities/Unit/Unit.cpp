        ...
        if (procExtra & (PROC_EX_CRITICAL_HIT | PROC_EX_PARRY | PROC_EX_DODGE | PROC_EX_BLOCK))
        {
            ...
            if (isVictim)
            {
                ...
                if (procExtra & PROC_EX_DODGE)
                {
                    ...
                    if (!IsClass(CLASS_ROGUE, CLASS_CONTEXT_ABILITY)) // skip Rogue Riposte
                    {
            ...
            else // For attacker
            {
                ...
                if (procExtra & PROC_EX_DODGE)
                {
                    if (IsClass(CLASS_WARRIOR, CLASS_CONTEXT_ABILITY))
                    {
...
void Unit::ClearAllReactives()
{
    ...
    if (IsClass(CLASS_HUNTER, CLASS_CONTEXT_ABILITY) && HasAuraState(AURA_STATE_HUNTER_PARRY))
        ModifyAuraState(AURA_STATE_HUNTER_PARRY, false);
    if (IsClass(CLASS_WARRIOR, CLASS_CONTEXT_ABILITY) && GetTypeId() == TYPEID_PLAYER)
        ClearComboPoints();
}
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
    ...
    if (GetTypeId() == TYPEID_UNIT)
    {
        ...
        if (charmer->GetTypeId() == TYPEID_PLAYER && charmer->IsClass(CLASS_WARLOCK, CLASS_CONTEXT_ABILITY) && ToCreature()->GetCreatureTemplate()->type == CREATURE_TYPE_DEMON)
        {
    ...
    if (playerCharmer)
    {
        switch (type)
        {
            ...
            case CHARM_TYPE_CHARM:
                if (GetTypeId() == TYPEID_UNIT && charmer->IsClass(CLASS_WARLOCK, CLASS_CONTEXT_ABILITY))
                {
