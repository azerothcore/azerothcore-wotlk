...
ObjectMgr::LoadInstanceTemplate()
{
    uint32 oldMSTime = getMSTime();
        ...
        for (uint8 i = 0; i < MAX_GOSSIP_TEXT_OPTIONS; ++i)
        {
            ...
            if (gOption.BroadcastTextID)
            {
                if (BroadcastText const* bcText = sGameLocale->GetBroadcastText(gOption.BroadcastTextID))
                {
                    ...
                    {
                        FMT_LOG_ERROR("sql.sql", "Row {} in table `npc_text` has mismatch between text{}_0 and the corresponding Text in `broadcast_text` row {}",
                            id, i, gOption.BroadcastTextID);
                    }

                    if (bcText->Text1[DEFAULT_LOCALE] != gOption.Text_1)
                    {
                        FMT_LOG_ERROR("sql.sql", "Row {} in table `npc_text` has mismatch between text{}_1 and the corresponding Text1 in `broadcast_text` row {}",
                            id, i, gOption.BroadcastTextID);
                    }
                }
                else
                {
                    FMT_LOG_ERROR("sql.sql", "GossipText (Id: {}) in table `npc_text` has non-existing or incompatible BroadcastTextID{} {}.", id, i, gOption.BroadcastTextID);
