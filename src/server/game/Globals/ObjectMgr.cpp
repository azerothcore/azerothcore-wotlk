...
void ObjectMgr::LoadPlayerInfo()
{
    ...
    LOG_INFO("server", "Loading Player Create Spell Data...");
    {
        ...
        if (!result)
        {
            LOG_ERROR("sql.sql", ">> Loaded 0 player create spells. DB table `playercreateinfo_spell_custom` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            LOG_INFO("server", ">> Loaded %u custom player create spells in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server", " ");
        }
    }
