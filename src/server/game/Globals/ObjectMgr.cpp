...
void ObjectMgr::LoadCreatureTemplateAddons()
{
    ...
    do
    {
        ...
        for (std::string_view aura : Acore::Tokenize(fields[7].GetStringView(), ' ', false))
        {
            ...
            {
                FMT_LOG_ERROR("sql.sql", "Creature (Entry: {}) has duplicate aura (spell {}) in `auras` field in `creature_template_addon`.", entry, spellInfo->Id);
                continue;
            }
            ...
            case CREATURE_TO_CREATURE:
                {
                    ...
                    if (!slave)
                    {
                        LOG_ERROR("sql.sql", "Couldn't get creature data for GUIDLow {}", guidLow);
                        error = true;
                        break;
                    }
                    ...
                    if (!master)
                    {
                        LOG_ERROR("sql.sql", "Couldn't get creature data for GUIDLow {}", linkedGuidLow);
                        error = true;
                        break;
                    }
            ...
            case CREATURE_TO_GO:
                {
                    ...
                    if (!master)
                    {
                        LOG_ERROR("sql.sql", "Couldn't get gameobject data for GUIDLow {}", linkedGuidLow);
                        error = true;
                        break;
                    }
            ...
            case GO_TO_GO:
                {
                    ...
                    if (!slave)
                    {
                        LOG_ERROR("sql.sql", "Couldn't get gameobject data for GUIDLow {}", guidLow);
                        error = true;
                        break;
                    }
