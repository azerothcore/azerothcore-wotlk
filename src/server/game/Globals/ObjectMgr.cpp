...
void ObjectMgr::LoadPlayerInfo()
{
    ...
    sLog->outString("Loading Player Create Spell Data...");
    {
        ...
        if (!result)
        {
            sLog->outString(">> Loaded 0 player create spells. DB table `playercreateinfo_spell_custom` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            sLog->outString(">> Loaded %u custom player spells in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }
