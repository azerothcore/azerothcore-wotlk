...
{
    TempSummonGroupKey(uint32 summonerEntry, SummonerType summonerType, uint8 group) : _summonerEntry(summonerEntry), _summonerType(summonerType), _summonGroup(group) {}
...
struct PetLevelInfo
{
    PetLevelInfo()
    {
        for (unsigned short& stat : stats) stat = 0;
    }
...
public:
    ...
    [[nodiscard]] GameObjectTemplateContainer const* GetGameObjectTemplates() const
    {
    ...
    [[nodiscard]] std::vector<ItemTemplate*> const* GetItemTemplateStoreFast() const
    {
    ...
    [[nodiscard]] GameObjectQuestItemMap const* GetGameObjectQuestItemMap() const
    {
    ...
    template <HighGuid type> inline ObjectGuidGeneratorBase& GetGenerator()
    {
