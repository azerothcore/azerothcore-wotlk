...
{
    LOG_TRACE("server.authserver", "'%s:%d' Accepting connection", socket().getRemoteAddress().c_str(), socket().getRemotePort());
}

void AuthSocket::OnClose(void)
{
    LOG_TRACE("server.authserver", "AuthSocket::OnClose");
}
...
{
    LOG_TRACE("server.authserver", "Entering _HandleLogonChallenge");
    ...
    if (result)
    {
        ...
        LOG_DEBUG("server.authserver", "'%s:%d' [AuthChallenge] Banned ip tries to login!",socket().getRemoteAddress().c_str(), socket().getRemotePort());
    }
    else
    {
        ...
        if (res2)
        {
            ...
            {
                LOG_DEBUG("server.authserver", "[AuthChallenge] Account '%s' is locked to IP - '%s'", _login.c_str(), fields[3].GetCString());
                LOG_DEBUG("server.authserver", "[AuthChallenge] Player address is '%s'", ip_address.c_str());
                ...
                if (strcmp(fields[4].GetCString(), ip_address.c_str()) != 0)
                {
                    ...
                    LOG_DEBUG("server.authserver", "[AuthChallenge] Account IP matches");
            }
            else
            {
                ...
                else if (!accountCountry.empty())
                {
                    ...
                    if (PreparedQueryResult sessionCountryQuery = LoginDatabase.Query(stmt))
                    {
                        ...
                        LOG_DEBUG("server.authserver", "[AuthChallenge] Account '%s' is locked to country: '%s' Player country is '%s'", _login.c_str(), accountCountry.c_str(), loginCountry.c_str());
                        if (loginCountry != accountCountry)
                        {
                            ...
                            LOG_DEBUG("server.authserver", "[AuthChallenge] Account country matches");
                    }
                    else
                        LOG_DEBUG("server.authserver", "[AuthChallenge] IP2NATION Table empty");
                }
            }
            ...
            if (!locked)
            {
                ...
                if (banresult)
                {
                    ...
                    {
                        pkt << uint8(WOW_FAIL_BANNED);
                        LOG_DEBUG("server.authserver", "'%s:%d' [AuthChallenge] Banned account %s tried to login!", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str ());
                    }
                    else
                    {
                        pkt << uint8(WOW_FAIL_SUSPENDED);
                        LOG_DEBUG("server.authserver", "'%s:%d' [AuthChallenge] Temporarily banned account %s tried to login!", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str ());
                    }
                }
                else
                {
                    ...
                    LOG_DEBUG("server.authserver", "Database authentication values: v='%s' s='%s'", databaseV.c_str(), databaseS.c_str());
                    ...
                    LOG_DEBUG("server.authserver", "'%s:%d' [AuthChallenge] account %s is using '%c%c%c%c' locale (%u)", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str (), ch->country[3], ch->country[2], ch->country[1], ch->country[0], GetLocaleByName(_localizationName) );
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    {
        LOG_DEBUG("server.authserver", "'%s:%d' User '%s' successfully authenticated", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str());
    ...
    else
    {
        ...
        LOG_DEBUG("server.authserver", "'%s:%d' [AuthChallenge] account %s tried to login with invalid password!", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str());
            ...
            if (PreparedQueryResult loginfail = LoginDatabase.Query(stmt))
            {
                ...
                if (failed_logins >= MaxWrongPassCount)
                {
                    ...
                    if (WrongPassBanType)
                    {
                        ...
                        LOG_DEBUG("server.authserver", "'%s:%d' [AuthChallenge] account %s got banned for '%u' seconds because it failed to authenticate '%u' times", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str(), WrongPassBanTime, failed_logins);
                    }
                    else
                    {
...
bool AuthSocket::_HandleReconnectChallenge()
{
    ...
    if (!result)
    {
        LOG_ERROR("server.authserver", "'%s:%d' [ERROR] user %s tried to login and we cannot find his session key in the database.", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str());
        socket().shutdown();
        return false;
    }
...
bool AuthSocket::_HandleReconnectProof()
{
    ...
    {
        LOG_ERROR("server.authserver", "'%s:%d' [ERROR] user %s tried to login, but session is invalid.", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str());
        socket().shutdown();
        return false;
    }
}
...
bool AuthSocket::_HandleRealmList()
{
    ...
    if (!result)
    {
        LOG_ERROR("server.authserver", "'%s:%d' [ERROR] user %s tried to login but we cannot find him in the database.", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str());
        socket().shutdown();
        return false;
    }
...
{
    LOG_TRACE("server.authserver", "Entering _HandleXferCancel");
...
{
    LOG_TRACE("server.authserver", "Entering _HandleXferAccept");
