...
void TCSoapRunnable::run()
{
    ...
    {
        LOG_ERROR("root", "TCSoap: couldn't bind to %s:%d", _host.c_str(), _port);
        exit(-1);
    }

    LOG_INFO("root", "TCSoap: bound to http://%s:%d", _host.c_str(), _port);
    ...
    while (!World::IsStopped())
    {
        ...
        LOG_DEBUG("root", "TCSoap: accepted connection from IP=%d.%d.%d.%d", (int)(soap.ip>>24)&0xFF, (int)(soap.ip>>16)&0xFF, (int)(soap.ip>>8)&0xFF, (int)soap.ip&0xFF);
#endif
...
int ns1__executeCommand(soap* soap, char* command, char** result)
{
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "TCSoap: Client didn't provide login information");
#endif
        return 401;
    }
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "TCSoap: Client used invalid username '%s'", soap->userid);
#endif
        return 401;
    }
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "TCSoap: invalid password for account '%s'", soap->userid);
#endif
        return 401;
    }
    ...
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("root", "TCSoap: %s's gmlevel is too low", soap->userid);
#endif
        return 403;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("root", "TCSoap: got command '%s'", command);
#endif
    ...
    if (acc)
    {
        LOG_ERROR("root", "TCSoap: Error while acquiring lock, acc = %i, errno = %u", acc, errno);
    }
