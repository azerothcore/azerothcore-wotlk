...
int RASocket::open(void *)
{
    ...
    {
        LOG_ERROR("root", "RASocket::open: peer().get_remote_addr error is %s", ACE_OS::strerror(errno));
        return -1;
    }

    LOG_DEBUG("root", "Incoming connection from %s", remoteAddress.get_host_addr());
...
{
    LOG_DEBUG("root", "Closing connection");
    peer().close_reader();
    wait();
...
int RASocket::recv_line(std::string& out_line)
{
    ...
    {
        LOG_DEBUG("root", "Recv error %s", ACE_OS::strerror(errno));
        return -1;
    }
...
int RASocket::process_command(const std::string& command)
{
    ...
    LOG_DEBUG("root", "Got command: %s", command.c_str());
...
int RASocket::check_access_level(const std::string& user)
{
    ...
    if (!result)
    {
        LOG_DEBUG("root", "User %s does not exist in database", user.c_str());
        return -1;
    }
    ...
    {
        LOG_DEBUG("root", "User %s has no privilege to login", user.c_str());
        return -1;
    }
    else if (fields[2].GetInt32() != -1)
    {
        LOG_DEBUG("root", "User %s has to be assigned on all realms (with RealmID = '-1')", user.c_str());
        return -1;
    }
...
int RASocket::check_password(const std::string& user, const std::string& pass)
{
    ...
    if (!result)
    {
        LOG_DEBUG("root", "Wrong password for user: %s", user.c_str());
        return -1;
    }
...
int RASocket::authenticate()
{
    ...
    LOG_DEBUG("root", "Login attempt for user: %s", user.c_str());
    ...
    LOG_DEBUG("root", "User login: %s", user.c_str());
...
int RASocket::subnegotiate()
{
    ...
    if (n >= 1024)
    {
        LOG_DEBUG("root", "RASocket::subnegotiate: allocated buffer 1024 bytes was too small for negotiation packet, size: %u", uint32(n));
        return -1;
    }
...
void RASocket::zprint(void* callbackArg, const char * szText)
{
    ...
    {
        LOG_DEBUG("root", "Failed to enqueue message, queue is full or closed. Error is %s", ACE_OS::strerror(errno));
        mb->release();
    }
}
...
void RASocket::commandFinished(void* callbackArg, bool /*success*/)
{
    ...
    if (socket->putq(mb->duplicate()) == -1)
    {
         ...
         //LOG_DEBUG("root", "Failed to enqueue command end message. Error is %s", ACE_OS::strerror(errno));
    }
