...
public:
    ...
    {
        static std::vector<ChatCommand> rbacGroupsCommandTable =
        {
            {         "add", RBAC_PERM_COMMAND_RBAC_ACC_GROUP_ADD,  true, &HandleRBACGroupAddCommand,    "" },
            {      "remove", RBAC_PERM_COMMAND_RBAC_ACC_GROUP_DEL,  true, &HandleRBACGroupRemoveCommand, "" },
            {            "", RBAC_PERM_COMMAND_RBAC_ACC_GROUP,      true, &HandleRBACGroupListCommand,   "" }
        };

        static std::vector<ChatCommand> rbacRolesCommandTable =
        {
            {       "grant", RBAC_PERM_COMMAND_RBAC_ACC_ROLE_GRANT,  true, &HandleRBACRoleGrantCommand,   "" },
            {        "deny", RBAC_PERM_COMMAND_RBAC_ACC_ROLE_DENY,   true, &HandleRBACRoleDenyCommand,    "" },
            {      "revoke", RBAC_PERM_COMMAND_RBAC_ACC_ROLE_REVOKE, true, &HandleRBACRoleRevokeCommand,  "" },
            {            "", RBAC_PERM_COMMAND_RBAC_ACC_ROLE,        true, &HandleRBACRoleListCommand,    "" }
        };

        static std::vector<ChatCommand> rbacPermsCommandTable =
        {
            {       "grant", RBAC_PERM_COMMAND_RBAC_ACC_PERM_GRANT,  true, &HandleRBACPermGrantCommand,   "" },
            {        "deny", RBAC_PERM_COMMAND_RBAC_ACC_PERM_DENY,   true, &HandleRBACPermDenyCommand,    "" },
            {      "revoke", RBAC_PERM_COMMAND_RBAC_ACC_PERM_REVOKE, true, &HandleRBACPermRevokeCommand,  "" },
            {            "", RBAC_PERM_COMMAND_RBAC_ACC_PERM,        true, &HandleRBACPermListCommand,    "" }
        };

        static std::vector<ChatCommand> rbacListCommandTable =
        {
            {      "groups", RBAC_PERM_COMMAND_RBAC_LIST_GROUPS, true, &HandleRBACListGroupsCommand,      "" },
            {       "roles", RBAC_PERM_COMMAND_RBAC_LIST_ROLES,  true, &HandleRBACListRolesCommand,       "" },
            { "permissions", RBAC_PERM_COMMAND_RBAC_LIST_PERMS,  true, &HandleRBACListPermissionsCommand, "" }
        };

        static std::vector<ChatCommand> rbacAccountCommandTable =
        {
            {       "group", RBAC_PERM_COMMAND_RBAC_ACC_GROUP, true, nullptr, "", rbacGroupsCommandTable },
            {        "role", RBAC_PERM_COMMAND_RBAC_ACC_ROLE,  true, nullptr, "", rbacRolesCommandTable },
            {  "permission", RBAC_PERM_COMMAND_RBAC_ACC_PERM,  true, nullptr, "", rbacPermsCommandTable },
            {            "", RBAC_PERM_COMMAND_RBAC_ACC,       true, &HandleRBACAccountPermissionCommand, "" }
        };

        static std::vector<ChatCommand> rbacCommandTable =
        {
            {    "account", RBAC_PERM_COMMAND_RBAC_ACC,  true, nullptr, "", rbacAccountCommandTable },
            {       "list", RBAC_PERM_COMMAND_RBAC_LIST, true, nullptr, "", rbacListCommandTable }
        };

        static std::vector<ChatCommand> commandTable =
        {
            {       "rbac", RBAC_PERM_COMMAND_RBAC, true, nullptr, "", rbacCommandTable }
        };
