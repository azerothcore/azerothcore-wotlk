        ...
        bool Load(char const* fn, SqlDbc* sql)
        {
            ...
            if (sql)
            {
                ...
                if (result)
                {
                    ...
                    {
                        LOG_ERROR("maps", "Invalid index pos for dbc:'%s'", sql->sqlTableName.c_str());
                        return false;
                    }
                }
            }
                ...
                if (indexTable.asT)
                {
                    ...
                    do
                    {
                        ...
                        if (sql->indexPos >= 0)
                        {
                            ...
                            {
                                LOG_ERROR("maps", "Index %d already exists in dbc:'%s'", id, sql->sqlTableName.c_str());
                                return false;
                            }
                        ...
                        for (; columnNumber < sql->formatString->size(); ++columnNumber)
                        {
                                ...
                                switch (fmt[columnNumber])
                                {
                                    ...
                                    case FT_BYTE:
                                        *reinterpret_cast<uint8*>(&sqlDataTable[offset]) = fields[sqlColumnNumber].GetUInt8();
                                        ...
                                        break;
                                    case FT_STRING:
                                        LOG_ERROR("maps", "Unsupported data type in table '%s' at char %d", sql->sqlTableName.c_str(), columnNumber);
                                        return false;
                                    case FT_SORT:
                                        break;
                                    default:
                            ...
                            {
                                LOG_ERROR("maps", "Incorrect sql format string '%s' at char %d", sql->sqlTableName.c_str(), columnNumber);
                                return false;
                            }
                        }
                        ...
                        {
                            LOG_ERROR("maps", "SQL and DBC format strings are not matching for table: '%s'", sql->sqlTableName.c_str());
                            return false;
                        }
