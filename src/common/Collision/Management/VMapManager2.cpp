    ...
    */
    ...
    WorldModel* VMapManager2::acquireModelInstance(const std::string& basepath, const std::string& filename)
    {
        ...
        if (model == iLoadedModelFiles.end())
        {
            ...
            {
                LOG_ERROR("root", "VMapManager2: could not load '%s%s.vmo'", basepath.c_str(), filename.c_str());
                delete worldmodel;
                return NULL;
            }
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            LOG_DEBUG("root", "VMapManager2: loading file '%s%s'", basepath.c_str(), filename.c_str());
#endif
    ...
    void VMapManager2::releaseModelInstance(const std::string &filename)
    {
        ...
        if (model == iLoadedModelFiles.end())
        {
            LOG_ERROR("root", "VMapManager2: trying to unload non-loaded file '%s'", filename.c_str());
            return;
        }
        if (model->second.decRefCount() == 0)
        {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            LOG_DEBUG("root", "VMapManager2: unloading file '%s'", filename.c_str());
#endif
