...
bool DBUpdaterUtil::CheckExecutable()
{
    ...
    if (!exists(exe))
    {
        ...
        sLog->outError("Didn't find any executable MySQL binary at \'%s\' or in path, correct the path in the *.conf (\"MySQLExecutable\").",
...
{
    sLog->outString("Database \"%s\" does not exist, do you want to create it? [yes (default) / no]: ",
    ...
    sLog->outString("Creating database \"%s\"...", pool.GetConnectionInfo()->database.c_str());
    ...
    if (!file.is_open())
    {
        sLog->outError("Failed to create temporary query file \"%s\"!", temp.generic_string().c_str());
        return false;
    }
    ...
    {
        sLog->outError("sql.updates", "Failed to create database %s! Does the user (named in *.conf) have `CREATE`, `ALTER`, `DROP`, `INSERT` and `DELETE` privileges on the MySQL server?", pool.GetConnectionInfo()->database.c_str());
...
bool DBUpdater<T>::Update(DatabaseWorkerPool<T>& pool)
{
    ...
    sLog->outString("Updating %s database...", DBUpdater<T>::GetTableName().c_str());

    Path const sourceDirectory(BuiltInConfig::GetSourceDirectory());

    if (!is_directory(sourceDirectory))
    {
        sLog->outError("DBUpdater: The given source directory %s does not exist, change the path to the directory where your sql directory exists (for example c:\\source\\trinitycore). Shutting down.",
    ...
    if (!result.updated)
        sLog->outString(">> %s database is up-to-date! %s", DBUpdater<T>::GetTableName().c_str(), info.c_str());
    else
        sLog->outString(">> Applied " SZFMTD " %s. %s", result.updated, result.updated == 1 ? "query" : "queries", info.c_str());
...
bool DBUpdater<T>::Populate(DatabaseWorkerPool<T>& pool)
{
    ...
    sLog->outString("Database %s is empty, auto populating it...", DBUpdater<T>::GetTableName().c_str());
    ...
    for (std::filesystem::directory_iterator itr(DirPath); itr != DirItr; ++itr)
    {
        ...
        sLog->outString(">> Applying \'%s\'...", itr->path().filename().generic_string().c_str());
                             ...
                             std::string const& password, std::string const& port_or_socket, std::string const& database, Path const& path)
{
    ...
    int const ret = acore::StartProcess(DBUpdaterUtil::GetCorrectedMySQLExecutable(), args, path.generic_string(), true);

    if (ret != EXIT_SUCCESS)
    {
        sLog->outError("Applying of file \'%s\' to database \'%s\' failed!", path.generic_string().c_str(), pool.GetConnectionInfo()->database.c_str());
