...
void UpdateFetcher::FillFileListRecursively(Path const& path, LocaleFileStorage& storage, State const state, uint32 const depth) const
{
    ...
    for (directory_iterator itr(path); itr != end; ++itr)
    {
        ...
        else if (itr->path().extension() == ".sql")
        {
            ...
            {
                sLog->outError("Duplicate filename \"%s\" occurred. Because updates are ordered " \
...
UpdateFetcher::DirectoryStorage UpdateFetcher::ReceiveIncludedDirectories() const
{
    ...
    do
    {
        ...
        if (!is_directory(p))
        {
            sLog->outError("DBUpdater: Given update include directory \"%s\" does not exist, skipped!", p.generic_string().c_str());
            continue;
        }
...
std::string UpdateFetcher::ReadSQLUpdate(Path const& file) const
{
    ...
    if (!in.is_open())
    {
        sLog->outError("Failed to open the sql update \"%s\" for reading! "
                                   ...
                                   int32 const cleanDeadReferencesMaxCount) const
{
    ...
    for (auto const& availableQuery : available)
    {
        ...
        if (iter == applied.end())
        {
            ...
            if (hashIter != hashToName.end())
            {
                ...
                {
                    sLog->outError("sql.updates", ">> It seems like the update \"%s\" \'%s\' was renamed, but the old file is still there! " \
                ...
                {
                    sLog->outString("sql.updates", ">> Renaming update \"%s\" to \"%s\" \'%s\'.",
            ...
            {
                sLog->outString(">> Applying update \"%s\" \'%s\'...",
                         availableQuery.first.filename().string().c_str(), hash.substr(0, 7).c_str());
            }
        }
        ...
        else if (allowRehash && iter->second.hash.empty())
        {
            ...
            sLog->outString(">> Re-hashing update \"%s\" \'%s\'...", availableQuery.first.filename().string().c_str(),
        ...
        else
        {
            ...
            {
                sLog->outString(">> Reapplying update \"%s\" \'%s\' -> \'%s\' (it changed)...", availableQuery.first.filename().string().c_str(),
    ...
    if (!applied.empty())
    {
        ...
        for (auto const& entry : applied)
        {
            sLog->outError(">> The file \'%s\' was applied to the database, but is missing in" \
                     " your update directory now!", entry.first.c_str());

            if (doCleanup)
                sLog->outString("Deleting orphaned entry \'%s\'...", entry.first.c_str());
        }
        ...
        {
            sLog->outError("Cleanup is disabled! There were  " SZFMTD " dirty files applied to your database, " \
