...
DatabaseLoader::DatabaseLoader(uint32 const defaultUpdateMask)
...
DatabaseLoader& DatabaseLoader::AddDatabase(DatabaseWorkerPool<T>& pool, std::string const& name)
{
    ...
    _open.push([this, name, updatesEnabledForThis, &pool]() -> bool
    {
        ...
        if (dbString.empty())
        {
            sLog->outError("Database %s not specified in configuration file!", name.c_str());
            return false;
        }
        ...
        {
            sLog->outError("%s database: invalid number of worker threads specified. Please pick a value between 1 and 32.", name.c_str());
            return false;
        }

        uint8 const synchThreads = static_cast<uint8>(sConfigMgr->GetIntDefault(name + "Database.SynchThreads", 1));
        ...
        if (uint32 error = pool.Open())
        {
            ...
            {
                // Possible improvement for future: make ATTEMPTS configurable values
                ...
                auto _log = [&]()
                {
                    sLog->outError("> Retrying after 5 seconds\n");
            ...
            if (error)
            {
                sLog->outError("DatabasePool %s NOT opened. There were errors opening the MySQL connections. "
    ...
    if (updatesEnabledForThis)
    {
        _populate.push([this, name, &pool]() -> bool
        {
            ...
            {
                sLog->outError("Could not populate the %s database, see log for details.", name.c_str());
                return false;
            }
        ...
        _update.push([this, name, &pool]() -> bool
        {
            ...
            {
                sLog->outError("Could not update the %s database, see log for details.", name.c_str());
                return false;
            }
...
{
    if (!_updateFlags)
        sLog->outString("Automatic database updates are disabled for all databases!");
