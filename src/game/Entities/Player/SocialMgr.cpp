...
void SocialMgr::GetFriendInfo(Player* player, uint32 friendGUID, FriendInfo &friendInfo)
{
    ...
    Player* target = ObjectAccessor::FindPlayerInOrOutOfWorld(friendGUID);
    if (!target || AccountMgr::IsGMAccount(target->GetSession()->GetSecurity()))
        return;
    ...
    if (target->GetTeamId() != player->GetTeamId() && !player->GetSession()->HasPermission(RBAC_PERM_TWO_SIDE_WHO_LIST))
        return;
...
void SocialMgr::BroadcastToFriendListers(Player* player, WorldPacket* packet)
{
    ...
    for (SocialMap::const_iterator itr = m_socialMap.begin(); itr != m_socialMap.end(); ++itr)
    {
        ...
        if (itr2 != itr->second.m_playerSocialMap.end() && (itr2->second.Flags & SOCIAL_FLAG_FRIEND))
        {
            ...
            if (target->GetTeamId() != player->GetTeamId() && !session->HasPermission(RBAC_PERM_TWO_SIDE_WHO_LIST))
                continue;
