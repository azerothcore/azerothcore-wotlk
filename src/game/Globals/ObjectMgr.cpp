    ...
    do
    {
        ...
        for (uint8 i = 0; i < MAX_GOSSIP_TEXT_OPTIONS; i++)
        {
            if (gText.Options[i].BroadcastTextID)
            {
                ...
                {
                    sLog->outErrorDb("GossipText (Id: %u) in table `npc_text` has non-existing or incompatible BroadcastTextID%u %u.", Text_ID, i, gText.Options[i].BroadcastTextID);
...
void ObjectMgr::LoadGossipMenuItems()
{
    ...
    do
    {
        ...
        if (gMenuItem.OptionBroadcastTextId)
        {
            ...
            {
                sLog->outErrorDb("Table `gossip_menu_option` for menu %u, id %u has non-existing or incompatible OptionBroadcastTextId %u, ignoring.", gMenuItem.MenuId, gMenuItem.OptionIndex, gMenuItem.OptionBroadcastTextId);
        ...
        if (gMenuItem.BoxBroadcastTextId)
        {
            ...
            {
                sLog->outErrorDb("Table `gossip_menu_option` for menu %u, id %u has non-existing or incompatible BoxBroadcastTextId %u, ignoring.", gMenuItem.MenuId, gMenuItem.OptionIndex, gMenuItem.BoxBroadcastTextId);
