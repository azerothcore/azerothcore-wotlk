...
public:
	...
	bool OnUse(Player* player, Item* item, SpellCastTargets const& /*targets*/) override // Any hook here
	{
		...
		std::string temp = part1 + std::to_string(player->GetGUID()) + part2;
		...
		do
		{
			...
			if (name == "" || name.empty())
			{
				...
				std::string part2 = " LIMIT 0, 200000; ";
				std::string temp = part1 + std::to_string(entry) + part2;
				const char *query = temp.c_str();
...
public:
	...
	void OnGossipSelect(Player* player, Item* item, uint32 /*sender*/, uint32 action) override
	{
		...
		std::string part2 = ");";
		std::string part3 = ",";
		std::string temp = part1 + std::to_string(player->GetGUID()) + part3 + std::to_string(item->GetMaxStackCount()) + part2;
		const char *query = temp.c_str();
	...
	void OnGossipSelectCode(Player* player, Item* item, uint32 /*sender*/, uint32 action, const char* code) override
	{
		...
		std::string part3 = ",";
		std::string apici = "\"";
		std::string temp = part1 + std::to_string(player->GetGUID()) + part3 + std::to_string(item->GetMaxStackCount()) + part3 + apici + code + apici + part2;
		const char *query = temp.c_str();
