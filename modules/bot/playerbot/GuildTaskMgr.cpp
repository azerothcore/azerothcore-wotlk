...
void GuildTaskMgr::Update(Player* player, Player* guildMaster)
{
    ...
    if (!activeTask)
    {
        ...
        sLog->outDetail("%s / %s: guild task %u is set for %u secs",
                guildMaster->GetGuild()->GetName().c_str(), player->GetName().c_str(),
                task, time);
        return;
    }
    ...
    if (!advertisement)
    {
        sLog->outDetail("%s / %s: sending advertisement",
                guildMaster->GetGuild()->GetName().c_str(), player->GetName().c_str());
    ...
    if (!thanks)
    {
        sLog->outDetail("%s / %s: sending thanks",
                guildMaster->GetGuild()->GetName().c_str(), player->GetName().c_str());
    ...
    if (!reward)
    {
        sLog->outDetail("%s / %s: sending reward",
                guildMaster->GetGuild()->GetName().c_str(), player->GetName().c_str());
...
bool GuildTaskMgr::CreateItemTask(uint32 owner, uint32 guildId)
{
    ...
    sLog->outDetail("%s / %s: item task %u (x%d)",
            sGuildMgr->GetGuildById(guildId)->GetName().c_str(), player->GetName().c_str(),
            itemId, count);
...
bool GuildTaskMgr::CreateKillTask(uint32 owner, uint32 guildId)
{
    ...
    sLog->outDetail("%s / %s: kill task %u",
            sGuildMgr->GetGuildById(guildId)->GetName().c_str(), player->GetName().c_str(),
            creatureId);
...
bool GuildTaskMgr::HandleConsoleCommand(ChatHandler* handler, char const* args)
{
    ...
    if (cmd == "reset")
    {
        ...
        sLog->outBasic("Guild tasks were reset for all players");
        return true;
    }

    if (cmd == "stats")
    {
        sLog->outBasic("Usage: gtask stats <player name>");
        return true;
    }
    ...
    if (cmd.find("stats ") != string::npos)
    {
        ...
        uint64 guid = sWorld->GetGlobalPlayerGUID(charName);
        if (!guid)
        {
        ...
        if (result)
        {
            do
            {
                ...
                sLog->outBasic("Player '%s' Guild '%s' %s=%s (%u secs)",
    ...
    if (cmd == "reward")
    {
        sLog->outBasic("Usage: gtask reward <player name>");
        return true;
    }
...
void GuildTaskMgr::CheckItemTask(uint32 itemId, uint32 obtained, Player* ownerPlayer, Player* bot, bool byMail)
{
    ...
    sLog->outDetail("%s / %s: checking guild task",
            bot->GetGuild()->GetName().c_str(), ownerPlayer->GetName().c_str());

    uint32 itemTask = GetTaskValue(owner, guildId, "itemTask");
    if (itemTask != itemId)
    {
        sLog->outDetail("%s / %s: item %u is not guild task item (%u)",
                bot->GetGuild()->GetName().c_str(), ownerPlayer->GetName().c_str(),
                itemId, itemTask);
        return;
    }
    ...
    if (byMail)
    {
        ...
        SetTaskValue(owner, guildId, "payment", money + proto->BuyPrice * obtained,
    ...
    if (obtained >= count)
    {
        sLog->outDetail("%s / %s: guild task complete",
                bot->GetGuild()->GetName().c_str(), ownerPlayer->GetName().c_str());
    ...
    {
        sLog->outDetail("%s / %s: guild task progress",
                bot->GetGuild()->GetName().c_str(), ownerPlayer->GetName().c_str());
