...
{
    list<uint64> targets = AI_VALUE(list<uint64>, "possible targets");
    return !targets.size();
}
...
{
    list<uint64> targets = AI_VALUE(list<uint64>, "nearest adds");
    return targets.size();
}
	...
	if (ai->GetBot()->InBattleground())
	{
		if (ai->GetBot()->GetBattlegroundTypeId() == BattlegroundTypeId::BATTLEGROUND_WS)
		{
			...
			if (!(bg->GetFlagState(bg->GetOtherTeamId(bot->GetTeamId())) == BG_WS_FLAG_STATE_ON_PLAYER))
				return false;
			if (bot->GetGUID() == bg->GetFlagPickerGUID(bg->GetOtherTeamId(bot->GetTeamId()) == TEAM_ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE)) //flag-Carrier, bring it home
			{
				return false;
			}
		}
