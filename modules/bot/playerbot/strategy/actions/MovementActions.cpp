...
bool MovementAction::ChaseTo(WorldObject* obj)
{
	...
	Unit* destUnit = (Unit*)obj;
	if (destUnit)
	{
		mm.MoveChase(destUnit);
		return true;
	}
...
bool MovementAction::MoveNear(WorldObject* target, float distance)
{
	...
	Unit* destUnit = (Unit*)target;
	if (destUnit)
	{
		distance += destUnit->GetCombatReach() / 2.0f;
	}    
...
bool MovementAction::Follow(WorldObject* target, float distance, float angle)
{
    ...
    if (!bot->InBattleground() && bot->GetDistance2d(target->GetPositionX(), target->GetPositionY()) <= sPlayerbotAIConfig.sightDistance &&
	...
	{
		if (((Unit *)target)->IsFriendlyTo(bot) && bot->IsMounted() && AI_VALUE(list<uint64>, "possible targets").empty())
			distance += angle;
	}	
	...
	if (destUnit)
	{
		mm.MoveFollow(destUnit, distance, angle);
		AI_VALUE(LastMovement&, "last movement").Set(target);
		return true;
	}
