...
void PlayerbotHolder::LogoutPlayerBot(uint64 guid)
{
	...
	if (bot)
	{
		...
		sLog->outBasic("Bot %s logged out", bot->GetName());
		//bot->SaveToDB();
...
void PlayerbotHolder::OnBotLogin(Player * const bot)
{
	...
	ai->ResetStrategies();
	ai->TellMaster("Hello!");
	sLog->outBasic("Bot %s logged in", bot->GetName());

	uint32 account = sObjectMgr->GetPlayerAccountIdByGUID(bot->GetGUID());
	if (sPlayerbotAIConfig.IsInRandomAccountList(account))
	{
		sLog->outBasic("%d/%d Bot %s logged in", playerBots.size(), sRandomPlayerbotMgr.GetMaxAllowedBotCount(), bot->GetName());
	}
	else sLog->outBasic("Bot %s logged in", bot->GetName());
}
