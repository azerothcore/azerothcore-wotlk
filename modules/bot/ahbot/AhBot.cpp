...
void AhBot::Init()
{
    sLog->outBasic("Initializing AhBot by ike3");
    ...
    sLog->outBasic("AhBot configuration loaded");
}
...
uint64 AhBot::GetAHBplayerGUID()
{
    return uint64(sAhBotConfig.guid);
}
...
void AhBot::ForceUpdate()
{
    ...
    sLog->outBasic("AhBot is now checking auctions in background");
    updating = true;
    ...
    if (!allBidders.size())
    {
        sLog->outBasic("Ahbot is enabled but there are no bidders available");
        return;
    }
    ...
    sLog->outBasic("AhBot auction check finished. %d auctions answered, %d new auctions added. Next check in %d seconds",
        ...
        Category*& category, int& auction)
{
            ...
            itr != auctionHouse->GetAuctionsEnd(); ++itr)
    {
        ...
        Item *item = sAuctionMgr->GetAItem(entry->item_guidlow);
        if (!item)
            continue;
        ...
        {
            sLog->outBasic("%s (x%d) in auction %d: price cannot be determined",
        ...
        uint32* minBuyout)
{
    ...
    *minBuyout = 0;
            ...
            itr != auctionHouse->GetAuctionsEnd(); ++itr)
    {
        ...
        Item *otherItem = sAuctionMgr->GetAItem(other->item_guidlow);
...
int AhBot::Answer(int auction, Category* category, ItemBag* inAuctionItems)
{
    ...
    for (vector<AuctionEntry*>::iterator itr = entries.begin(); itr != entries.end(); ++itr)
    {
        ...
        {
            sLog->outBasic("%s (x%d) in auction %d: unavailable item",
                    item->GetTemplate()->Name1.c_str(), item->GetCount(), auctionIds[auction]);
            continue;
        }
        ...
        {
            sLog->outBasic("%s (x%d) in auction %d: answer count %d > %d (max)",
        ...
        {
            sLog->outBasic("%s (x%d) in auction %d: above max required or item level",
                    item->GetTemplate()->Name1.c_str(), item->GetCount(), auctionIds[auction]);
            continue;
        }
        ...
        if (!price)
        {
            sLog->outBasic("%s (x%d) in auction %d: cannot determine price",
                    item->GetTemplate()->Name1.c_str(), item->GetCount(), auctionIds[auction]);
            continue;
        }
        ...
        {
            sLog->outBasic("%s (x%d) in auction %d: price %d > %d (buyout price)",
        ...
        {
            sLog->outBasic("%s (x%d) in auction %d: price %d > %d (available money)",
        ...
        {
            sLog->outBasic("%s (x%d) in auction %d: %d (bid) > %d (minBid)",
        ...
        {
            sLog->outBasic("%s (x%d) in auction %d: %d (startbid) > %d (minBid)",
        ...
        if (time(0) < buytime)
        {
            sLog->outBasic("%s (x%d) in auction %d: will buy/bid in %d seconds",
        ...
        if (!bidder)
        {
            sLog->outBasic("No bidders for auction %d", auctionIds[auction]);
            break;
        }
        ...
        {
            sLog->outBasic("AhBot %d won %s (x%d) in auction %d for %d",
        ...
        {
            sLog->outBasic("AhBot %d placed bid %d for %s (x%d) in auction %d",
...
int AhBot::AddAuction(int auction, Category* category, ItemTemplate const* proto)
{
    ...
    sLog->outBasic("AddAuction: market price adjust");
	...
	auctionEntry->item_guidlow = item->GetGUIDObject().GetCounter();
    ...
    sLog->outBasic("AhBot %d added %d of %s to auction %d for %d..%d", owner, stackCount, proto->Name1.c_str(), auctionIds[auction], bidPrice, buyoutPrice);
    return 1;
}
...
void AhBot::HandleCommand(string command)
{
    ...
    if (!itemId)
    {
        sLog->outBasic("ahbot stats - show short summary");
        sLog->outBasic("ahbot expire - expire all auctions");
        sLog->outBasic("ahbot update - update all auctions");
        sLog->outBasic("ahbot <itemId> - show item price");
        return;
    }
...
void AhBot::Expire(int auction)
{
    ...
    sLog->outBasic("%d auctions marked as expired in auction %d", count, auctionIds[auction]);
}
...
void AhBot::PrintStats(int auction)
{
    ...
    AuctionHouseObject* auctionHouse = sAuctionMgr->GetAuctionsMap(auction);
    sLog->outBasic("%d auctions available on auction house %d", auctionHouse->Getcount(), auctionIds[auction]);
}
...
void AhBot::AddToHistory(AuctionEntry* entry, uint32 won)
{
    ...
    sLog->outBasic("AddToHistory: market price adjust");
    ...
    CharacterDatabase.PExecute("INSERT INTO ahbot_history (buytime, item, bid, buyout, category, won, auction_house) "
        "VALUES ('%u', '%u', '%u', '%u', '%s', '%u', '%u')",
        now, entry->item_template, entry->bid ? entry->bid : entry->startbid, entry->buyout,
...
void AhBot::LoadRandomBots()
{
    ...
    sLog->outBasic("{A=%d,H=%d,N=%d} bidders loaded", bidders[1].size(), bidders[2].size(), bidders[3].size());
}
