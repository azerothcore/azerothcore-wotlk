...
bool AhBotConfig::Initialize()
{
    string error;
	vector<string> args;
    if (!config.LoadInitial("Settings/bot_ahbot.conf"))
    {
        sLog->outBasic("AhBot is Disabled. Unable to open configuration file bot_ahbot.conf");
        return false;
    }
    ...
    if (!enabled)
        sLog->outBasic("AhBot is Disabled in ahbot.conf");
