...
void ItemBag::Init(bool silent)
{
    ...
    sLog->outBasic("Loading/Scanning %s...", GetName().c_str());
    ...
    for (int i = 0; i < CategoryList::instance.size(); i++)
    {
        ...
        sLog->outBasic("loaded %d %s items", content[category].size(), category->GetDisplayName().c_str());
    }
}
...
void InAuctionItemsBag::Load()
{
    ...
    {
        ItemTemplate const* proto = sObjectMgr->GetItemTemplate(itr->second->item_template);
        if (!proto)
            continue;
