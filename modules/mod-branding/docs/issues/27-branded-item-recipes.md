...
**Status:** implemented (resources, Branded outputs, `branding_recipe` repoint, config) — upgrade-curve
tuning (#05) still open · **Deps:** #09 (`EconomyMgr`/`RecipeBook` loader, shipped) · **Parallel-safe:** yes (data + conf) · **Size:** M
**Companion:** #29 (native profession crafting — the client-coupled half of the initial-craft surface)

> **Implementation note.** Resources (`190000` Material BoE / `190001` Fragment BoA), Branded outputs
> (`190010–190012`) and recipe patterns (`190050–190052`) ship in
> `modules/mod-branding/data/sql/db-world/2026_06_22_00_native_craft.sql`; the `branding_recipe` rows are
> repointed in the #09 core pending rev. All generated by `tools/branding-craft` (see #29). Config
> (`Branding.Economy.MaterialItemId`/`FragmentItemId`, `Branding.Event.RewardItemId`) repointed off the
> placeholder cloth. Scope item 5 (upgrade cost curve, #05) is the remaining open piece.
