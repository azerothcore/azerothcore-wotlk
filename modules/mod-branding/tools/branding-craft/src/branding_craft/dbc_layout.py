"""Ordered DBC column layouts (stock 3.3.5a), used by the binary DBC patcher.

Generated from the AzerothCore base schema (data/sql/base/db_world/{spell_dbc,
skilllineability_dbc}.sql); the field ORDER is the on-disk DBC field order. All fields are
4-byte in these two DBCs, so a column index maps to byte offset index*4.
"""
