...
| world/dalaran | Dalaran's faction guards eject a hostile player from the Silver Enclave and Sunreaver's Sanctuary, and from nowhere else — public mailboxes, the street and the sewers are left alone. Both directions are asserted: only checking public ground would pass a build where the guards never fire | P2 | covered (`TestAC_4467_*`) | #4467 |
