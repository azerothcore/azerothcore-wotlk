...
| instances/ulduar/XT | Boombots in both raid sizes survive nonlethal hits, explode once on lethal damage or arrival at XT, hit players at 5/9.5 yards but not 10.5/15/25 yards (measured center-to-center on the same floor), and chain to other bots while retaining Scrapbot kill credit; checks AC's existing damage estimate, not an exact retail value | P2 | covered (`TestAC_27699_XTBoombotExplosion`, `/raid_10/range` and `/raid_25/range` for range only); live execution pending | #27699 |
