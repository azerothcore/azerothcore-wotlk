...
// Issue: https://github.com/azerothcore/azerothcore-wotlk/issues/27368
// Mechanolift 304-A (33214) is a hostile NPC vehicle that had no AI at all; the
// default VehicleAI has no evade logic, so damaging one without killing it left
// the attacker in combat with it until it died. The fix gives it a passive
// SmartAI whose timed event stops combat 10s after the LAST hit (each hit
// refreshes the timer), in place (no fly-home, no vehicle kit reset), leaving
// the lift attackable afterwards.
// Spawned at the Ulduar entrance; nearest other spawns are friendly Earthen
// 40y+ away, so the combat oracle stays clean.
func TestAC_27368_MechanoliftCombatDrop(t *testing.T) {
	meta.Begin(t, meta.TestMeta{
	...
	const npcMechanolift = uint32(33214)
	...
	lift := bot.Spawn(t, npcMechanolift, 15*time.Second)
	bot.CombatReady(t)
	self := bot.World.CharGUID()

	hp, maxHP := bot.WaitUnitHPKnown(t, lift, 10*time.Second)
	if hp == 0 {
		e2eharness.Preconditionf(t, "spawned mechanolift 0x%X not alive (hp=%d/%d)", lift, hp, maxHP)
	}

	// Nonlethal hit (lift has ~6k HP): forms mutual combat refs like a
	// demolisher volley does.
	bot.Damage(t, lift, 500)
	first := time.Now()
	bot.WaitUnitCombat(t, self, 10*time.Second)
	t.Logf("bot in combat after damaging mechanolift 0x%X (hp was %d/%d)", lift, hp, maxHP)

	if hp, _ := bot.UnitHP(lift); hp == 0 {
		e2eharness.Preconditionf(t, "mechanolift died to the nonlethal hit (combat drop unjudgeable)")
	}

	// Refresh slice: a second hit inside the 10s window must re-arm the
	// combat-stop timer, so combat must still hold ~12s after the first hit
	// (an unrefreshed timer would have dropped it at ~10s). Deliberate
	// timing oracle with documented thresholds.
	time.Sleep(6 * time.Second)
	bot.Damage(t, lift, 500)
	time.Sleep(6 * time.Second)
	if !bot.UnitInCombat(self) {
		e2eharness.Assertf(t,
			"combat dropped ~%s after the first hit despite a second hit at ~6s (timer not refreshed)",
			time.Since(first).Round(time.Second))
	}
	t.Logf("still in combat at %s (timer refreshed by second hit)", time.Since(first).Round(time.Millisecond))

	// Fixed core: the lift stops combat 10s after the LAST hit, ending its
	// combat refs; the bot must leave combat shortly after (~16s from the
	// first hit here).
	...
	if !dropped {
		e2eharness.ConfirmedBugf(t, 27368,
			"bot still in combat %s after damaging mechanolift 0x%X without killing it",
			time.Since(first).Round(time.Second), lift)
	}
	t.Logf("bot left combat %s after the first hit", time.Since(first).Round(time.Millisecond))

	// Second slice: the combat drop must leave the lift attackable, or it
	// could never be farmed again (e.g. a full evade would leave it briefly
	// immune, and a stuck evade state would deflect all further hits).
	// Polled with Assertf (not WaitUnitCombat) so a regression here reports
	// as a product failure, not a precondition.
	bot.Damage(t, lift, 500)
	...
	if !reengaged {
		e2eharness.Assertf(t,
			"bot did not re-enter combat after second hit on mechanolift 0x%X (not attackable after combat drop?)", lift)
	}
	if hp, _ := bot.UnitHP(lift); hp == 0 {
		e2eharness.Assertf(t, "mechanolift died to second nonlethal hit")
	}
	t.Logf("PASS AC#27368 mechanolift refreshes on hit, drops combat after the last hit, stays attackable")
}
