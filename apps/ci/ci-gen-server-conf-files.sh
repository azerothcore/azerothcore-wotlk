...
if [[ $APP_NAME == "worldserver" || $APP_NAME == "dbimport" ]]; then
    cp ./env/dist/$CONFIG_FOLDER/modules/playerbots.conf.dist ./env/dist/$CONFIG_FOLDER/modules/playerbots.conf
    sed -i "s/127.0.0.1;3306;acore;acore/localhost;3306;root;$MYSQL_ROOT_PASSWORD/" ./env/dist/$CONFIG_FOLDER/modules/playerbots.conf
fi
