...
if [[ $CONTINUOUS_INTEGRATION ]]; then
  ...
  libncurses5-dev libboost-system1.6*-dev libboost-program-options1.6*-dev libboost-iostreams1.6*-dev
else
  case $UBUNTU_VERSION in
    "14.04")
      ...
      curl unzip libboost-all-dev
      ;;
    *)
      sudo apt-get install -y git cmake make gcc g++ clang libmysqlclient-dev \
      libssl-dev libbz2-dev libreadline-dev libncurses-dev \
      mysql-server libace-6.* libace-dev curl unzip libboost-all-dev
      ;;
  esac
fi
