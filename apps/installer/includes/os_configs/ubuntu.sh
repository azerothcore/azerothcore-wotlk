...
if [[ $CONTINUOUS_INTEGRATION || $DOCKER ]]; then
  sudo apt update && sudo apt-get -y install --no-install-recommends build-essential cmake-data libboost-dev \
  ...
  case $UBUNTU_VERSION in
     "20.04")
       ...
       ;;
     *)
       sudo apt update && sudo apt-get install -y --no-install-recommends g++ gdb gdbserver gcc git \
       libboost-dev libbz2-dev libncurses-dev libreadline-dev \
       libssl-dev mysql-server
       ;;
  esac
