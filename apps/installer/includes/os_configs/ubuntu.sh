...
if [[ $TRAVIS && $CONTINUOUS_INTEGRATION ]]; then
  ...
  case $UBUNTU_VERSION in
    "14.04")
      ...
      libssl-dev libmysqlclient-dev libmysql++-dev libreadline6-dev zlib1g-dev libbz2-dev libace-dev mysql-server libncurses-dev curl 
      ;;
    *)
      sudo apt-get install -y git cmake make gcc g++ clang libmysqlclient-dev \
      libssl-dev libbz2-dev libreadline-dev libncurses-dev \
      mysql-server libace-6.* libace-dev curl
      ;;
  esac
fi
