...
3. Conventions (linted): every `INSERT` preceded by a matching `DELETE` (idempotency); spawn (`creature`, `gameobject`) `DELETE` filters on both `id` and `guid`, `UPDATE` on `id` whenever it filters `guid`; `=`/`IN`/`BETWEEN` only, never `OR`; no double semicolons; no multiple blank lines; InnoDB engine.
