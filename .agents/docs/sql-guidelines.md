...
- `creature_template_model`: rows were generated from the old `modelid1`-`modelid4` columns, with `creature_template.VerifiedBuild` copied onto every `Idx`; a stamp on `Idx` >= 1 is therefore not sniff evidence, and equal probabilities are that import's default, not a randomization claim. Authority is the creature query response's per-index `Probability`, which Classic and retail sniffs carry and 3.3.5 clients omit; absent a sniff, a Classic-era stamp on `Idx` 0 alone flags the rest as unverified. Suppress a model by setting its `Probability` to 0 and keeping the row, never by deleting it; a creature's only row stays at 1, since a zero total makes the loader reset every row to 1.0 and log an `sql.sql` error.
